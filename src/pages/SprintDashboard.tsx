...
const columns: ColumnConfig[] = [
  { key: "id", label: "Sprint Id", sortable: true, draggable: true, defaultVisible: true },
  { key: "title", label: "Sprint Title", sortable: true, draggable: true, defaultVisible: true },
  { key: "status", label: "Status", sortable: true, draggable: true, defaultVisible: true },
  { key: "sprint_owner", label: "Sprint Owner", sortable: true, draggable: true, defaultVisible: true },
  ...
  { key: "number_of_projects", label: "Number Of Projects", sortable: true, draggable: true, defaultVisible: true },
];
...
export const SprintDashboard = () => {
  ...
  const { loading: fetchLoading } = useSelector((state: RootState) => state.fetchSprints);
  const { loading: createLoading } = useSelector((state: RootState) => state.createSprint);
  const { loading: updateLoading } = useSelector((state: RootState) => state.updateSprint);
  ...
  const fetchData = useCallback(
    async (page = 1) => {
      try {
        const response = await dispatch(fetchSprints({ token, baseUrl, page })).unwrap();
        const sprintList = Array.isArray(response) ? response : (response.sprints ?? []);
  ...
  const handlePageChange = async (page: number) => {
    if (page < 1 || page > pagination.total_pages || page === pagination.current_page || fetchLoading) return;
  ...
  const handleUpdateSprint = async (id: string, data: Partial<Sprint>) => {
    try {
      await dispatch(updateSprint({ token, baseUrl, id, data: { sprint: data } })).unwrap();
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    try {
      await dispatch(updateSprintStatus({ token, baseUrl, id, data: { status: newStatus.toLowerCase() } })).unwrap();
  ...
  const renderActions = (item: any) => (
    <div className="flex items-center justify-center gap-2">
      <Button size="sm" variant="ghost" className="p-1" onClick={() => navigate(`/vas/sprint/details/${item.id}`)}>
        <Eye className="w-4 h-4" />
      </Button>
      <Button size="sm" variant="ghost" className="p-1" onClick={() => navigate(`/vas/sprint/${item.id}`)}>
    ...
    switch (columnKey) {
      ...
      case "status": {
        const statusColorMap: Record<string, { dot: string }> = {
          ...
          started: { dot: "bg-green-500" },
          stopped: { dot: "bg-red-400" },
        };
        const rawStatus = (item.status ?? "").toLowerCase();
        const colors = statusColorMap[rawStatus] ?? { dot: "bg-gray-400" };
        return (
            ...
            <Select
              ...
              renderValue={(value) => (
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  <span className={`inline-block w-2 h-2 rounded-full ${statusColorMap[value as string]?.dot ?? "bg-gray-400"}`} />
              ...
              sx={{
                fontSize: "0.875rem",
                cursor: "pointer",
                "& .MuiSelect-select": { padding: "4px 0", display: "flex", alignItems: "center", gap: "8px" },
              }}
            >
              {statusOptions.map((opt) => (
                <MenuItem key={opt.value} value={opt.value} sx={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  <span className={`inline-block w-2 h-2 rounded-full ${statusColorMap[opt.value]?.dot ?? "bg-gray-500"}`} />
                  <span>{opt.label}</span>
                </MenuItem>
              ))}
            </Select>
  ...
  const leftActions = (
    <Button className="bg-[#C72030] hover:bg-[#A01020] text-white" onClick={() => setAddSprintModalOpen(true)}>
  ...
  const renderEditableCell = (columnKey: string, value: any, onChange: any) => {
    if (columnKey === "status") {
      return (
        <Select value={value?.toLowerCase() || ""} onChange={(e) => onChange(e.target.value)} displayEmpty size="small" sx={{ minWidth: 120 }}>
    ...
    if (columnKey === "start_date" || columnKey === "end_date") {
      return <TextField type="date" value={value || ""} onChange={(e) => onChange(e.target.value)} size="small" sx={{ minWidth: 120 }} />;
    }
    ...
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50 min-w-[180px]">
          <div className="py-2">
            <button onClick={() => { setSelectedView("Kanban"); setIsOpen(false); }} className="flex items-center gap-3 w-full px-4 py-2 text-left hover:bg-gray-50">
              <div className="w-4 flex justify-center"><ChartNoAxesColumn className="rotate-180 text-[#C72030]" /></div>
              <span className="text-gray-700">Kanban</span>
            </button>
            <button onClick={() => { setSelectedView("List"); setIsOpen(false); }} className="flex items-center gap-3 w-full px-4 py-2 text-left hover:bg-gray-50">
              <div className="w-4 flex justify-center"><List className="w-4 h-4 text-red-600" /></div>
    ...
    if (showEllipsis) {
      items.push(
        <PaginationItem key={1} className="cursor-pointer">
          <PaginationLink onClick={() => handlePageChange(1)} isActive={currentPage === 1} aria-disabled={isLoading} className={isLoading ? "pointer-events-none opacity-50" : ""}>1</PaginationLink>
        </PaginationItem>
      );
      if (currentPage > 4) {
        ...
        for (let i = 2; i <= Math.min(3, totalPages - 1); i++) {
          items.push(
            <PaginationItem key={i} className="cursor-pointer">
              <PaginationLink onClick={() => handlePageChange(i)} isActive={currentPage === i} aria-disabled={isLoading} className={isLoading ? "pointer-events-none opacity-50" : ""}>{i}</PaginationLink>
            </PaginationItem>
          );
        }
      }
      ...
      if (totalPages > 1) {
        items.push(
          <PaginationItem key={totalPages} className="cursor-pointer">
            <PaginationLink onClick={() => handlePageChange(totalPages)} isActive={currentPage === totalPages} aria-disabled={isLoading} className={isLoading ? "pointer-events-none opacity-50" : ""}>{totalPages}</PaginationLink>
          </PaginationItem>
        );
      }
    } else {
          ...
          <PaginationContent>
            <PaginationItem>
              <PaginationPrevious
                ...
                className={pagination.current_page === 1 || isLoading ? "pointer-events-none opacity-50" : "cursor-pointer"}
              />
            </PaginationItem>
            ...
            <PaginationItem>
              <PaginationNext
                ...
                className={pagination.current_page === pagination.total_pages || isLoading ? "pointer-events-none opacity-50" : "cursor-pointer"}
              />
            </PaginationItem>
          </PaginationContent>
        </Pagination>
