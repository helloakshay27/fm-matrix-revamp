...
const GoalSection = ({ config, wrapClass }: { config: GoalSectionConfig; wrapClass: string }) => {
    ...
    const fetchStrategicGoal = useCallback(async () => {
        try {
            ...
            setStrategicGoals(mapped.filter((sg) => config.strategicPeriodFilter(sg.period)));
        } catch (err) { console.error("fetchStrategicGoal:", err); }
    }, [config]);
    ...
    const fetchGoals = useCallback(async () => {
        ...
        try {
            ...
            setAllGoals(mapped);
            setInitiatives(mapped.filter((g) => g.period === config.operationalPeriod));
        } catch (err: any) {
    ...
    useEffect(() => {
        ...
        fetchGoals();
        fetchUsers();
        const handler = () => { fetchStrategicGoal(); fetchGoals(); };
    ...
    const sliderPatchTimer = useRef<Record<number, ReturnType<typeof setTimeout>>>({});

    const handleCardSlider = (id: number, val: string) => {
        const clamped = clampProgress(val);
        // Immediate optimistic UI — no flicker
        setInitiatives((prev) => prev.map((i) => (i.id === id ? { ...i, progress: clamped } : i)));
        // Cancel any pending API call for this id, fire only after user stops sliding (600ms)
        if (sliderPatchTimer.current[id]) clearTimeout(sliderPatchTimer.current[id]);
        sliderPatchTimer.current[id] = setTimeout(async () => {
            try {
                ...
                // No fetchGoals() here — avoids page refresh/flicker on success or failure
            } catch { /* silent fail — user's slider value is already set */ }
        }, 600);
    };

    const closeModal = () => {
        setActiveModal(null); setSaveError(null); setTempGoal(null);
        setTempGoalDate(""); setEditingGoalId(null); setTempStrategic(null); setStrategicGoalId(null);
    };
    ...
    const openStrategicModal = (sg?: StrategicGoalData) => {
        ...
        if (sg) {
            ...
            setTempStrategic({ title: "", period: config.defaultStrategicPeriod, targetDate: "", revenueTarget: "", profitTarget: "" });
            setLinkedStrategicInitiatives([]);
        }
        setActiveModal("edit_strategic");
    };

    const confirmDeleteStrategic = (id: number) => { setStrategicGoalId(id); setActiveModal("confirm_delete_strategic"); };
    ...
    const executeDeleteStrategic = async () => {
        ...
        try {
            ...
            setStrategicGoalId(null); setLinkedStrategicInitiatives([]);
            dispatchGoalsUpdated();
        } catch (err: any) { alert("Failed to delete: " + (err.message || "Unknown error")); }
        finally { setIsDeleting(false); closeModal(); }
    };

    const openEditGoalModal = (goal: GoalItem) => {
        setTempGoal({ ...goal }); setTempGoalDate(goal.targetDate || "");
        setEditingGoalId(goal.id ?? null); setSaveError(null); setActiveModal("goal_details");
    };

    const openCreateGoalModal = () => {
        setTempGoal({ title: "", progress: 0, description: "", targetValue: "100", currentValue: "0", unit: "%", period: config.operationalPeriod, status: "On Track", ownerId: "", updateRemarks: "" });
    ...
    const saveStrategicGoal = async () => {
        ...
        try {
            ...
            const keyInitiatives = allGoals.filter((g) => linkedStrategicInitiatives.includes(g.id as number)).map((g) => ({ id: g.id, title: g.title }));
            const payload = {
                goal: {
                    goal_category: "strategic", title: tempStrategic.title.trim(),
                    description: "Strategic objective", target_date: apiTargetDate,
                    revenue_target: Number(tempStrategic.revenueTarget) || 0,
                    profit_target: Number(tempStrategic.profitTarget) || 0,
                    target_value: 100, current_value: 0, unit: "percent",
                    period: tempStrategic.period, status: "on_track", owner_id: null,
                    update_remarks: "", key_initiative_goals: keyInitiatives,
                },
            };
            let res: Response;
            if (strategicGoalId) {
            ...
            if (!res.ok) throw new Error(`API error ${res.status}`);
            closeModal(); dispatchGoalsUpdated();
    ...
    const saveGoalDetails = async () => {
        ...
        const payload = {
            goal: {
                goal_category: "operational", title: tempGoal.title.trim(),
                ...
                unit: tempGoal.unit || "%", period: config.operationalPeriod,
    ...
    return (
        <div className={wrapClass} style={{ padding: "24px 0", fontFamily: C.font }}>
            <div className="rounded-2xl overflow-hidden shadow-sm mt-6 border" style={{ background: C.cardBg, borderColor: C.borderLgt }}>
                ...
                <div className="px-6 py-4 border-b flex items-center justify-between" style={{ borderColor: C.borderLgt, background: C.primaryBg }}>
                    <div className="flex items-center gap-2">
                        <HeaderTargetIcon />
                        <h2 className="font-black text-lg m-0" style={{ color: C.textMain }}>{config.sectionTitle}</h2>
                        <span
                            onMouseEnter={(e) => {
                ...
                {isInfoHovered && ReactDOM.createPortal(
                    <div style={{ position: "absolute", top: infoPos.top, left: infoPos.left, transform: infoPos.transform, zIndex: 99999, background: "#16102b", color: "#fff", borderRadius: 12, boxShadow: "0 10px 30px rgba(0,0,0,0.3)", padding: "16px", width: 340, textAlign: "center", fontFamily: "'Poppins', sans-serif", pointerEvents: "none", border: "1px solid rgba(218,119,86,0.2)" }}>
                        <h4 style={{ margin: "0 0 10px 0", fontSize: 13, fontWeight: 800, color: "#fff" }}>{config.tooltipTitle}</h4>
                        {config.tooltipLines.map((line, i) => (
                            <p key={i} style={{ margin: "0 0 8px 0", fontSize: 12, lineHeight: 1.5, color: i === config.tooltipLines.length - 1 ? "#9ca3af" : "#d1d5db", fontStyle: i === config.tooltipLines.length - 1 ? "italic" : "normal" }}>{line}</p>
                        ))}
                    </div>,
                    document.body
                )}
                ...
                <div className="p-6">
                    ...
                    <div className="mb-8">
                        {isFetching ? (
                                ...
                                {strategicGoals.map((sg) => (
                                    <div key={sg.id} className="bg-white rounded-xl p-5 flex justify-between items-center group transition-all" style={{ border: `1px solid ${C.borderLgt}`, boxShadow: "0 2px 8px rgba(0,0,0,0.02)" }}>
                                        ...
                                        <div className="flex gap-2">
                                            <button onClick={() => openStrategicModal(sg)} className="p-2 bg-gray-50 hover:bg-gray-100 rounded-lg cursor-pointer transition-colors text-gray-500 border" style={{ borderColor: C.borderLgt }} title="Edit"><EditIcon /></button>
                                            <button onClick={() => confirmDeleteStrategic(sg.id as number)} className="p-2 bg-gray-50 hover:bg-red-50 rounded-lg cursor-pointer transition-colors text-gray-500 hover:text-red-500 border" style={{ borderColor: C.borderLgt }} title="Delete"><TrashIcon /></button>
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="text-center py-10 rounded-2xl mb-4" style={{ border: `2px dashed ${C.primaryBord}`, backgroundColor: "#fffaf9" }}>
                                <TargetLargeIcon />
                                <h3 className="text-[16px] font-bold mb-1" style={{ color: C.textMain }}>{config.emptyStrategicTitle}</h3>
                                <p className="text-[13px] mb-5" style={{ color: C.textMuted }}>{config.emptyStrategicSubtitle}</p>
                    ...
                    <div className="mb-3 flex items-center gap-2">
                        <span className="text-[10px] font-black uppercase tracking-[0.15em]" style={{ color: "#070707" }}>{config.initiativesSectionLabel}</span>
                    ...
                    <Modal onClose={closeModal}>
                        <div style={{ background: "#fff", borderRadius: 16, boxShadow: "0 24px 64px rgba(0,0,0,0.18)", width: "100%", maxWidth: 380, overflow: "hidden", fontFamily: C.font }}>
                            <div style={{ padding: "28px 28px 8px", textAlign: "center" }}>
                                ...
                                <div style={{ fontSize: 15, fontWeight: 700, color: C.textMain, marginBottom: 8 }}>{config.deleteStrategicTitle}</div>
                            ...
                            <div style={{ padding: "20px 28px 28px", display: "flex", justifyContent: "center", gap: 12 }}>
                                <button onClick={executeDeleteStrategic} disabled={isDeleting} style={{ padding: "10px 24px", fontWeight: 700, color: "#fff", background: "#dc2626", border: "none", borderRadius: 10, fontSize: 13, cursor: isDeleting ? "not-allowed" : "pointer", opacity: isDeleting ? 0.7 : 1, display: "flex", alignItems: "center", gap: 8, fontFamily: C.font }}>
                                    {isDeleting && <LoaderIcon />}{isDeleting ? "Deleting..." : "Delete"}
                                </button>
                    ...
                    <Modal onClose={closeModal}>
                        <div className="st-modal-box" style={{ maxWidth: 600 }}>
                            <div className="flex justify-between items-center px-6 py-5 border-b bg-white" style={{ borderColor: C.primaryBord }}>
                                <div>
                                    <h2 className="font-black text-[17px] m-0" style={{ color: C.textMain }}>{config.modalStrategicTitle(isEditingStrategic)}</h2>
                                    <p style={{ margin: "4px 0 0", fontSize: 12, color: C.textMuted }}>{config.modalStrategicSubtitle(isEditingStrategic)}</p>
                                </div>
                            ...
                            <div className="p-6 space-y-5 overflow-y-auto overflow-x-hidden" style={{ maxHeight: "65vh" }}>
                                ...
                                <div>
                                    ...
                                    <p className="text-[11px] mb-2" style={{ color: C.textMuted }}>Link operational goals as key initiatives</p>
                    ...
                    <Modal onClose={closeModal}>
                        <div style={{ background: "#fff", borderRadius: 16, boxShadow: "0 24px 64px rgba(0,0,0,0.18)", width: "100%", maxWidth: 640, display: "flex", flexDirection: "column", maxHeight: "90vh", overflow: "hidden", fontFamily: C.font }}>
                            <div style={{ padding: "28px 28px 0", position: "relative" }}>
                                ...
                                <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: C.textMain }}>{config.modalInitiativeTitle(!!editingGoalId)}</h2>
                                <p style={{ margin: "6px 0 0", fontSize: 13, color: C.textMuted }}>Set a measurable target</p>
                            </div>
...
const mediumTermConfig: GoalSectionConfig = {
    ...
    modalInitiativeTitle: (editing) => editing ? "Edit Operational Goal" : "Create New Operational Goal",
    modalStrategicTitle: (editing) => editing ? "Edit Strategic Goal" : "Add Strategic Goal",
    modalStrategicSubtitle: (editing) => editing ? "Update your strategic direction" : "Define your core objective and period",
...
const shortTermConfig: GoalSectionConfig = {
    ...
    modalInitiativeTitle: (editing) => editing ? "Edit Annual Initiative" : "Create Annual Initiative",
    modalStrategicTitle: (editing) => editing ? "Edit Annual Strategic Goal" : "Add Annual Strategic Goal",
    modalStrategicSubtitle: (editing) => editing ? "Update your annual direction" : "Define your core objective for this year",
...
const quarterlyConfig: GoalSectionConfig = {
    ...
    strategicPeriodFilter: (p) => p === "this_quarter" || p === "quarterly" || (p || "").toLowerCase().includes("quarter"),
    ...
    modalInitiativeTitle: (editing) => editing ? "Edit Quarterly Initiative" : "Create Quarterly Initiative",
    modalStrategicTitle: (editing) => editing ? "Edit Quarterly Strategic Goal" : "Add Quarterly Strategic Goal",
    modalStrategicSubtitle: (editing) => editing ? "Update your quarterly direction" : "Define your core objective for this quarter",
