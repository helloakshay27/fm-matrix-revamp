...
// Medium-term period value for API — matches actual API response
...
const mapPeriodToApi = (label: string): string => {
  ...
  return map[label] || MEDIUM_TERM_PERIOD;
};
...
interface Goal {
  ...
  currentValue?: string;
  unit?: string;
  period?: string;        // API period value e.g. '3_to_5_years'
  periodLabel?: string;   // Display label e.g. '3-5 Years'
...
export const MediumTermSection = () => {
  ...
  // Linked initiatives (for strategic goal modal checkbox list)
  const [linkedInitiatives, setLinkedInitiatives] = useState<number[]>([]);

  // ── Fetch goals — filter medium-term (3-5 years) ──
  const fetchGoals = useCallback(async () => {
    ...
    try {
      ...
      // Filter medium-term / 3-5 year goals only
      ...
      const mapped: Goal[] = mediumGoals.map((g: any, idx: number) => ({
        id: g.id ?? idx + 1,
        ...
        targetDate: g.target_date || '',  // YYYY-MM-DD
      ...
      console.error('[MediumTermSection] fetch error:', err);
  ...
  const saveGoalDetails = async () => {
    ...
    const payload = {
      goal: {
        ...
        unit: tempGoal.unit || '%',
        period: mapPeriodToApi(tempGoal.periodLabel || '3-5 Years'),
                          ...
                          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                            ...
                            {(goal.ownerName || goal.targetDate) && (
                              <span className="text-xs font-medium" style={{ color: C.textMuted }}>
                                ...
                                {goal.ownerName}
                                {goal.targetDate && (
                                  <span className="ml-1">📅 {apiDateToDisplay(goal.targetDate)}</span>
                                )}
                              </span>
                            )}
                          </div>
              ...
              <div style={{ padding: '24px 28px', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                {editingGoalId && (
                  <div style={{ background: '#f9fafb', borderRadius: 10, padding: '16px 18px', border: '1px solid #e5e7eb' }}>
                    ...
                    <div
                      className="text-white font-bold text-center py-2 rounded-lg text-[13px] mt-4"
                ...
                {/* Link Initiatives — shown in both create & edit */}
                {goals.length > 0 && (
                  <div
                  ...
                  >
                    <div
                      ...
                      style={{ borderColor: C.borderLgt, background: '#f9fafb' }}
                    >
                      <h3 className="font-bold text-[13px] m-0" style={{ color: C.textMain }}>
                        Link Initiatives (Optional)
                      </h3>
                      <p className="text-[12px] mt-0.5 m-0" style={{ color: C.textMuted }}>
                        Select goals that contribute to this medium-term goal
                      </p>
                    </div>
                    <div className="p-2 max-h-40 overflow-y-auto space-y-1">
                      {goals
                        ...
                        .map((g) => (
                          <label
                            key={g.id}
                            className="flex items-start gap-3 p-2.5 hover:bg-gray-50 rounded-lg cursor-pointer transition-colors"
                          >
                            <input
                              type="checkbox"
                              checked={linkedInitiatives.includes(g.id as number)}
                              onChange={() => toggleInitiativeLink(g.id as number)}
                              className="mt-0.5 w-4 h-4 cursor-pointer rounded"
                              style={{ accentColor: C.primary }}
                            />
                            <div>
                              <div className="text-[13px] font-bold leading-tight" style={{ color: C.textMain }}>
                                {g.title}
                              </div>
                              {g.periodLabel && (
                                <span
                                  className="text-[10px] font-bold px-2 py-0.5 rounded-md inline-block mt-1 uppercase tracking-wide"
                                  style={{ background: C.primaryTint, color: C.primary }}
                                >
                                  {g.periodLabel}
                                </span>
