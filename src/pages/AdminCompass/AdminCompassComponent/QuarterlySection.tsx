...
export const QuarterlySection = () => {
  ...
  const fetchStrategicGoal = useCallback(async () => {
    try {
      ...
      if (data) setStrategicGoal({title:data.values?.[0]||'',targetDate:data.target_date||''});
      else setStrategicGoal({title:'',targetDate:''});
  ...
  const openStrategicModal=()=>{setTempStrategic({title:strategicGoal.title,targetDate:apiDateToDisplay(strategicGoal.targetDate)});setActiveModal('edit_strategic');};
  ...
  const saveStrategicGoal=async()=>{
    ...
    if (!tempStrategic.title.trim()){setSaveError('Statement cannot be empty.');return;}
    setIsSaving(true);setSaveError(null);
    try {
      const payload:any={extra_field:{group_name:'quarterly_strategic',values:[tempStrategic.title.trim()]}};
      ...
      setStrategicGoal({title:tempStrategic.title.trim(),targetDate:formatDateForApi(tempStrategic.targetDate.trim())});
      closeModal();
  ...
  return (
    <div className="quarterly-wrap" style={{padding:'24px 0',fontFamily:C.font}}>
      ...
      {/* ── Teal section card (replaces orange gradient) ── */}
      <div className="rounded-2xl overflow-hidden shadow-sm mt-6" style={{background:C.tealBg}}>
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={openStrategicModal} className="p-1.5 rounded-xl bg-white/30 hover:bg-white/50 transition-colors border border-white/20" style={{color:C.textMain}}>
              <EditIcon/>
            </button>
          </div>
          ...
          <div className="mb-5 pl-[36px]">
            {isFetching?<div className="st-skeleton h-7 w-2/3"/>:(
              <p className="text-lg font-black leading-snug" style={{color:C.textMain}}>
                {strategicGoal.title||<span className="opacity-50 font-normal text-sm italic">No quarterly priorities set — click edit to add.</span>}
              </p>
            )}
          </div>
        ...
        {activeModal==='edit_strategic'&&tempStrategic&&(
          <Modal onClose={closeModal}>
            <div className="q-modal-box" style={{maxWidth:520}}>
              <div className="flex justify-between items-center px-6 py-5 border-b bg-white" style={{borderColor:C.primaryBord}}>
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="font-black text-[17px] m-0" style={{color:C.textMain}}>Edit Quarterly Priorities</h2>
                </div>
              ...
              <div className="p-6 space-y-5 overflow-y-auto">
                ...
                <div>
                  <label className="block text-[12px] font-black mb-1.5" style={{color:C.textMain}}>Strategic Statement <span style={{color:C.primary}}>*</span></label>
                  <textarea value={tempStrategic.title} onChange={e=>setTempStrategic({...tempStrategic,title:e.target.value})} placeholder="e.g. Achieve ₹100Cr Revenue this quarter" className="q-input font-black" style={{minHeight:80,resize:'vertical'}}/>
                </div>
                <div>
                  <label className="block text-[12px] font-black mb-1.5" style={{color:C.textMain}}>Target Date</label>
                  <DatePicker value={tempStrategic.targetDate} onChange={v=>setTempStrategic({...tempStrategic,targetDate:v})} placeholder="Select target date"/>
                </div>
              </div>
              <div className="p-5 flex justify-end gap-3 border-t bg-white" style={{borderColor:C.primaryBord}}>
                ...
                <button onClick={saveStrategicGoal} disabled={isSaving} style={{...modalBtnBase,color:'#fff',background:C.primary,opacity:isSaving?0.7:1}} onMouseEnter={e=>{if(!isSaving)e.currentTarget.style.background=C.primaryHov;}} onMouseLeave={e=>e.currentTarget.style.background=C.primary}>
                  {isSaving&&<LoaderIcon/>}{isSaving?'Saving...':'Save Priorities'}
                </button>
              </div>
            </div>
          </Modal>
