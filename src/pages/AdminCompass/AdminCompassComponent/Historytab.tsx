...
const compileDynamicNotes = (data) => {
  ...
  reports.forEach((r) => {
    ...
    const accomplishments =
      r.report_data?.accomplishments || r.accomplishments || [];
    if (accomplishments.length === 0) text += `- None\n`;
    accomplishments.forEach((a) => {
      text += `✓ ${a.title || a.text || a}\n`;
    });
...
const HistoryTab = () => {
  ...
  const selectedMeetingLabel =
    meetings.find((m) => m.id === selectedMeetingId)?.label ?? "Meeting";

  const missedMembers = meetingData?.missed_members || [];
  const attendeesCount = meetingData?.submitted || 0;
  const isMissedDay =
    meetingData?.date_row?.find((d) => d.full_date === selectedDate)?.status ===
    "missed";
  ...
  const handleOpenEditModal = () => {
    ...
    const journal =
      meetingData?.journal || meetingData?.user_journal || meetingData;
    ...
    setEditForm({
      ...
      accomplishments: (rd.accomplishments || [])
        .map((a) => a.title || a.text || a)
        .join("\n"),
      tomorrow_plan: (rd.tomorrow_plan || [])
        .map((p) => p.title || p.text || p)
        .join("\n"),
      tasks_issues: (rd.tasks_issues || [])
        .map((t) => t.title || t.text || t)
        .join("\n"),
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      const payload = {
        user_journal: {
          ...
          report_data: {
            ...
            tasks_issues: parseAccomplishments(editForm.tasks_issues).map(
              (t) => ({
                ...t,
                status: "open",
              })
      ...
      console.log(
        "[HistoryTab] PUT /user_journals/" + editingJournalId,
  ...
  const handleDeleteNotes = async () => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to delete these notes? (ID: ${journalId})`
      )
    )
      return;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[rgba(218,119,86,0.15)] pb-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative">
            ...
            >
              <option value="">
                {isFetchingMeetings ? "Loading…" : "Select Meeting"}
              </option>
              {meetings.map((m) => (
          ...
          >
            <div>
              ...
              <div className="flex items-center gap-3 mb-2 text-white/90 text-[13px]">
                <span className="px-3 py-0.5 border border-white/40 text-white text-[11px] font-semibold rounded-md shadow-sm">
                  submitted
                </span>
                    ...
                    <div className="flex flex-col gap-1.5">
                      {missedMembers.map((member) => (
                        <span
                          ...
                          className="text-blue-500 text-[14px] hover:underline cursor-pointer w-fit"
                        >
                          {member.name || member}
                        </span>
                      ))}
              ...
              <div className="p-6 flex-1 overflow-y-auto bg-neutral-50/50 space-y-5">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setField("accomplishments", e.target.value)
                    }
