...
const DailyTab = ({ selectedDateId, setSelectedDateId }) => {
  const selectedDate =
    datesData.find((d) => d.id === selectedDateId) || datesData[0];
    ...
    <div className="space-y-5 pb-12">
      <SectionCard>
        ...
        <div className="flex flex-col items-center">
          <div className="flex gap-5 text-xs font-medium flex-wrap justify-center text-neutral-500">
            {[
              ...
              <div key={label} className="flex items-center gap-1.5">
                <span
                  className="w-2.5 h-2.5 rounded-md inline-block border border-black/10"
          ...
          <p className="text-[11px] mt-1.5 italic text-neutral-400">
            Note: Select the date for which users have filled the report, not
            the meeting date.
          </p>
        </div>
      </SectionCard>
      ...
      {selectedDate.status === "missed" && (
        <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4 shadow-sm">
          <div className="flex items-center gap-2 text-orange-600 font-bold text-sm mb-3">
            <AlertTriangle className="w-4 h-4" /> Missed Meetings on{" "}
            {missedDateLabel} (1):
          </div>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        {[
          ...
          <div key={label} className="flex items-center gap-2">
            ...
            <button
              onClick={() =>
                setter(label === "MEETING" ? "All Meetings" : "All Members")
              }
      ...
      <div className="border border-[rgba(218,119,86,0.18)] rounded-2xl shadow-sm overflow-hidden bg-white">
        <div className="p-4 border-b border-[rgba(218,119,86,0.1)] flex justify-between items-start flex-wrap gap-3 bg-[#fffaf8]">
          ...
          <div className="flex items-center gap-2 text-xs font-semibold flex-wrap">
            ...
            <span className="px-2.5 py-1 rounded-xl text-xs font-bold bg-[#f6a67d] text-white shadow-sm">
              Submitted 2
            </span>
          ...
          <div className="bg-white border border-[rgba(218,119,86,0.18)] rounded-2xl overflow-hidden shadow-sm">
            <div className="flex items-center justify-between p-3 border-b border-[rgba(218,119,86,0.1)] bg-[#fffaf8]">
              ...
              <div className="flex gap-2">
                <BtnIcon title="Calendar">
                  <CalendarIcon className="w-3.5 h-3.5 text-[#DA7756]" />
                </BtnIcon>
      ...
      <div className="bg-[#fef6f4] border border-[rgba(218,119,86,0.25)] rounded-2xl p-4 shadow-sm">
        <div className="flex items-center gap-2 text-[#DA7756] font-bold text-sm mb-3">
          ...
          Submit Reports ({failedMembers.length}):
        </div>
        <div className="flex flex-wrap gap-2">
          {failedMembers.map((member) => (
            <button
              key={member}
              className="text-[11px] font-bold px-3 py-1.5 rounded-xl border border-[rgba(218,119,86,0.4)] bg-white text-[#DA7756] shadow-sm hover:bg-[#DA7756] hover:text-white hover:border-[#DA7756] active:scale-[0.96] transition-all duration-150"
            >
              {member}
            </button>
          ))}
      ...
      <div className="space-y-3">
        {detailedReports.map((report) => {
          ...
          return (
            <div
              key={report.id}
              className="bg-white border border-[rgba(218,119,86,0.2)] rounded-2xl shadow-sm overflow-hidden transition-all duration-300"
            >
              <div
                className={cn(
                  "p-4 flex items-start sm:items-center justify-between gap-4 cursor-pointer hover:bg-[#fef6f4] transition-colors",
                  isExpanded ? "border-b border-[rgba(218,119,86,0.12)]" : ""
                )}
                ...
                <div className="flex items-start sm:items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-[10px] font-bold text-[#993C1D] border border-[rgba(218,119,86,0.18)] bg-[#FAECE7] px-2 py-0.5 rounded-xl">
                        {report.dept}
                      </span>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 mt-2">
                      {report.kpiStats.map((stat, i) => (
                        <span
                          key={i}
                          className="bg-red-50 text-red-500 text-[10px] font-bold px-2 py-0.5 rounded-xl border border-red-100"
                        >
                          {stat.label}: {stat.val}
                        </span>
                      ))}
                ...
                <div className="bg-white">
                    ...
                    {[
                      {
                        ...
                        icon: Circle,
                        color: "red",
                        items: report.tasksAndIssues,
                      },
                      {
                        title: "Accomplishments",
                        icon: CheckCircle2,
                        color: "green",
                        items: report.accomplishments,
                      },
                      {
                      ...
                      <div key={title} className="p-4">
                        <h4
                          className={cn(
                            "flex items-center gap-2 font-bold mb-3 text-xs uppercase tracking-wider",
                            color === "red"
                              ? "text-red-500"
                              : color === "green"
                                ? "text-green-600"
                              ...
                              {item.type && (
                                <span
                                  className={cn(
                                    ...
                                    item.type === "task"
                                      ? "text-[#993C1D] border-[rgba(218,119,86,0.18)] bg-[#FAECE7]"
                  ...
                  <div className="p-4 border-t border-[rgba(218,119,86,0.1)] bg-neutral-50">
                    ...
                    {activeInlineAction.reportId !== report.id ? (
                      <div className="flex items-center gap-2 flex-wrap">
                        <BtnOutline
                          onClick={() => handleActionClick(report.id, "task")}
                          icon={Plus}
                        ...
                        <BtnOutline
                          onClick={() => handleActionClick(report.id, "issue")}
                          icon={Plus}
                        ...
                        <BtnOutline
                          onClick={() => handleActionClick(report.id, "plan")}
                          icon={Plus}
                        ...
                        <BtnPurple
                          onClick={() =>
                            handleActionClick(report.id, "feedback")
                          }
                          icon={MessageSquare}
                        >
                          Feedback
                  ...
                  <div className="py-2 border-t border-[rgba(218,119,86,0.1)] bg-white flex justify-center">
                    <button
                      onClick={() => toggleExpand(report.id)}
                      className="flex items-center gap-1.5 text-xs font-semibold text-neutral-500 hover:text-[#DA7756] bg-white border border-neutral-200 px-3 py-1 rounded-full shadow-sm transition-all"
                    >
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-[rgba(218,119,86,0.18)]">
              <div className="flex justify-between items-center p-4 border-b border-neutral-100 bg-[#fef6f4]">
                <h5 className="font-bold text-sm text-neutral-900 flex items-center gap-2">
                  <FileText className="w-4 h-4 text-[#DA7756]" /> Add Task for{" "}
                  {
                    detailedReports.find((r) => r.id === taskModalReportId)
                      ?.user
                  }
                </h5>
                <BtnIcon onClick={() => setIsTaskModalOpen(false)}>
