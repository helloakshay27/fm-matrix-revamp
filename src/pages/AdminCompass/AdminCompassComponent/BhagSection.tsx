...
const ChevronLeft  = () => <svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7"/></svg>;
const ChevronRight = () => <svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7"/></svg>;
const LoaderIcon   = ({ size = 16 }: { size?: number }) => (
  <svg width={size} height={size} style={{ animation: 'spin 0.8s linear infinite', display: 'inline-block' }} fill="none" viewBox="0 0 24 24">
...
const DatePicker: React.FC<{
  ...
  return (
    <div className="bh-dp-wrap" ref={ref}>
      <button type="button" className={`bh-dp-btn${open ? ' open' : ''}`}
        ...
        <div className="bh-dp-cal">
          {view === 'days' && (<>
            <div className="bh-dp-hd">
              <button className="bh-dp-nav" onClick={() => setCur(new Date(cur.getFullYear(), cur.getMonth() - 1, 1))}><ChevronLeft /></button>
              <span className="bh-dp-my" onClick={() => setView('months')}>{MONTHS[cur.getMonth()]} {cur.getFullYear()}</span>
              <button className="bh-dp-nav" onClick={() => setCur(new Date(cur.getFullYear(), cur.getMonth() + 1, 1))}><ChevronRight /></button>
            </div>
            <div className="bh-dp-grid">
              {DAYS_SHORT.map(d => <div key={d} className="bh-dp-dow">{d}</div>)}
              {Array.from({ length: fdow }).map((_, i) => <div key={`e${i}`} className="bh-dp-day e" />)}
            ...
            {value && <div className="bh-dp-clr"><button onClick={() => { onChange(''); setOpen(false); }}>Clear</button></div>}
          </>)}
          {view === 'months' && (<>
            <div className="bh-dp-hd">
              <button className="bh-dp-nav" onClick={() => setCur(new Date(cur.getFullYear() - 1, cur.getMonth(), 1))}><ChevronLeft /></button>
              <span className="bh-dp-my" onClick={() => setView('years')}>{cur.getFullYear()}</span>
              <button className="bh-dp-nav" onClick={() => setCur(new Date(cur.getFullYear() + 1, cur.getMonth(), 1))}><ChevronRight /></button>
            </div>
            <div className="bh-dp-3g">
              {MONTHS.map((m, i) => (
                <div key={m} className={`bh-dp-item${cur.getMonth() === i ? ' a' : ''}`}
          ...
          {view === 'years' && (<>
            <div className="bh-dp-hd">
              <button className="bh-dp-nav" onClick={() => setCur(new Date(cur.getFullYear() - 12, cur.getMonth(), 1))}><ChevronLeft /></button>
              <span className="bh-dp-my">{years[0]} – {years[years.length - 1]}</span>
              <button className="bh-dp-nav" onClick={() => setCur(new Date(cur.getFullYear() + 12, cur.getMonth(), 1))}><ChevronRight /></button>
            </div>
            <div className="bh-dp-3g">
              {years.map(y => (
                <div key={y} className={`bh-dp-item${cur.getFullYear() === y ? ' a' : ''}`}
...
const SkeletonCards = () => (
  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
    {[1, 2, 3, 4].map(n => (
      <div key={n} style={{ background: 'rgba(255,255,255,0.3)', borderRadius: 16, padding: 16 }}>
...
export const BhagSection = () => {
  ...
  const saveBhagStatement = async () => {
    ...
    try {
      ...
      const payload: any = {
        extra_field: { group_name: 'business_plan_bhag', values: [tempStatement.trim()] },
      };
  ...
  return (
    <div className="bh-wrap" style={{ padding: '24px 0', fontFamily: C.font }}>
      ...
      <div style={{ borderRadius: 20, overflow: 'hidden', background: C.tealBg, boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
        ...
        {/* ── 2. Dark header bar ── */}
        <div style={{ background: '#1a1a24', color: '#fff', padding: '18px 22px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.15em', color: 'rgba(255,255,255,0.55)', marginBottom: 7 }}>
              Long Term — BHAG
            </div>
            <h2 style={{ fontSize: 17, fontWeight: 700, lineHeight: 1.45, color: '#fff', margin: 0 }}>
              {isFetching ? 'Loading…' : bhagStatement || 'No BHAG statement yet — click ✏️ to add one.'}
            </h2>
            {!isFetching && bhagTargetDate && (
              <span style={{ display: 'inline-block', marginTop: 10, fontSize: 11, fontWeight: 600, padding: '4px 12px', borderRadius: 20, background: 'rgba(255,255,255,0.14)', color: '#fff' }}>
                Target: {toDisplayDate(bhagTargetDate)}
              </span>
            )}
          </div>
          <button onClick={openBhagModal}
            style={{ flexShrink: 0, background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 10, padding: '8px 9px', cursor: 'pointer', color: '#fff', display: 'flex', alignItems: 'center', transition: 'background .15s' }}
        ...
        <div style={{ padding: '20px 22px 24px' }}>
          ...
          {fetchError && (
            <div style={{ marginBottom: 16, background: '#fee2e2', border: '1px solid #fca5a5', color: '#991b1b', borderRadius: 12, padding: '10px 16px', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              <span>⚠ {fetchError}</span>
              <button onClick={() => { fetchBhagStatement(); fetchGoals(); }}
          ...
          <div style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.15em', color: '#1a1a1a', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div key={ini.id} className="bh-card">
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 14 }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, flex: 1, minWidth: 0 }}>
                      <div style={{ width: 13, height: 13, borderRadius: '50%', border: `3px solid ${C.primary}`, background: '#fff', flexShrink: 0, marginTop: 3 }} />
                      <span style={{ fontSize: 13, fontWeight: 700, color: C.textMain, lineHeight: 1.45 }}>{ini.title}</span>
                    </div>
          ...
          <div style={{ marginTop: 18, display: 'flex', justifyContent: 'flex-end' }}>
            <button onClick={addInitiative}
              style={{ fontSize: 13, fontWeight: 700, padding: '9px 20px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.5)', background: 'rgba(255,255,255,0.5)', color: C.primary, cursor: 'pointer', transition: 'background .15s' }}
              ...
              <div className="bh-modal-body">
                ...
                <FieldGroup>
                  ...
                  <input type="url" value={tempVideoUrl} onChange={e => setTempVideoUrl(e.target.value)}
                    placeholder="https://youtube.com/watch?v=…" className="bh-fld" />
                </FieldGroup>
              ...
              <div className="bh-goal-modal-body">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
                  ...
                  <FieldGroup>
                    ...
                    <select value={tempGoal.status || 'On Track'} onChange={e => setTempGoal({ ...tempGoal, status: e.target.value })} className="bh-select">
