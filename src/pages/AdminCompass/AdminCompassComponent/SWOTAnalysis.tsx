...
const ThemeStyle = () => (
  <style>{`
    ...
    .swot-wrap, .swot-wrap * { font-family: 'Poppins', sans-serif !important; box-sizing:border-box; }
    ...
    .swot-modal-box {
      ...
      box-shadow: 0 26px 76px rgba(0,0,0,0.20);
    ...
    .swot-input::placeholder { color: #a3a3a3; font-weight: 400; }
    ...
    .swot-card-lift:hover {
      box-shadow: 0 10px 22px rgba(17,24,39,0.055) !important;
    ...
    .swot-shell {
      ...
      border:1px solid #e9edf3; box-shadow:0 12px 30px rgba(17,24,39,0.06);
    }
    .swot-section-header {
      border-bottom:1px solid #eef0f4; background:#ffffff; padding:12px 16px;
      display:flex; align-items:center; justify-content:space-between; gap:16px;
    }
    .swot-header-left { flex:1; min-width:0; display:flex; align-items:center; gap:12px; }
    .swot-icon-box {
      width:38px; height:38px; border-radius:13px; background:#f7f7f7;
      color:#ff6b4a; display:flex; align-items:center; justify-content:center; flex-shrink:0;
    }
    .swot-kicker {
      font-size:18px; font-weight:600; text-transform:none; letter-spacing:0;
      color:#111827; display:flex; align-items:center; gap:6px; min-width:0;
    }
    .ac-heading { font-size: 18px; font-weight: 600; color: #111827; margin: 0; font-family: inherit; }
    .swot-body { padding:18px 20px 22px; background:#ffffff; }
    .swot-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:14px; }
    @media (max-width:768px) { .swot-grid { grid-template-columns:1fr; } }
    .swot-quadrant-card {
      position:relative; background:#ffffff; border-radius:15px; padding:14px;
      border:1px solid #e9edf3; box-shadow:0 6px 16px rgba(17,24,39,0.035);
      transition:transform .18s ease, box-shadow .18s ease, border-color .18s ease, background .18s ease;
      overflow:hidden;
    }
    .swot-quadrant-card:hover {
      transform:translateY(-1px); border-color:rgba(218,119,86,0.20);
      background:#ffffff; box-shadow:0 10px 22px rgba(17,24,39,0.055);
    }
    .swot-soft-action {
      font-size:12px; font-weight:500; padding:8px 15px; border-radius:12px;
      border:1px dashed #cbd5e1; background:#ffffff; color:#111827; cursor:pointer;
      box-shadow:0 6px 14px rgba(17,24,39,.035); transition:transform .15s, box-shadow .15s, background .15s;
      display:inline-flex; align-items:center; justify-content:center; gap:8px; font-family:'Poppins',sans-serif;
    }
    .swot-soft-action:hover { transform:translateY(-1px); box-shadow:0 10px 20px rgba(17,24,39,.08); background:#ffffff; }
...
export default function SWOTAnalysis() {
  ...
  // ── Render ──
  return (
    <div className="swot-wrap" style={{ padding: '18px 0', fontFamily: C.font }}>
      <ThemeStyle />

      <div className="swot-shell">
        {/* ── Section header ── */}
        <div className="swot-section-header">
          <div className="swot-header-left">
            <div className="swot-icon-box">
            ...
            <h1 className="swot-kicker ac-heading" style={{ fontFamily: C.font }}>
              SWOT Analysis
            </h1>
          </div>
            ...
            {showInfoTooltip && (
              <div className="swot-tooltip">
                ...
                <div style={{ marginBottom: 12 }}>
                  <span style={{ fontWeight: 500 }}>Strengths & Weaknesses:</span> What you control inside your business
                </div>
                <div style={{ marginBottom: 16 }}>
                  <span style={{ fontWeight: 500 }}>Opportunities & Threats:</span> External market forces you must respond to
                </div>
          ...
          <div className="swot-grid">
            {Object.entries(CONFIG).map(([key, cf]) => {
              ...
              return (
                <div
                  key={key}
                  className="swot-quadrant-card"
                  style={{
                    borderTop: `3px solid ${cf.accent}`,
                  }}
                >
                  {/* Card header */}
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <h3 style={{
                        fontSize: 13, fontWeight: 500, color: cf.titleClr,
                  ...
                  {error && (
                    <div style={{
                      ...
                      <button
                        ...
                        style={{ fontSize: 11, fontWeight: 500, background: 'none', border: 'none', cursor: 'pointer', color: '#991b1b', textDecoration: 'underline', fontFamily: C.font }}
                      >Retry</button>
                    </div>
                  )}
                ...
                {tempItems.map((item, idx) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      style={{ flex: 1, border: 'none', padding: 0, background: 'transparent', boxShadow: 'none', fontSize: 13, fontWeight: 400, cursor: 'text' }}
                    />
                    <button
