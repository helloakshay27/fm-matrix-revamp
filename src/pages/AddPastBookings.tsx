...
export const AddPastBookings = () => {
    ...
    const fetchSlots = async (facilityId: string, date: string, userId?: string) => {
        try {
            ...
            const response = await apiClient.get(`/pms/admin/facility_setups/${facilityId}/get_schedules_past.json`, {
                params
            });
        ...
        if (selectedUser && selectedFacility) {
            const fetchAmenityBooking = async () => {
                try {
                    ...
                    const baseUrl = localStorage.getItem('baseUrl');
                    const bookingRuleResponse = await axios.get(`https://${baseUrl}/pms/admin/facility_setups/${facilityId}/booking_rule_for_user.json?user_id=${selectedUser}`, {
                        headers: {
                            'Authorization': `Bearer ${token}`,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await apiClient.post('/pms/admin/facility_bookings/create_past_booking.json', payload, {
                headers: {
                    'Content-Type': 'application/json',
                },
            });
        ...
        <div className="p-6 mx-auto">
            ...
            <div className="mb-6">
                ...
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <h1 className="text-xl font-semibold" style={{ color: '#C72030' }}>Past Facility Booking</h1>
                </div>
            </div>
                    ...
                    {slots.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            {slots.map((slot) => {
                                ...
                                return (
                                    <div key={slot.id} className={`flex items-center space-x-2 p-3 border rounded-lg ${isBooked ? 'bg-red-50 opacity-60 border-red-300' : 'hover:bg-gray-50'}`}>
                                        <input
                                            type="checkbox"
                                            id={`slot-${slot.id}`}
                                            checked={isSelected}
                                        ...
                                        <Label
                                            ...
                                            className={`cursor-pointer text-sm font-medium flex items-center gap-2 ${isBooked ? 'text-red-600' : ''}`}
                                        >
                                            {slot.ampm}
                                            {isBooked && (
