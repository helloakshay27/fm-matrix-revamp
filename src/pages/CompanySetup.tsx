...
const CompanySetup: React.FC = () => {
          ...
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center overflow-hidden flex-shrink-0 border border-gray-200">
              {(item.profile_image || item.field_description) ? (
                <img
                  src={item.profile_image || item.field_description}
                  alt="Avatar"
  ...
  const renderActionsHistory = (item: EOMHistoryItem) => (
    <div className="flex justify-end gap-2">
      <Button
        variant="ghost"
        size="icon"
        onClick={() => handleDeleteEOMHistory(item.extra_field_id || Number(item.id))}
  ...
  useEffect(() => {
    ...
    const fetchEOMHistory = async () => {
      ...
      try {
          ...
          const processedHistory = rawHistory.map((h: EOMHistoryItem) => {
            if (h.field_value) {
              try {
                ...
                console.error("Failed to parse field_value JSON", e);
              }
            }
            return h;
          });
        ...
        console.error("Failed to fetch EOM history:", error);
      } finally {
  ...
  const handleEOMUpdate = async () => {
    ...
    try {
      ...
      // Sync extra_fields record (for history)
      try {
        const selectedUser = users.find(
          (u) => String(u.id) === employeeOfTheMonth.userId
        );

        // Convert "March 2026" to "2026-03" format
        ...
        const historyPayload = {
          extra_field: {
            ...
            field_value: String(employeeOfTheMonth.userId),
          },
        };
        ...
        // Check if entry for THIS month already exists in history
        let existingId: number | null = null;
        const historyRes = await axios.get(
          `${protocol}${baseUrl}/extra_fields/employee_of_the_month`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        const rawHistoryData = historyRes.data?.employee_of_the_month;
        const historyData = Array.isArray(rawHistoryData) ? rawHistoryData : (rawHistoryData ? [rawHistoryData] : []);
        
        const existing = historyData.find(h => h.field_name === formattedMonth || h.month === employeeOfTheMonth.month);
        if (existing) existingId = existing.extra_field_id || existing.id;
        ...
        // Refresh history
        const updatedHistoryRes = await axios.get(
          `${protocol}${baseUrl}/extra_fields/employee_of_the_month`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        if (updatedHistoryRes.data?.employee_of_the_month) {
          const rawUpdatedData = updatedHistoryRes.data.employee_of_the_month;
          const rawHistory = Array.isArray(rawUpdatedData) ? rawUpdatedData : (rawUpdatedData ? [rawUpdatedData] : []);
        ...
        console.error("Extra fields history sync failed:", efError);
      }
    } catch (error) {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2 text-sm border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-gray-300 w-full sm:w-64 bg-white h-10"
              />
            </div>
          </div>
