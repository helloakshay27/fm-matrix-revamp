...
const PreventiveActionListPage = () => {
    ...
    const fetchTemplates = async () => {
      try {
        ...
        const filteredData = Array.isArray(data) 
          ? data.filter(template => template.identifier === 'Preventive Action')
          : [];
  ...
  const handleAddTemplate = () => {
    navigate('/master/template/preventive-action/add');
  };

  const handleEdit = (id: number) => {
    navigate(`/master/template/preventive-action/edit/${id}`);
  };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Preventive Action Templates</h1>
      </div>
      {showActionPanel && shouldShow('preventive-action', 'add') && (
        <SelectionPanel
          actions={[
      ...
      <EnhancedTable
        ...
        columns={[
          ...
          ...(shouldShow('preventive-action', 'edit') ? [{ key: 'actions', label: 'Actions' }] : []),
        ...
        leftActions={
          shouldShow('preventive-action', 'add') && (
            <Button
          ...
          if (key === 'actions') {
            return (
              <div className="flex gap-2 justify-center items-center">
                {shouldShow('preventive-action', 'edit') && (
                  <Button
                    variant="ghost"
                    size="icon"
        ...
        emptyMessage="No Preventive Action templates found."
      />
    </div>
  );
};

export default PreventiveActionListPage;
