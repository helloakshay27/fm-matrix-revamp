...
const CorrectiveActionListPage = () => {
    ...
    const fetchTemplates = async () => {
      try {
        ...
        const filteredData = Array.isArray(data) 
          ? data.filter(template => template.identifier === 'Corrective Action')
          : [];
  ...
  const handleAddTemplate = () => {
    navigate('/master/template/corrective-action/add');
  };

  const handleEdit = (id: number) => {
    navigate(`/master/template/corrective-action/edit/${id}`);
  };
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <EnhancedTable
        ...
        emptyMessage="No Corrective Action templates found."
      />
    </div>
  );
};
