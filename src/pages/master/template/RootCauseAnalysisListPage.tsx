...
const RootCauseAnalysisListPage = () => {
    ...
    const fetchTemplates = async () => {
      try {
        ...
        const filteredData = Array.isArray(data) 
          ? data.filter(template => template.identifier === 'Root Cause Analysis')
          : [];
  ...
  const handleAddTemplate = () => {
    navigate('/master/template/root-cause-analysis/add');
  };

  const handleEdit = (id: number) => {
    navigate(`/master/template/root-cause-analysis/edit/${id}`);
  };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Root Cause Analysis Templates</h1>
      </div>
      {showActionPanel && (
        <SelectionPanel
          actions={[
      ...
      <EnhancedTable
        ...
        emptyMessage="No Root Cause Analysis templates found."
      />
    </div>
  );
};

export default RootCauseAnalysisListPage;
