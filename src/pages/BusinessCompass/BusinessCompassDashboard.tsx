...
import { ChevronRight, ChevronLeft, Sparkles, X, Send, Clock } from "lucide-react";
...
interface DashboardData {
  success: boolean;
  data: {
    profile: { user_name: string; organization_name: string; department: string; designation: string };
    critical_numbers: { total: number; items: CriticalNumber[] };
    business_health_score: { score: number; out_of: number; label: string; components: BusinessHealthComponents };
    top_stuck_issues: { total: number; items: TopStuckIssue[] };
    latest_team_chat: { total: number; items: any[] };
    hall_of_fame: { items: any[] };
    counters: {
      daily_reports: number; daily_pending: number; kpis: number;
      weekly_reports: number; weekly_pending: number; job_descriptions: number;
    };
  };
}
...
function getDayStatus(date: Date, today: Date, filledDates: Set<string>): DayStatus {
...
const STATUS_DOT: Record<DayStatus, string> = {
  filled: "#22c55e", missed: "#f87171", holiday: "#fbbf24",
  upcoming: "#fb923c", none: "transparent",
};

// ── Circle Progress ────────────────────────────────────────────────────────
function CircleProgress({ value, max = 300, color, label }: {
...
const BusinessCompassDashboard: React.FC = () => {
  ...
  const [todosOverdue, setTodosOverdue]     = useState<{ title: string; id: number }[]>([]);
  ...
  const [weeklyTodosOverdue, setWeeklyTodosOverdue]           = useState<{ title: string; id: number }[]>([]);
  const [weeklyTodosHigh, setWeeklyTodosHigh]                 = useState<{ title: string; id: number }[]>([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Dashboard
        const dashRes = await fetch(`${baseUrl}/business_compass/dashboard`, { headers });
        ...
        if (todayJRes.ok) {
          ...
          const accItems: any[] = Array.isArray(rawAcc)
            ...
            : Array.isArray(rawAcc?.items)
              ...
              : [];
          const items: string[] = accItems
            .map((item: any) => (typeof item === "string" ? item : item?.title ?? item?.text ?? ""))
            .filter(Boolean);
          setAccomplishments(items);
        }
            ...
            if (todoRes.ok) {
              ...
              setTodosOverdue(
                todos.filter((t: any) => {
                  ...
                  return due < now;
                }).slice(0, 5).map((t: any) => ({ title: t.title, id: t.id }))
              );
              // P1 = Urgent & Important (Eisenhower Q1)
              setTodosHigh(
                todos.filter((t: any) => t.priority === "P1")
                  .slice(0, 5).map((t: any) => ({ title: t.title, id: t.id }))
              );
            }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const jRes = await fetch(`${baseUrl}/user_journals.json?${jParams}`, { headers });
        if (jRes.ok) {
          const jData = await jRes.json();
          const journals: any[] = Array.isArray(jData) ? jData : jData.user_journals ?? [];
          const all: string[] = [];
          journals.forEach((j: any) => {
            (j?.report_data?.accomplishments?.items ?? []).forEach((it: any) => {
  ...
  const profileComplete = useMemo(() => {
    try {
      ...
      if (!p) return false;
      const fields = [p.displayName, p.email, p.jobTitle, p.city, p.state,
                      p.pinCode, p.dob, p.doj, p.emergencyContactName, p.emergencyContactNumber];
  ...
  const issueBadgeStyle = (priority: string): React.CSSProperties => {
    ...
    if (p === "medium") return { background: "#fff", border: "1px solid #f59e0b", color: "#b45309" };
    if (p === "low")    return { background: "#fff", border: "1px solid #22c55e", color: "#15803d" };
    return { background: "#fff", border: "1px solid #fca5a5", color: "#e05050" };
  };
  ...
  return (
    <div className="relative bg-white min-h-screen font-poppins">
      <div className="p-3 sm:p-4 lg:p-5">
        <h1 className="text-[18px] sm:text-[22px] font-bold text-[#1a1a1a] mb-4">Dashboard</h1>
          ...
          <Card className="rounded-2xl border border-gray-100 bg-[#F6F4EE] shadow-sm">
            <CardContent className="px-4 sm:px-6 py-4 sm:py-5">
              <div className="flex items-start justify-between mb-8">
                <h2 className="text-[14px] sm:text-[15px] font-bold text-[#1a1a1a]">Business Health Score</h2>
                <div className="flex flex-col items-end gap-1">
                  <span className="text-[12px] sm:text-[13px] font-bold text-[#1a1a1a]">+ {score}/{outOf}</span>
              ...
              <div className="relative pt-8 mb-1 mx-1">
                <div className="absolute flex flex-col items-center"
                  style={{ left: `${scorePct}%`, transform: "translateX(-50%)", top: 0 }}>
                  <span className="text-white text-[10px] sm:text-[11px] font-bold px-2.5 py-0.5 rounded-full whitespace-nowrap shadow-sm"
                    style={{ backgroundColor: "#4ECDC4" }}>you</span>
                  <div style={{ width: 0, height: 0, borderLeft: "4px solid transparent", borderRight: "4px solid transparent", borderTop: "5px solid #4ECDC4" }} />
                </div>
              ...
              <div className="flex justify-between text-[10px] sm:text-[11px] text-gray-400 font-medium mb-4 mt-1.5">
                <span>Poor</span><span>Normal</span><span>Good</span><span>Excellent</span>
              </div>

              {/* 4 metric cards */}
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-4">
                <div className="rounded-xl p-2.5 sm:p-3 text-center" style={{ backgroundColor: "#eeedf8", border: "1px solid #dddcf0" }}>
                  <div className="text-[10px] sm:text-[11px] text-gray-400 font-semibold mb-1">KPI</div>
                  <div className="text-[18px] sm:text-[22px] font-black text-[#1a1a1a]">{health?.components?.kpi?.percentage ?? 0}%</div>
                </div>
                <div className="rounded-xl p-2.5 sm:p-3 text-center" style={{ backgroundColor: "#fce8e8", border: "1px solid #fbcfcf" }}>
                  <div className="text-[10px] sm:text-[11px] text-gray-500 font-semibold mb-1">Issues</div>
                  <div className="text-[18px] sm:text-[22px] font-black text-[#1a1a1a]">{health?.components?.issues?.count ?? 0}</div>
                </div>
                <div className="rounded-xl p-2.5 sm:p-3 text-center" style={{ backgroundColor: "#e8f0fe", border: "1px solid #c7d9fd" }}>
                  <div className="text-[10px] sm:text-[11px] text-gray-500 font-semibold mb-1">Systems</div>
                  <div className="text-[18px] sm:text-[22px] font-black text-[#1a1a1a]">{health?.components?.systems?.healthy ?? 0}/{health?.components?.systems?.total ?? 0}</div>
                </div>
                <div className="rounded-xl p-2.5 sm:p-3 text-center" style={{ backgroundColor: "#e8f4fe", border: "1px solid #c7e5fd" }}>
                  <div className="text-[10px] sm:text-[11px] text-gray-500 font-semibold mb-1">Goals</div>
                  <div className="text-[18px] sm:text-[22px] font-black text-[#1a1a1a]">{health?.components?.goals?.achieved ?? 0}/{health?.components?.goals?.total ?? 0}</div>
                </div>
              </div>

              {/* AI suggestion bar */}
              <div style={{ borderRadius: "16px", background: "white", border: "1px solid rgba(218,119,86,0.10)", boxShadow: "0 0 0 5px rgba(218,119,86,0.07), 0 4px 18px rgba(218,119,86,0.10)" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", padding: "12px 16px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    ...
                    <span style={{ fontSize: "13px", color: "#1a1a1a", fontWeight: 500 }}>Ai suggestion to improve business score.</span>
                  </div>
                  <button
                    onClick={handleGenerateAiSummary}
                    style={{
                      backgroundColor: "#DA7756", color: "#fff", border: "none",
                      borderRadius: "10px", padding: "8px 22px", fontSize: "13px",
                      fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap",
                      flexShrink: 0, transition: "background-color 0.2s",
                    }}
                    onMouseEnter={(e) => { (e.target as HTMLButtonElement).style.backgroundColor = "#c9673f"; }}
                    onMouseLeave={(e) => { (e.target as HTMLButtonElement).style.backgroundColor = "#DA7756"; }}>
                    Generate
                  </button>
                </div>
                {aiSuggestionsOpen && (
                  <div className="ai-suggestions-expand">
                    ...
                    <div className="flex items-start justify-between px-3 pt-3 pb-1">
                      <div>
                        <h4 className="text-[13px] font-bold text-[#1a1a1a]">AI Suggestions</h4>
                        <p className="text-[11px] text-gray-400 mt-0.5">
                          Implementing all suggestions could boost your score by{" "}
                          <span className="font-bold text-[#DA7756]">+46 pts</span>{" "}→{" "}
                          <span className="font-bold text-[#DA7756]">121/100</span>
                        </p>
                      </div>
                      <button onClick={() => setAiSuggestionsOpen(false)}
                    ...
                    <div className="grid grid-cols-2 gap-2 px-3 pb-2">
                      <div className="rounded-xl p-2.5 flex flex-col gap-1" style={{ backgroundColor: "#fce8e8", border: "1px solid #fbcfcf" }}>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1">
                            <div className="w-1.5 h-1.5 rounded-full bg-[#f87171]" />
                            <span className="text-[10px] font-bold text-gray-700">Issues</span>
                            <span className="text-[9px] font-semibold px-1 py-0.5 rounded-full" style={{ backgroundColor: "#ffe4e4", color: "#e05050" }}>High</span>
                          </div>
                          <span className="text-[9px] font-bold text-gray-600 bg-white px-1 py-0.5 rounded-full">+12 pts</span>
                        </div>
                        <p className="text-[11px] font-bold text-[#1a1a1a] leading-snug">Resolve your top 20 critical issues</p>
                        <p className="text-[10px] text-gray-500 leading-relaxed">Fix top 20 by severity to reduce score drag and improve KPI by ~8%.</p>
                        <div className="flex items-center justify-between mt-0.5">
                          <div className="flex items-center gap-1 text-[9px] text-gray-400"><Clock size={9} /><span>1–2 weeks</span></div>
                          <button className="text-[10px] font-bold text-[#DA7756] hover:underline">Start &gt;</button>
                        </div>
                      </div>

                      <div className="rounded-xl p-2.5 flex flex-col gap-1" style={{ backgroundColor: "#e8f0fe", border: "1px solid #c7d9fd" }}>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1">
                            <div className="w-1.5 h-1.5 rounded-full bg-[#60a5fa]" />
                            <span className="text-[10px] font-bold text-gray-700">Systems</span>
                            <span className="text-[9px] font-semibold px-1 py-0.5 rounded-full" style={{ backgroundColor: "#ffe4e4", color: "#e05050" }}>High</span>
                          </div>
                          <span className="text-[9px] font-bold text-gray-600 bg-white px-1 py-0.5 rounded-full">+18 pts</span>
                        </div>
                        <p className="text-[11px] font-bold text-[#1a1a1a] leading-snug">Connect all 6 business systems</p>
                        <p className="text-[10px] text-gray-500 leading-relaxed">0 of 6 systems integrated. Enables automated data flow and removes reporting gaps.</p>
                        <div className="flex items-center justify-between mt-0.5">
                          <div className="flex items-center gap-1 text-[9px] text-gray-400"><Clock size={9} /><span>2–3 weeks</span></div>
                          <button className="text-[10px] font-bold text-[#DA7756] hover:underline">Start &gt;</button>
                        </div>
                      </div>

                      <div className="rounded-xl p-2.5 flex flex-col gap-1" style={{ backgroundColor: "#edf7ed", border: "1px solid #c3e6c4" }}>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1">
                            <div className="w-1.5 h-1.5 rounded-full bg-[#4ade80]" />
                            <span className="text-[10px] font-bold text-gray-700">Goals</span>
                            <span className="text-[9px] font-semibold px-1 py-0.5 rounded-full bg-amber-100 text-amber-700">Medium</span>
                          </div>
                          <span className="text-[9px] font-bold text-gray-600 bg-white px-1 py-0.5 rounded-full">+9 pts</span>
                        </div>
                        <p className="text-[11px] font-bold text-[#1a1a1a] leading-snug">Define and track 4 business goals</p>
                        <p className="text-[10px] text-gray-500 leading-relaxed">No goals set. Measurable quarterly goals improve KPI tracking by up to 15%.</p>
                        <div className="flex items-center justify-between mt-0.5">
                          <div className="flex items-center gap-1 text-[9px] text-gray-400"><Clock size={9} /><span>4–6 weeks</span></div>
                          <button className="text-[10px] font-bold text-[#DA7756] hover:underline">Start &gt;</button>
                        </div>
                      </div>

                      <div className="rounded-xl p-2.5 flex flex-col gap-1" style={{ backgroundColor: "#fff3e8", border: "1px solid #fddcbc" }}>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1">
                            <div className="w-1.5 h-1.5 rounded-full bg-[#fb923c]" />
                            <span className="text-[10px] font-bold text-gray-700">KPI</span>
                            <span className="text-[9px] font-semibold px-1 py-0.5 rounded-full bg-amber-100 text-amber-700">Medium</span>
                          </div>
                          <span className="text-[9px] font-bold text-gray-600 bg-white px-1 py-0.5 rounded-full">+7 pts</span>
                        </div>
                        <p className="text-[11px] font-bold text-[#1a1a1a] leading-snug">Recalibrate KPI targets to benchmarks</p>
                        <p className="text-[10px] text-gray-500 leading-relaxed">KPI at 59%, below 72% industry average. Benchmark data can close this gap faster.</p>
                        <div className="flex items-center justify-between mt-0.5">
                          <div className="flex items-center gap-1 text-[9px] text-gray-400"><Clock size={9} /><span>1 week</span></div>
                          <button className="text-[10px] font-bold text-[#DA7756] hover:underline">Start &gt;</button>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="px-3 pb-3">
                      <button
                        ...
                        style={{ background: "linear-gradient(135deg, #DA7756 0%, #c9673f 100%)", boxShadow: "0 3px 12px rgba(218,119,86,0.25)" }}>
          ...
          <Card className="rounded-2xl border border-gray-100  bg-[#F6F4EE] shadow-sm">
            <CardContent className="p-4 sm:p-5">
              ...
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-[15px] sm:text-[16px] font-bold text-[#1a1a1a]">Today's Focus</h3>
                <div className="relative">
                  <select value={focusMode}
                    onChange={(e) => { setFocusMode(e.target.value as "Daily" | "Weekly"); setKpiPage(0); }}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-3">
                ...
                <div className="lg:col-span-1 bg-[#ffffff] rounded-2xl p-3 sm:p-4">
                  ...
                  {(() => {
                    ...
                    const pending   = focusMode === "Weekly" ? (counters?.weekly_pending ?? 0)  : (counters?.daily_pending ?? 0);
                    const completed = Math.max(0, assigned - pending);
                    const maxVal = Math.max(assigned, 1);
                    const rings = [
                      { value: assigned,  color: "#a78bfa", track: "#ede9fe", label: "Assigned" },
                      { value: completed, color: "#2dd4bf", track: "#ccfbf1", label: "Completed" },
                      { value: pending,   color: "#f87171", track: "#fee2e2", label: "Overdue" },
                    ];
                          ...
                          return (
                            <div key={label} className="flex flex-col items-center gap-2">
                              <div className="relative w-[68px] h-[68px]">
                                ...
                                <span className="absolute inset-0 flex items-center justify-center text-[15px] font-black" style={{ color }}>{value}</span>
                              </div>
                              <span className="text-[10px] sm:text-[11px] text-gray-500 font-semibold">{label}</span>
                            </div>
                          );
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                ...
                <div className="lg:col-span-2 bg-[#ffffff] rounded-2xl p-3 sm:p-4">
                  <div className="flex items-center justify-between gap-2 mb-3">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="text-[12px] sm:text-[13px] font-bold text-[#1a1a1a] truncate">Accomplishments Ai overview</span>
                      <span className="text-[9px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap flex-shrink-0"
                        style={{ backgroundColor: "#ede9fe", color: "#7c3aed" }}>8/20 Pts</span>
                    </div>
                    <button onClick={() => navigate("/business-compass/daily-report")}
                  ...
                  {displayLoading ? (
                    ...
                    <p className="text-[11px] text-gray-400 italic">
                      {focusMode === "Weekly" ? "No accomplishments this week." : "No accomplishments for today."}
                    </p>
                  )}
                </div>
                ...
                <div className="lg:col-span-1 bg-[#ffffff] rounded-2xl p-3 sm:p-4">
                  <div className="flex items-center justify-between gap-2 mb-3">
                    ...
                    <button onClick={() => navigate("/business-compass/todo")}
                      className="text-[11px] text-[#DA7756] font-bold hover:underline whitespace-nowrap flex-shrink-0">
                      View All &gt;
                    </button>
                  </div>
                    ...
                    <>
                      <div className="mb-3">
                        ...
                        {displayTodosOverdue.length > 0 ? (
                          ...
                          <p className="text-[11px] text-gray-400 italic">No overdue todos.</p>
                        )}
                      </div>
                      <div>
                        <button
                        ...
                        {displayTodosHigh.length > 0 ? (
                          ...
                          <p className="text-[11px] text-gray-400 italic">No urgent &amp; important todos.</p>
                        )}
                      </div>
                    </>
                  )}
            ...
            <Card className="rounded-2xl border border-gray-100 bg-[#F6F4EE] shadow-sm">
              <CardContent className="px-4 py-4">
                ...
                {(() => {
                  ...
                  const paged = pool.slice(safePage * perPage, safePage * perPage + perPage);
                  ...
                  return (
                    <div className="flex items-stretch gap-2">
                          ...
                          paged.map((kpi, i) => (
                            <div key={kpi.id ?? i} className="flex flex-col gap-1 p-2.5 rounded-xl bg-white border border-gray-200">
                              ...
                              <div className="flex items-baseline gap-0.5 mt-1">
                                <span className="text-[13px] font-black text-[#1a1a1a]">{kpi.current_value}</span>
                                <span className="text-[9px] text-gray-400">/{kpi.target_value} {kpi.unit}</span>
                              </div>
                              <div className="w-full bg-gray-100 rounded-full h-1 mt-0.5">
                                <div className="bg-[#DA7756] h-1 rounded-full" style={{ width: `${Math.min(kpi.progress_percentage, 100)}%` }} />
                              </div>
                      ...
                      <button
                        ...
                        className={cn("flex items-center justify-center w-6 flex-shrink-0 transition-colors",
                          hasMore ? "text-gray-400 hover:text-[#DA7756] cursor-pointer" : "text-gray-200 cursor-default")}
                        disabled={!hasMore}
                        title={hasMore ? `Page ${safePage + 1}/${totalPages}` : ""}>
            ...
            <Card className="rounded-2xl border border-gray-100 bg-[#F6F4EE] shadow-sm">
              <CardContent className="px-4 py-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={() => navigate("/vas/issues")}
          ...
          <Card className="rounded-2xl border border-gray-100 bg-[#F6F4EE] shadow-sm">
            <CardContent className="px-4 py-4">
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-2 gap-2">
                <div className="text-center p-3 rounded-xl" style={{ backgroundColor: "#fce8e8", border: "1px solid #fbcfcf" }}>
                  <p className="text-[9px] text-[#e05050] font-semibold leading-tight mb-1">Daily report/<br />pending</p>
                  <p className="text-[15px] font-black text-[#1a1a1a]">{counters?.daily_reports ?? 0}/{counters?.daily_pending ?? 0}</p>
                </div>
                <div className="text-center p-3 rounded-xl" style={{ backgroundColor: "#fce8e8", border: "1px solid #fbcfcf" }}>
      ...
      {aiChatOpen && (
        <div className="fixed z-50 ai-chat-enter"
          style={{ bottom: "68px", right: "16px", width: "min(420px, calc(100vw - 32px))" }}>
          <div style={{
            padding: "1.5px", borderRadius: "20px",
            background: "linear-gradient(135deg, rgba(218,119,86,0.70) 0%, rgba(251,200,170,0.40) 40%, rgba(190,185,235,0.60) 100%)",
            boxShadow: "0 8px 32px rgba(0,0,0,0.10)",
          }}>
            <div style={{ borderRadius: "18.5px", background: "#ffffff", overflow: "hidden" }}>
              <div className="flex items-center justify-between px-5 pt-4 pb-3"
                ...
                <div className="flex gap-1.5 sm:gap-2 overflow-x-auto no-scrollbar">
                  {(["accomplishments", "plan"] as const).map((tab) => (
                    <button key={tab} onClick={() => setAiChatTab(tab)}
                      className={cn("px-3 sm:px-4 py-1.5 rounded-full text-[11px] sm:text-[12px] font-semibold transition-all whitespace-nowrap",
                        aiChatTab === tab ? "bg-[#DA7756] text-white shadow-sm" : "text-gray-600 bg-white/40 hover:bg-white/60")}>
                      {tab === "accomplishments" ? "Fill my accomplishments" : "Plan for next day"}
                    </button>
                  ))}
                </div>
              ...
              <div className="h-36 sm:h-40 px-4 sm:px-5 py-4 flex items-center justify-center">
                <p className="text-[12px] sm:text-[13px] text-gray-400 italic text-center">
                  {aiChatTab === "accomplishments" ? "AI will help you fill your accomplishments..." : "AI will help you plan for tomorrow..."}
                </p>
              </div>
              <div className="px-4 sm:px-5 pb-5 pt-1">
                <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 rounded-2xl"
                  style={{ background: "rgba(255,255,255,0.65)", border: "1px solid rgba(255,255,255,0.80)", boxShadow: "0 2px 10px rgba(0,0,0,0.06)" }}>
                  ...
                  {aiMessage && (
                    <button onClick={() => setAiMessage("")} className="text-[#DA7756] hover:text-[#c9673f] flex-shrink-0 transition-colors">
                      <Send size={13} />
                    </button>
                  )}
                </div>
      ...
      {!aiChatOpen && (
        <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50">
          <div className="absolute pointer-events-none"
            style={{ inset: "-18px", borderRadius: "50%",
              background: "radial-gradient(circle at 42% 38%, rgba(218,119,86,0.38) 0%, rgba(249,170,130,0.16) 50%, transparent 72%)",
              filter: "blur(10px)" }} />
          <div className="ai-btn-glow-outer">
            <div className="ai-btn-glow-spinner" />
            <button onClick={() => setAiChatOpen(true)} title="AI Assistant"
              onMouseEnter={(e) => { e.currentTarget.style.transform = "scale(1.06)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.transform = "scale(1)"; }}
              style={{
                position: "relative", zIndex: 1, width: "52px", height: "52px",
                borderRadius: "17.5px",
                background: "linear-gradient(148deg, rgba(255,255,255,0.97) 10%, rgba(253,218,196,0.84) 100%)",
                backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)",
                display: "flex", alignItems: "center", justifyContent: "center",
                cursor: "pointer", border: "none", outline: "none",
                boxShadow: "inset 0 1px 0 rgba(255,255,255,1), 0 4px 14px rgba(218,119,86,0.15)",
