...
export const AMCDashboard = () => {
    ...
    const renderCell = (item: AMCRecord, columnKey: string) => {
        switch (columnKey) {
            case 'actions':
                return (
                    <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => navigate(`/maintenance/amc/${item.id}`)}>
                            <Eye className="w-4 h-4" />
                        </Button>
                        <Button size="sm" variant="outline" onClick={() => console.log('Edit', item.id)}>
                            <Settings className="w-4 h-4" />
                        </Button>
                        <Button size="sm" variant="outline" onClick={() => console.log('Delete', item.id)}>
            ...
            case 'active':
                return (
                    <span className={`px-2 py-1 rounded text-sm ${item.active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                        {item.active ? 'Active' : 'Inactive'}
                    </span>
                );
            default:
                return String(item[columnKey as keyof AMCRecord] || '');
        }
    };

    const handleSelectAll = (checked: boolean) => {
        if (checked) {
            setSelectedItems(amcData.map((item: AMCRecord) => item.id.toString()));
        } else {
            setSelectedItems([]);
        }
    };
    ...
    const bulkActions = [
        { 
            ...
            onClick: (selectedItems: any) => console.log('Bulk delete', selectedItems),
            icon: Trash2,
            variant: 'destructive' as const
        },
        { 
            label: 'Export Selected', 
            onClick: (selectedItems: any) => console.log('Bulk export', selectedItems),
            icon: Download,
