...
export const EditExternalUserPage = () => {
  ...
  const [lmMenuId] = useState(
    () => `lm-menu-${Math.random().toString(36).slice(2)}`
  );
  ...
  const handleChange = (field: string, value: any) => {
      ...
      setFieldErrors((prev) => ({
        ...
        [field]:
          cleaned.length === 0
            ...
            : field === "firstname" && cleaned.length < 2
              ? "First Name must be at least 2 characters"
              : field === "lastname" && cleaned.length < 1
    ...
    if (field === "joining_date") {
      ...
      setFieldErrors((prev) => ({
        ...prev,
        [field]:
          val && val > todayISO ? "Joining Date cannot be in the future" : "",
      }));
      return;
    }
  ...
  const getErrors = () => {
    ...
    if (
      !isEmpty(formData.joining_date) &&
      !dateRe.test(String(formData.joining_date))
    ) {
    ...
    if (
      !errors.joining_date &&
      !isEmpty(formData.joining_date) &&
      String(formData.joining_date) > todayISO
    ) {
  ...
  useEffect(() => {
    if (saveBlocked) {
      ...
      console.log(
        "[EditExternalUserPage] Save disabled. Blocking fields:",
        keys,
        validationErrors
      );
    }
  ...
  // Sync validation errors to field errors so users can see what's blocking the save
  useEffect(() => {
    if (!loading && originalUser) {
      setFieldErrors(validationErrors);
    }
  }, [loading, originalUser, validationErrors]);
    ...
    try {
      ...
      const payload = {
        user: {
          ...
          report_to_id:
            formData.report_to_id || selectedLineManager?.id || null,
      ...
      await axios.put(
        `${cleanBaseUrl}/pms/users/${idForUpdate}/update_vi_user`,
        payload,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast.success("External user updated");
      navigate(`/safety/m-safe/external/user/${idForUpdate}`, {
        state: { user: { ...originalUser, ...formData } },
      });
    } catch (e: any) {
      ...
      if (respData) {
        if (Array.isArray(respData.errors)) {
          ...
          Object.entries(respData.errors).forEach(([k, v]) => {
            if (Array.isArray(v))
              v.forEach((msg) => errors.push(`${k} ${msg}`));
          });
        }
      }
      const emailErr = errors.find(
        (er) => /email/i.test(er) && /(taken|exists|already)/i.test(er)
      );
  ...
  const handleCancel = () =>
    navigate(`/safety/m-safe/external/user/${userId}`, {
      state: { user: location.state?.user || initialUser },
    });
    ...
    const fetchUser = async () => {
      ...
      try {
        ...
        setFormData((prev: any) => ({
          ...
          company_id:
            data.account_id ||
            data.lock_user_permission?.account_id ||
            prev.company_id ||
            "",
          ...
          circle_id:
            data.lock_user_permission?.circle_id || data.circle_id || "",
          department_name:
            data.lock_user_permission?.department_name ||
            data.department_name ||
            data.department?.department_name ||
            "",
          circle_name:
            data.circle_name ||
            data.lock_user_permission?.circle_name ||
            data.circle?.circle_name ||
            "",
          cluster_name: data.cluster_name || "",
          company_cluster_id:
            data.company_cluster_id ||
            data.lock_user_permission?.company_cluster_id ||
            "",
          work_location:
            typeof data.work_location === "object" &&
            data.work_location !== null
              ? data.work_location.name ||
                data.work_location.work_location_name ||
                ""
              : data.work_location || "",
          role_id:
            data.lock_user_permission?.lock_role_id || data.lock_role_id || "",
          role_name:
            data.lock_user_permission?.lock_role_name ||
            data.role_name ||
            data.role?.name ||
            "",
          ext_company_name: data.ext_company_name || "",
          org_user_id:
            data.org_user_id || data.lock_user_permission?.employee_id || "",
          joining_date: normalizeDate(
            data.lock_user_permission?.joining_date || data.joining_date
          ),
        }));
        ...
        const existingManager =
          ...
          (data.report_to_id
            ? {
                id: data.report_to_id,
                email: data.report_to_email,
                name: data.report_to_name,
              }
            : null);
        if (existingManager && existingManager.id) {
          setSelectedLineManager(existingManager);
          setLmOptions((prev) =>
            prev.some((o) => o.id === existingManager.id)
              ? prev
              : [existingManager, ...prev]
          );
        }
      } catch (e: any) {
  ...
  useEffect(() => {
    ...
    if (!exists) {
      ...
      setDepartments((prev) => [
        { id, department_name: label, __synthetic: true },
        ...prev,
      ]);
    }
  ...
  useEffect(() => {
    const fetchCircles = async () => {
      try {
        ...
        console.error("Fetch circles error", e);
      }
    };
    fetchCircles();
  }, []);
  ...
  useEffect(() => {
    ...
    if (!exists) {
      ...
      setCircles((prev) => [
        { id, circle_name: label, __synthetic: true },
        ...prev,
      ]);
    }
  ...
  useEffect(() => {
    const fetchClusters = async () => {
      try {
        ...
        if (!baseUrl || !token) return;
        const companyId =
          getSelectedCompanyId() ?? (formData.company_id || 145);
        const cleanBaseUrl = baseUrl.startsWith("http")
          ? baseUrl
          : `https://${baseUrl}`;
  ...
  useEffect(() => {
    ...
    if (!exists) {
      ...
      setClusters((prev) => [
        { id, cluster_name: label, __synthetic: true },
        ...prev,
      ]);
    }
  ...
  useEffect(() => {
    const fetchWorkLocations = async () => {
      try {
        ...
        console.error("Fetch work locations error", e);
      }
    };
    fetchWorkLocations();
  }, []);
  ...
  useEffect(() => {
    ...
    const exists = workLocations.some(
      (w: any) => (w.name || w.work_location_name) === name
    );
    if (!exists) {
    ...
    if (lmQuery.length < 3) {
      ...
      const selectedManager =
        ...
        (formData.report_to_id &&
          lmOptions.find(
            (opt) => opt.id?.toString() === formData.report_to_id?.toString()
          ));
    ...
    const handler = setTimeout(async () => {
      try {
        ...
        const companyId =
          ...
          originalUser?.company_id ??
          originalUser?.lock_user_permission?.company_id ??
          formData.company_id ??
          145;
        const cleanBaseUrl = baseUrl.startsWith("http")
          ? baseUrl
        ...
        if (
          selectedManager &&
          !users.some(
            (u: any) => u.id?.toString() === selectedManager.id?.toString()
          )
        ) {
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            <TextField
              label={
                <>
                  First Name<span style={{ color: "#C72030" }}>*</span>
                </>
              }
            ...
            <TextField
              label={
                <>
                  Last Name<span style={{ color: "#C72030" }}>*</span>
                </>
              }
            ...
            <TextField
              label={
                <>
                  Mobile Number<span style={{ color: "#C72030" }}>*</span>
                </>
              }
            ...
            <TextField
              label={
                <>
                  Company Name<span style={{ color: "#C72030" }}>*</span>
                </>
              }
              value={formData.ext_company_name}
              onChange={(e) => handleChange("ext_company_name", e.target.value)}
              size="small"
              fullWidth
              error={!!fieldErrors.ext_company_name}
              helperText={fieldErrors.ext_company_name || ""}
            />
            <TextField
              label={
                <>
                  Employer Code<span style={{ color: "#C72030" }}>*</span>
                </>
              }
              value={formData.org_user_id}
              onChange={(e) => handleChange("org_user_id", e.target.value)}
              size="small"
              fullWidth
              error={!!fieldErrors.org_user_id}
              helperText={fieldErrors.org_user_id || ""}
            />
            <FormControl fullWidth size="small" error={!!fieldErrors.gender}>
              <InputLabel>
                Gender<span style={{ color: "#C72030" }}>*</span>
              </InputLabel>
              <Select
            ...
            >
              <InputLabel>
                Reporting Manager<span style={{ color: "#C72030" }}>*</span>
              </InputLabel>
              <Select
                ...
                onOpen={() => {
                  ...
                  if (!selectedLineManager && formData.report_to_id) {
                    ...
                    const currentSelected = lmOptions.find(
                      (opt) =>
                        opt.id?.toString() === formData.report_to_id?.toString()
                    );
                    if (currentSelected) {
                ...
                onChange={(e) => {
                  ...
                  if (selectedId) {
                    const selected = lmOptions.find(
                      (opt) => opt.id?.toString() === selectedId?.toString()
                    );
                    if (selected) {
              ...
              {fieldErrors.report_to_id && (
                <FormHelperText>{fieldErrors.report_to_id}</FormHelperText>
              )}
            </FormControl>
            <FormControl
              fullWidth
            ...
            >
              <InputLabel>
                Department<span style={{ color: "#C72030" }}>*</span>
              </InputLabel>
              <Select
              ...
              {fieldErrors.department_id && (
                <FormHelperText>{fieldErrors.department_id}</FormHelperText>
              )}
            </FormControl>
          </div>
        </CardContent>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            <FormControl fullWidth size="small" error={!!fieldErrors.circle_id}>
              <InputLabel>
                Circle<span style={{ color: "#C72030" }}>*</span>
              </InputLabel>
              <Select
                value={formData.circle_id}
                label="Circle"
                onChange={(e) => handleChange("circle_id", e.target.value)}
              >
                <MenuItem value="">Select</MenuItem>
                {circles.map((c: any) => (
                  <MenuItem key={c.id} value={c.id}>
                    {c.circle_name || c.name || `Circle ${c.id}`}
                  </MenuItem>
                ))}
              </Select>
              {fieldErrors.circle_id && (
                <FormHelperText>{fieldErrors.circle_id}</FormHelperText>
              )}
            </FormControl>
            <FormControl
              fullWidth
              size="small"
              error={!!fieldErrors.company_cluster_id}
            >
              <InputLabel>
                Cluster<span style={{ color: "#C72030" }}>*</span>
              </InputLabel>
              <Select
                ...
                onChange={(e) => {
                  ...
                  const selected = clusters.find(
                    (cl: any) => cl.id?.toString() === val?.toString()
                  );
              ...
              >
                ...
                {clusters.map((cl: any) => (
                  <MenuItem key={cl.id} value={cl.id}>
                    {cl.cluster_name || `Cluster ${cl.id}`}
                  </MenuItem>
                ))}
              </Select>
              {fieldErrors.company_cluster_id && (
                <FormHelperText>
                  {fieldErrors.company_cluster_id}
                </FormHelperText>
              )}
            ...
            >
              <InputLabel>
                Work Location<span style={{ color: "#C72030" }}>*</span>
              </InputLabel>
              <Select
              ...
              >
                ...
                {workLocations.map((w: any) => (
                  <MenuItem key={w.id} value={w.name}>
                    {w.name || w.work_location_name || `Location ${w.id}`}
                  </MenuItem>
                ))}
              </Select>
              {fieldErrors.work_location && (
                <FormHelperText>{fieldErrors.work_location}</FormHelperText>
              )}
            </FormControl>
            <FormControl fullWidth size="small" error={!!fieldErrors.role_id}>
              <InputLabel>
                Role<span style={{ color: "#C72030" }}>*</span>
              </InputLabel>
              <Select
              ...
              >
                ...
                {roles.map((r: any) => (
                  <MenuItem key={r.id} value={r.id}>
                    {r.name || r.display_name || `Role ${r.id}`}
                  </MenuItem>
                ))}
              </Select>
              {fieldErrors.role_id && (
                <FormHelperText>{fieldErrors.role_id}</FormHelperText>
              )}
            </FormControl>
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DatePicker
                label={
                  <>
                    Joining Date<span style={{ color: "#C72030" }}>*</span>
                  </>
                }
                value={
                ...
                onChange={(val) => {
                  ...
                  if (d.isAfter(today, "day")) {
                    setFieldErrors((prev) => ({
                      ...prev,
                      joining_date: `Joining Date cannot be after ${today.format("DD/MM/YYYY")}`,
                    }));
                    return;
                  }
