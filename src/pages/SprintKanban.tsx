...
const SprintKanban: React.FC<SprintKanbanProps> = ({ selectedProject: initialSelectedProject }) => {
    ...
    const { data: sprint } = useAppSelector(
    ...
    // Auto-select "All" and fetch initial tasks
    useEffect(() => {
    ...
    }, [fetchProjectsData, hasFetchedInitial, dispatch, token, baseUrl]);
    ...
    const handleProjectSelect = (project: any) => {
        if ((project === 'All' && selectedProject === 'All') ||
            (project?.id && selectedProject?.id === project.id)) {
            setIsDropdownOpen(false);
            return;
        }
                                ...
                                <>
                                                        ...
                                                        <div className="ml-5 mt-1">
                                                            {visibleSubtasks.map((subtask: any) => (
                                                                <div
                                                                    ...
                                                                    draggable
                                                                    onDragStart={(e) => {
                                                                        console.log('Dragging subtask:', subtask.id);
                                                                        e.dataTransfer.setData(
                                                                            'application/reactflow',
                                                                            JSON.stringify({
                                                                                type: 'SUBTASK',
                                    ...
                                    {filteredSubtasks.map((subtask: any) => (
                                        <div
                                            ...
                                            draggable
                                            onDragStart={(e) => {
                                                console.log('Dragging independent subtask:', subtask.id);
                                                e.dataTransfer.setData(
                                                    'application/reactflow',
                                                    JSON.stringify({
                                                        type: 'SUBTASK',
