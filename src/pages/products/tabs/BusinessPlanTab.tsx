...
const BusinessPlanTab: React.FC<BusinessPlanTabProps> = ({ productData }) => {
  ...
  const hasColoredQuestions = bpData?.planQuestions?.some((q) => q.colorContext);
      ...
      <div className="space-y-4 animate-fade-in">
        <div className="bg-transparent p-2">
          <div className="w-full bg-transparent">
            <div className="bg-white text-gray-800 border border-[#D3D1C7] px-4 py-3 text-[14px] font-bold uppercase tracking-wide font-poppins">
              ...
              {bpData!.planQuestions.map((q, i) => {
                return (
                  <div key={i} className="border border-[#D3D1C7] bg-white">
                    <div className="bg-white text-gray-800 border-b border-[#D3D1C7] px-4 py-3 text-sm font-semibold uppercase tracking-wide font-poppins">
                      {q.id}. {q.question}
                    </div>
            ...
            {bpData!.founderChecklist && bpData!.founderChecklist.length > 0 && (
              <div className="mt-6 space-y-3">
                <div className="bg-white text-gray-800 border border-[#D3D1C7] px-4 py-3 text-sm font-semibold uppercase tracking-wide font-poppins">
                  Founder Review Checklist - Items requiring personal input before investor/partner use
                </div>
                <div className="border border-[#D3D1C7] bg-white">
                  <table className="w-full border-collapse table-fixed text-[10px] leading-[1.5] font-poppins">
                    <thead>
                      <tr className="bg-gray-100 text-gray-800 font-semibold uppercase text-[9px]">
                        <th className="border border-[#D3D1C7] px-3 py-2 text-left w-[14%]">Q#</th>
                        <th className="border border-[#D3D1C7] px-3 py-2 text-left w-[66%]">What to Verify / Add</th>
                        <th className="border border-[#D3D1C7] px-3 py-2 text-left w-[20%]">Status</th>
                      </tr>
                    </thead>
                    <tbody>
                    ...
                    <table className="w-full border-collapse text-[9px] leading-[1.15] table-fixed font-poppins">
                      <thead>
                        <tr className="bg-white text-gray-800 border-b border-[#D3D1C7] font-semibold uppercase">
                          <th className="border border-[#E5E7EB] px-1 py-1 text-center w-[5%]">#</th>
                          <th className="border border-[#E5E7EB] px-1.5 py-1 text-left w-[25%]">Business plan question</th>
                          <th className="border border-[#E5E7EB] px-1.5 py-1 text-left w-[50%]">Suggested answer</th>
                          <th className="border border-[#E5E7EB] px-1.5 py-1 text-left w-[20%]">Status</th>
                        </tr>
                      </thead>
                      <tbody>
                        {bpData.planQuestions.map((q, i) => (
                          <tr key={i} className={`align-top ${i % 2 === 0 ? "bg-white" : "bg-gray-50"}`}>
                            <td className="border border-[#E5E7EB] px-1 py-1 text-center font-bold text-[#2C2C2C]/60">{i + 1}</td>
                            <td className="border border-[#E5E7EB] px-1.5 py-1 font-semibold text-gray-700 whitespace-pre-line break-words">{q.question}</td>
                            <td className="border border-[#E5E7EB] px-1.5 py-1 text-[#2C2C2C] font-medium whitespace-pre-line break-words">{q.answer}</td>
                            <td className="border border-[#E5E7EB] px-1.5 py-1 text-center">
                              <span className={`inline-block px-1.5 py-0.5 text-[8px] font-semibold uppercase border ${q.flag?.toLowerCase().includes("ready") ? "bg-green-50 text-green-800 border-green-200" : "bg-yellow-50 text-yellow-900 border-yellow-200"}`}>
                                {q.flag}
                              </span>
                            </td>
                          </tr>
  ...
  return (
    <div className="space-y-10">
      <div className="bg-white text-gray-800 border border-[#D3D1C7] p-4 mb-0 flex justify-between items-center">
          ...
          <table className="w-full border-collapse text-[10px] bg-transparent font-poppins">
            <thead>
              <tr className="bg-white text-gray-800 border-b border-[#D3D1C7] font-semibold uppercase text-center">
                <th className="border border-[#E5E7EB] p-3 w-[25%] text-left">Question</th>
                <th className="border border-[#E5E7EB] p-3 w-[60%] text-left">Suggested Answer</th>
            ...
            <tbody>
              {bpData.planQuestions?.map((q, i) => (
                <tr key={i} className={`align-top ${i % 2 === 0 ? "bg-white" : "bg-gray-50"}`}>
                  <td className="border border-[#E5E7EB] p-3 text-[#2C2C2C] font-semibold uppercase leading-tight">{q.question}</td>
                  <td className="border border-[#E5E7EB] p-3 text-[#2C2C2C]/80 font-medium leading-relaxed">{q.answer}</td>
                  <td className="border border-[#E5E7EB] p-3 text-center">
                    <span className={`px-2 py-1 rounded-sm font-semibold text-[8px] uppercase tracking-tighter block text-center ${q.flag.includes("Ready") ? "bg-green-50 text-green-800" : "bg-yellow-50 text-yellow-900"}`}>
                      {q.flag}
                    </span>
                  </td>
                </tr>
