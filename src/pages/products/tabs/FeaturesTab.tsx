...
import { getDisplayModule, getModuleTone, buildModuleIndexMap } from "../helpers";
              ...
              <div className="border border-[#D3D1C7] bg-white">
                ...
                <div className="w-full">
                  {hasDetailedFeatureSheet ? (
                    <table className="w-full table-fixed border-collapse text-[12px] leading-[1.55] font-poppins">
                      <thead>
                        <tr className="bg-gray-100 text-gray-800 border-b border-[#D3D1C7] font-semibold uppercase">
                          <th className="border border-[#E5E7EB] px-2 py-3 text-left w-[16%]">Feature Name</th>
                          <th className="border border-[#E5E7EB] px-2 py-3 text-left w-[30%]">Description</th>
                          <th className="border border-[#E5E7EB] px-2 py-3 text-center w-[6%]">USP?</th>
                          <th className="border border-[#E5E7EB] px-2 py-3 text-left w-[12%]">Module / Page</th>
                          <th className="border border-[#E5E7EB] px-2 py-3 text-left w-[12%]">User Type</th>
                          <th className="border border-[#E5E7EB] px-2 py-3 text-center w-[8%]">Status</th>
                          <th className="border border-[#E5E7EB] px-2 py-3 text-center w-[8%]">Priority</th>
                          <th className="border border-[#E5E7EB] px-2 py-3 text-left w-[18%]">Notes</th>
                        </tr>
                      </thead>
                      <tbody>
                        {featureItems.map((f, i) => (
                          <tr
                            ...
                            className={`${f.usp ? "bg-gray-100" : i % 2 === 0 ? "bg-white" : "bg-gray-50"} align-top`}
                          >
                            <td className="border border-[#E5E7EB] px-2 py-3 font-semibold text-[#2C2C2C] break-words whitespace-pre-line">
                              {f.feature}
                            </td>
                            <td className={`border border-[#E5E7EB] px-2 py-3 whitespace-pre-line break-words ${f.usp ? "text-[#374151] font-semibold" : "text-[#2C2C2C] font-medium"}`}>
                              {f.works}
                            </td>
                            <td className={`border border-[#E5E7EB] px-2 py-3 text-center font-bold ${f.usp ? "text-[#374151]" : "text-[#9CA3AF]"}`}>
                              {f.usp ? "Yes" : "No"}
                            </td>
                    ...
                    <table className="w-full border-collapse text-[9px] leading-[1.25] font-poppins">
                      <thead>
                        <tr className="bg-gray-100 text-gray-800 border-b border-[#D3D1C7] font-semibold uppercase">
                          <th className="border border-[#E5E7EB] px-1 py-1.5 text-center w-[4%]">#</th>
                          <th className="border border-[#E5E7EB] px-2 py-1.5 text-left w-[13%]">Module</th>
                          <th className="border border-[#E5E7EB] px-2 py-1.5 text-left w-[14%]">Feature</th>
                          <th className="border border-[#E5E7EB] px-2 py-1.5 text-left w-[16%]">Sub Feature</th>
                          <th className="border border-[#E5E7EB] px-2 py-1.5 text-left">How It Currently Works</th>
                          <th className="border border-[#E5E7EB] px-2 py-1.5 text-center w-[9%]">User Type</th>
                          <th className="border border-[#E5E7EB] px-2 py-1.5 text-center w-[4%]">USP</th>
                        </tr>
                      </thead>
                      <tbody>
                        {(() => {
                          type Group = { moduleKey: string; moduleLabel: string; items: typeof featureItems };
                          const groups: Group[] = [];
                          featureItems.forEach((it) => {
                            const key = it.module.trim().toLowerCase();
                            const last = groups[groups.length - 1];
                            if (!last || last.moduleKey !== key) {
                              groups.push({ moduleKey: key, moduleLabel: getDisplayModule(it.module, moduleIndexMap), items: [it] });
                            } else {
                              last.items.push(it);
                            }
                          });
                          ...
                          return groups.map((g) =>
                            g.items.map((f, localIdx) => {
                              ...
                              return (
                                <tr key={`${g.moduleKey}-${localIdx}-${f.feature}`} className={`${zebra} align-top`}>
                                  <td className="border border-[#E5E7EB] px-1 py-1.5 text-center font-bold text-[#2C2C2C]/50 text-[8px]">{excelFeatureRowStart + i}</td>
                                  {showModuleCell && (
                                    <td rowSpan={rowSpan} className="border border-[#E5E7EB] px-2 py-1.5 font-semibold align-top bg-gray-100 text-[#2C2C2C] text-[9px] break-words">
                                      {g.moduleLabel}
                                    </td>
                                  )}
                                  <td className="border border-[#E5E7EB] px-2 py-1.5 font-semibold text-[#2C2C2C] text-[9px] break-words">{f.feature}</td>
                                  <td className="border border-[#E5E7EB] px-2 py-1.5 text-[#2C2C2C] font-medium text-[9px] whitespace-pre-line break-words">{f.subFeatures}</td>
                                  <td className={`border border-[#E5E7EB] px-2 py-1.5 text-[9px] whitespace-pre-line break-words leading-relaxed ${f.usp ? 'text-[#374151] font-semibold' : 'text-[#2C2C2C]/80 font-medium'}`}>
                                    {f.works || ''}
                                  </td>
                                  <td className="border border-[#E5E7EB] px-1 py-1.5 text-center font-medium text-[8px] text-[#2C2C2C]/70 leading-snug break-words">{f.userType}</td>
                                  <td className={`border border-[#E5E7EB] px-1 py-1.5 text-center font-bold text-[8px] ${f.usp ? 'bg-gray-100 text-[#374151]' : 'bg-white text-[#D3D1C7]'}`}>
                                    {f.usp ? '★' : '—'}
                                  </td>
                                </tr>
                              );
                ...
                {hasDetailedFeatureSheet && (
                  <div className="border-t border-[#D3D1C7] px-3 py-2 text-[11px] text-[#4B5563] font-medium font-poppins bg-white">
                    Legend: gray rows are USP features that should appear in sales and marketing materials.
                  </div>
                )}
              </div>
            ) : (
