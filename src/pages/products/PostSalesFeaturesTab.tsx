    ...
    <div className="animate-fade-in font-poppins pb-20 w-full overflow-hidden">
      ...
      <div className="w-full mb-0">
        ...
        <div className="bg-[#e8eaf6] text-[#5c6bc0] p-2 text-center text-[9pt] border-b border-[#dddddd]">
          ★ USP features highlighted in blue  |  All {features.length} features listed  |  One row per feature
        </div>
      </div>

      {/* Feature Table */}
      <div className="overflow-x-auto w-full border-x border-[#dddddd]">
        <table className="w-full border-collapse border-spacing-0">
          <thead>
            <tr className="bg-[#1b3a5c] text-white font-bold text-[11pt] border-b border-[#dddddd]">
              <th className="p-3 border-r border-[#dddddd] w-[40px] text-center">#</th>
              <th className="p-3 border-r border-[#dddddd] w-[200px] text-center">Module / Section</th>
              <th className="p-3 border-r border-[#dddddd] w-[250px] text-center">Feature Name</th>
              <th className="p-3 border-r border-[#dddddd] text-center">Description</th>
          ...
          <tbody>
            {features.map((f: any, index: number) => {
              ...
              const moduleClass = `${baseCellClass} ${isUSP ? 'bg-[#e0f7fa] text-[#0d47a1]' : 'bg-[#fafafa] text-[#006064]'} font-bold`;
              const featureClass = `${baseCellClass} ${isUSP ? 'bg-[#f5f9ff] text-[#212121]' : 'bg-[#f5f5f5] text-[#212121]'} font-bold`;
              const descriptionClass = `${baseCellClass} ${isUSP ? 'bg-[#e3f2fd] text-[#0d47a1]' : 'bg-[#ffffff] text-[#212121]'} leading-relaxed`;
              const uspClass = `p-3 border-b border-[#dddddd] text-[10pt] align-middle text-center font-bold ${isUSP ? 'bg-[#e8f5e9] text-[#1b5e20]' : 'bg-[#ffebee] text-[#b71c1c]'}`;

              return (
                <tr key={index} className="border-b border-[#dddddd] min-h-[68px]">
