...
import { createProjectTeam } from '@/store/slices/projectTeamsSlice';
    ...
    try {
      const payload = {
        project_team: {
          ...
          user_ids: formData.teamMembers.map((member) => member),
        },
      }

      await dispatch(createProjectTeam({ baseUrl, token, data: payload })).unwrap();
