...
const AmenityBookingClubCalenderView = () => {
    ...
    const getFacilities = async () => {
        try {
            const response = await axios.get(`https://${baseUrl}/pms/admin/facility_setups.json?q[fac_type_eq]=${bookingType}&q[active_eq]=true`, {
                headers: {
    ...
    const handleSlotClick = (facilityId, slotId, isBooked, facilityBookingId, slot) => {
        if (isBooked && facilityBookingId) {
            ...
            if (currentPath.includes("bookings")) {
                ...
                navigate(`/club-management/amenities-booking-club/${facilityBookingId}`);
            } else {
            ...
            if (currentPath.includes("bookings")) {
                ...
                navigate(`/club-management/amenities-booking-club/add?${queryParams}`);
            } else {
    ...
    const handleAddBooking = () => {
        ...
        else if (currentPath.includes("club-management")) navigate("/club-management/amenities-booking-club/add");
...
export default AmenityBookingClubCalenderView;