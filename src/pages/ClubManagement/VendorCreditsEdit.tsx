...
export const VendorCreditsEdit: React.FC = () => {
    const { id: vendorCreditId } = useParams();
    const shouldPreserveVendorCreditSupply = useRef(false);
    ...
    const [isVendorCreditPrefillLoading, setIsVendorCreditPrefillLoading] = useState(Boolean(vendorCreditId));
    ...
    const mapVendorCreditToForm = (vendorCredit: any) => {
        ...
        const supplierId =
            vendorCredit?.pms_supplier_id ||
            vendorCredit?.supplier_id ||
            vendorCredit?.vendor_id ||
            vendorCredit?.supplier?.id;
        const supplierName = vendorCredit?.supplier_name || vendorCredit?.vendor_name || vendorCredit?.supplier?.company_name;
        const vendor =
            customers.find(c => String(c.id) === String(supplierId)) ||
            customers.find(c => String(c.company_name || c.name || '').trim().toLowerCase() === String(supplierName || '').trim().toLowerCase());
        const vendorCreditItems =
            vendorCredit?.item_details ||
            vendorCredit?.sale_order_items ||
            vendorCredit?.sale_order_items_attributes ||
            vendorCredit?.lock_account_supplier_credit_items ||
            [];
        const mappedItems = Array.isArray(vendorCreditItems)
            ? vendorCreditItems.map((item, index) => {
                ...
                return {
                    ...
                    account: String(item.lock_account_ledger_id || matchedItem?.account || item.account_id || ''),
        ...
        return {
            vendor,
            items: mappedItems,
            referenceNumber: vendorCredit?.order_number || vendorCredit?.reference_number || '',
            subject: vendorCredit?.subject || '',
            salesOrderDate: toDateInputValue(vendorCredit?.date || vendorCredit?.bill_date),
            billingAddress: vendorCredit?.billing_address?.formatted_address || vendorCredit?.supplier?.formatted_address || '',
            shippingAddress: vendorCredit?.shipping_address?.formatted_address || '',
            sourceOfSupply: vendorCredit?.source_of_supply || '',
            destinationOfSupply: vendorCredit?.destination_of_supply || '',
            customerNotes: vendorCredit?.notes || vendorCredit?.customer_notes || '',
            termsAndConditions: vendorCredit?.terms_and_conditions || '',
            discountOnTotal: Number(vendorCredit?.discount_per ?? vendorCredit?.discount_amount ?? 0),
            discountTypeOnTotal: vendorCredit?.discount_per ? 'percentage' : 'amount',
            adjustment: Number(vendorCredit?.charge_amount || 0),
            adjustmentLabel: vendorCredit?.charge_name || 'Adjustment',
            taxType: vendorCredit?.tax_type ? String(vendorCredit.tax_type).toUpperCase() : '',
            selectedTax: vendorCredit?.lock_account_tax_id ? String(vendorCredit.lock_account_tax_id) : '',
            reverseCharge: vendorCredit?.reverse_charge === true || vendorCredit?.reverse_charge === 'true',
        };
    };

    const fetchAndPrefillVendorCredit = async () => {
        if (!vendorCreditId) return;
        const creditBaseUrl = localStorage.getItem('baseUrl');
        const creditToken = localStorage.getItem('token');
        const creditLockAccountId = localStorage.getItem('lock_account_id');

        setIsVendorCreditPrefillLoading(true);
        try {
            const response = await axios.get(
                `https://${creditBaseUrl}/lock_account_supplier_credits/${vendorCreditId}.json?lock_account_id=${creditLockAccountId}&show=true`,
                {
                    headers: {
                        Authorization: creditToken ? `Bearer ${creditToken}` : undefined,
                        'Content-Type': 'application/json'
                    }
                }
            );
            const vendorCredit = response.data?.lock_account_supplier_credit || response.data;
            if (vendorCredit?.id) {
                const mapped = mapVendorCreditToForm(vendorCredit);
                shouldPreserveVendorCreditSupply.current = true;
                if (mapped.vendor) setSelectedCustomer(mapped.vendor);
                if (mapped.items.length > 0) setItems(mapped.items);
                setReferenceNumber(mapped.referenceNumber || '');
                setSubject(mapped.subject || '');
                setSalesOrderDate(mapped.salesOrderDate || '');
                setBillingAddress(mapped.billingAddress || '');
                setShippingAddress(mapped.shippingAddress || '');
                setSourceOfSupply(normalizeIndianState(mapped.sourceOfSupply));
                setDestinationOfSupply(normalizeIndianState(mapped.destinationOfSupply));
                setCustomerNotes(mapped.customerNotes || '');
                setTermsAndConditions(mapped.termsAndConditions || '');
                setDiscountOnTotal(mapped.discountOnTotal || 0);
                setDiscountTypeOnTotal(mapped.discountTypeOnTotal || 'percentage');
                setAdjustment(mapped.adjustment || 0);
                setAdjustmentLabel(mapped.adjustmentLabel || 'Adjustment');
                setReverseCharge(Boolean(mapped.reverseCharge));
                if (mapped.taxType === 'TDS' || mapped.taxType === 'TCS') {
                    setTaxType(mapped.taxType);
                }
                if (mapped.selectedTax) setSelectedTax(mapped.selectedTax);
            }
        } catch (error) {
            toast.error('Failed to fetch vendor credit details');
        } finally {
            setIsVendorCreditPrefillLoading(false);
        }
    };
    ...
    useEffect(() => {
        ...
        fetchAndPrefillVendorCredit();
        // eslint-disable-next-line
    }, [vendorCreditId, customers, itemOptions]);
    ...
    useEffect(() => {
        if (selectedCustomer) {
            ...
            fetchSupplierAddresses(selectedCustomer.id, Boolean(vendorCreditId || billingAddress || shippingAddress));
    ...
    useEffect(() => {
        if (selectedBillingAddress) {
            ...
            if (!shouldPreserveVendorCreditSupply.current && selectedBillingAddress.state) setSourceOfSupply(selectedBillingAddress.state);
        }
        // eslint-disable-next-line
    ...
    useEffect(() => {
        if (!sameAsBilling && selectedShippingAddress) {
            ...
            if (!shouldPreserveVendorCreditSupply.current && selectedShippingAddress.state) setDestinationOfSupply(selectedShippingAddress.state);
        }
        // eslint-disable-next-line
    ...
    const handleSubmit = async () => {
        ...
        if (!vendorCreditId) {
            toast.error("Vendor credit id is missing");
            return;
        }

        setIsSubmitting(true);
        const loadingToast = toast.loading("Updating Vendor Credit...");
        ...
        try {
            ...
            items.forEach((item, idx) => {
                ...
                if (item.charge_id) {
                    formData.append(`lock_account_supplier_credit[sale_order_items_attributes][${idx}][id]`, String(item.charge_id));
                }
                if (resolvedId) {
            ...
            const response = await fetch(`https://${baseUrl}/lock_account_supplier_credits/${vendorCreditId}.json?lock_account_id=${lock_account_id}`, {
                method: 'PUT',
    ...
    if (isVendorCreditPrefillLoading) {
        return (
    ...
    return (
        <div className="p-6 space-y-6 relative">
                                ...
                                <FormControl fullWidth error={!!errors.customer}>
                                    <Select
                                        ...
                                        onChange={(e) => {
                                            ...
                                            shouldPreserveVendorCreditSupply.current = false;
                                            setSelectedBillingAddressId(null);
                                            setSelectedShippingAddressId(null);
                                            setBillingAddress('');
                                            setShippingAddress('');
            ...
            <div className="flex items-center gap-3 justify-center pt-2">
                ...
                <Button className="fm-button-fix fm-button-brand px-8 py-2" onClick={() => handleSubmit()} disabled={isSubmitting}>
