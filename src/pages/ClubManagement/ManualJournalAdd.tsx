...
const ManualJournalAdd = () => {
	...
	const buildJournalPayload = () => {
		...
		return {
			...
			lock_account_transaction_records: rows.map(row => {
				...
				if (row.contact) {
					record.resource_id = row.contact.id;
					record.resource_type = row.contact.class_name;
				}
	...
	console.log("payload manual:", buildJournalPayload())
	const handleSubmit = async (e) => {
		...
		// Transaction fields
		formData.append(
			"lock_account_transaction[transaction_type]",
			"Journal Entry"
		);
		formData.append(
			"lock_account_transaction[transaction_date]",
			date
		);
		formData.append(
			"lock_account_transaction[description]",
			notes
		);
		formData.append(
			"lock_account_transaction[reference]",
			reference
		);
		formData.append(
			"lock_account_transaction[publish]",
			false
		);
		formData.append(
			"lock_account_transaction[reporting_method]",
			reportingMethod
		);
		formData.append(
		...
		rows.forEach((row, index) => {
			...
			formData.append(
				`lock_account_transaction_records[${index}][description]`,
				row.description || ""
			);

			if (row.contact) {
				formData.append(
					`lock_account_transaction_records[${index}][resource_id]`,
					row.contact.id
				);

				formData.append(
					`lock_account_transaction_records[${index}][resource_type]`,
					row.contact.class_name
				);
			}

			if (row.debit) {
				formData.append(
					`lock_account_transaction_records[${index}][dr]`,
					row.debit
				);
			}
			...
			if (row.credit) {
				formData.append(
					`lock_account_transaction_records[${index}][cr]`,
					row.credit
				);
			}
		});
		...
		attachments.forEach((file, index) => {
			formData.append(
				`lock_account_transaction[attachments_attributes][${index}][document]`,
				file
			);

			formData.append(
				`lock_account_transaction[attachments_attributes][${index}][active]`,
				true
			);
		});
										...
										<tr key={idx} className="hover:bg-gray-50">
											...
											<td className="border border-gray-300 px-4 py-3 ">
												<Select
													size="small"
													sx={{ width: 220 }}
													value={row.contact ? `${row.contact.id}-${row.contact.class_name}` : ''}
													displayEmpty
													onChange={(e) => {
														const [id, className] = e.target.value.split('-');

														const selected = contactOptions.find(
															(c) => c.id === Number(id) && c.class_name === className
														);

														handleRowChange(idx, 'contact', selected);
													}}
												>
													<MenuItem value="">
													...
													{contactOptions.map((contact) => (
														<MenuItem
															key={`${contact.id}-${contact.class_name}-${contact.name}`}
															value={`${contact.id}-${contact.class_name}`}
														>
															{contact.name}
														</MenuItem>
													))}
