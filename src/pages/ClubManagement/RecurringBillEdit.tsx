...
// Utility to map API item details to form item
const mapApiItemToFormItem = (item) => ({
  id: String(item.id),
  name: item.item_name || '',
...
// Utility to map API recurring_detail to form fields
const mapApiRecurringDetail = (recurring_detail) => ({
  repeatType: recurring_detail?.repeat_type || 'month',
  repeatCount: recurring_detail?.repeat_value?.toString() || '1',
  neverExpires: !!recurring_detail?.never_expires,
  salesOrderDate: recurring_detail?.start_date || '',
  expectedShipmentDate: recurring_detail?.end_date || '',
});
...
export const RecurringBillEdit: React.FC = () => {
  // Subject field
...
console.log("Editing bill with ID:", billId)
  // Fetch and prefill bill details
  useEffect(() => {
    if (!billId) return;
    const fetchBill = async () => {
      ...
      try {
        const res = await axios.get(`https://${baseUrl}/lock_account_bills/${billId}.json`, {
          headers: { Authorization: token ? `Bearer ${token}` : undefined, 'Content-Type': 'application/json' },
        });
        const data = res.data;
        // Set vendor
        if (data.pms_supplier_id) {
          setSelectedCustomer({ id: String(data.pms_supplier_id), name: data.vendor_name || '', company_name: data.vendor_name || '', email: '', currency: 'INR', billingAddress: '', shippingAddress: '', customerType: '', paymentTerms: '', portalStatus: '', language: '', outstandingReceivables: 0, unusedCredits: 0, contactPersons: [] });
        }
        setProfileName(data.profile_name || '');
        setSourceOfSupply(data.source_of_supply || '');
        setDestinationOfSupply(data.destination_of_supply || '');
        ...
        setDiscountTypeOnTotal(data.discount_per ? 'percentage' : 'amount');
        setSelectedTax(data.tax_type || '');
        setSelectedTerm(data.payment_term_id ? String(data.payment_term_id) : '');
        setTaxType((data.tax_type || 'tds').toUpperCase() === 'TCS' ? 'TCS' : 'TDS');
        setTotalAmount2(data.total_amount || 0);
        setSalesperson(data.sales_person_id ? String(data.sales_person_id) : '');
        // Recurring detail
        if (data.recurring_detail) {
          const recur = mapApiRecurringDetail(data.recurring_detail);
          setRepeatType(recur.repeatType);
          setRepeatCount(recur.repeatCount);
          setNeverExpires(recur.neverExpires);
          setSalesOrderDate(recur.salesOrderDate);
          setExpectedShipmentDate(recur.expectedShipmentDate);
        }
        // Items
        if (Array.isArray(data.item_details)) {
          setItems(data.item_details.map(mapApiItemToFormItem));
        }
      } catch (err) {
...
if (!profileName || profileName.trim() === "") {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      formData.append('lock_account_bill[sub_total_amount]', String(subTotal));
      formData.append('lock_account_bill[taxable_amount]', String(totalGSTAmount));
      formData.append('lock_account_bill[lock_account_tax_amount]', String(taxAmount2));
      ...
      formData.append('recurring_detail[end_date]', neverExpires ? '' : expectedShipmentDate);
      formData.append('recurring_detail[repeat_type]', repeatType);
      formData.append('recurring_detail[repeat_value]', String(repeatCount));
      formData.append('recurring_detail[never_expires]', neverExpires ? 'true' : 'false');
