...
export const CreditNoteClubEditPage: React.FC = () => {
    ...
    // --- Edit mode: fetch the existing credit_note and prefill the whole form ---
    ...
    useEffect(() => {
        ...
        const fetchCreditNoteForEdit = async () => {
            try {
                const baseUrl = localStorage.getItem('baseUrl');
                const token = localStorage.getItem('token');
                const lock_account_id = localStorage.getItem('lock_account_id');
                ...
                const data = res.data?.credit_note || res.data || {};
                ...
                if (Array.isArray(fetchedLineItems) && fetchedLineItems.length > 0) {
                    ...
                    const mappedItems: Item[] = fetchedLineItems.map((li: any, idx: number) => {
                        ...
                        // Confirmed: credit_note line_items use "item_name", not "name" (unlike bill_bookings)
                        const itemName = li.item_name || li.name || '';
                        // Confirmed: no top-level "amount" field on credit_note line_items — derive it
                ...
                // customer.id is an internal lock_account_customer id — customer.user_id (confirmed)
                // is the actual occupant/guest/staff id the Member/Guest/Staff dropdowns use.
                const billedUserId = data.customer?.user_id ?? data.customer?.id ?? data.user?.id;
                if (billedUserId) {
                ...
                console.error('Error fetching credit note for edit:', error);
                toast.error('Failed to load credit note details');
            } finally {
                setEditLoading(false);
            }
        };
        fetchCreditNoteForEdit();
    ...
    // so loading an existing credit note doesn't clobber its saved items/rates.
    ...
    // Maps a line_item_type to the PascalCase resource_type the credit_notes API expects
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            formData.append('credit_note[address_detail_attributes][gst_detail_id]', selectedGstDetail?.id ? String(selectedGstDetail.id) : '');
            formData.append('credit_note[address_detail_attributes][gst_preference]', gstTreatment || '');
            if (selectedBillingAddress) {
                formData.append('credit_note[address_detail_attributes][billing_address_attributes][address]', selectedBillingAddress.address || '');
                formData.append('credit_note[address_detail_attributes][billing_address_attributes][city]', selectedBillingAddress.city || '');
                formData.append('credit_note[address_detail_attributes][billing_address_attributes][state]', selectedBillingAddress.state || '');
                formData.append('credit_note[address_detail_attributes][billing_address_attributes][country]', selectedBillingAddress.country || '');
                formData.append('credit_note[address_detail_attributes][billing_address_attributes][pin_code]', selectedBillingAddress.pin_code || '');
                formData.append('credit_note[address_detail_attributes][billing_address_attributes][contact_person]', selectedBillingAddress.attention || '');
                formData.append('credit_note[address_detail_attributes][billing_address_attributes][mobile]', selectedBillingAddress.mobile || selectedBillingAddress.telephone_number || '');
            }
            if (selectedShippingAddress) {
                formData.append('credit_note[address_detail_attributes][shipping_address_attributes][address]', selectedShippingAddress.address || '');
                formData.append('credit_note[address_detail_attributes][shipping_address_attributes][city]', selectedShippingAddress.city || '');
                formData.append('credit_note[address_detail_attributes][shipping_address_attributes][state]', selectedShippingAddress.state || '');
                formData.append('credit_note[address_detail_attributes][shipping_address_attributes][country]', selectedShippingAddress.country || '');
                formData.append('credit_note[address_detail_attributes][shipping_address_attributes][pin_code]', selectedShippingAddress.pin_code || '');
            }
            ...
            await fetch(`https://${baseUrl}/lock_accounts/${lock_account_id}/credit_notes/${id}.json`, {
                method: 'PATCH',
                headers: {
                    Authorization: token ? `Bearer ${token}` : undefined
    ...
    return (
        <div className="p-6 space-y-6 relative">
            ...
            <div className="space-y-6">
                {/* Customer Section */}
                <Section title="Credit Note Information" icon={<Package className="w-5 h-5" />}>
                    <div className="space-y-6">
                        <div>
                            <RadioGroup
                                row
