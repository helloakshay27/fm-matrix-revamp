...
export const CreditNoteListPage: React.FC = () => {
    ...
    const fetchCreditNoteData = async (page = 1, per_page = 10, search = '', filters: CreditNoteFilters = {}) => {
        ...
        try {
                    const response = await axios.get(
                        `https://${baseUrl}/lock_account_credit_notes.json?lock_account_id=${lock_account_id}`,
                        {
                            params: {
                                // lock_account_id: 1,
                                //   page,
    ...
    const renderRow = (cn: CreditNote) => ({
        ...
        credit_note_number: (
            <div className="font-medium text-brand cursor-pointer" onClick={() => navigate(`/accounting/credit-note/${cn.id}`)}>
