...
export const DebitNoteClubEditPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchOrgState = async () => {
            ...
            try {
                ...
                console.error('[DebitNote Edit] Failed to fetch org state:', err);
            }
        };
        fetchOrgState();
    }, []);
    ...
    const mapItem = (item: any): Item => {
        ...
        const lineItemId =
            ...
            item.lock_account_debit_note_item_id ??
    ...
    useEffect(() => {
        ...
        const fetchDebitNoteDetails = async () => {
            ...
            try {
                ...
                const lock_account_id = localStorage.getItem('lock_account_id');
                const response = await axios.get(
                    `https://${baseUrl}/lock_account_debit_notes/${id}.json?lock_account_id=${lock_account_id}&show=true`,
                    { headers: { Authorization: token ? `Bearer ${token}` : undefined } }
                );
                const data = response.data?.lock_account_debit_note || response.data?.data || response.data || {};
                ...
                setDebitNoteDate(String(data.date || data.debit_note_date || '').slice(0, 10));
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            formData.append('lock_account_debit_note[id]', String(id));
            formData.append('lock_account_debit_note[lock_account_customer_id]', selectedCustomer?.id || '');
            formData.append('lock_account_debit_note[reference_number]', referenceNumber);
            formData.append('lock_account_debit_note[date]', debitNoteDate);
            formData.append('lock_account_debit_note[sales_person_id]', salesperson);
            ...
            formData.append('lock_account_debit_note[sub_total_amount]', String(subTotal));
            formData.append('lock_account_debit_note[taxable_amount]', '0');
            formData.append('lock_account_debit_note[lock_account_tax_amount]', String(tdsTcsAmount));
            formData.append('lock_account_debit_note[total_amount]', String(totalAmount));
            ...
            formData.append('lock_account_debit_note[lock_account_tax_id]', selectedTaxOption?.id ? String(selectedTaxOption.id) : selectedTax);
            ...
            formData.append('lock_account_debit_note[address_detail_attributes][billing_address_id]', billingAddressId);
            formData.append('lock_account_debit_note[address_detail_attributes][shipping_address_id]', shippingAddressId);
            formData.append('lock_account_debit_note[address_detail_attributes][gst_detail_id]', gstDetailId);
            formData.append('lock_account_debit_note[address_detail_attributes][gst_preference]', gstPreference);

            items.forEach((item, index) => {
                if (item.persisted && item.id) {
                    formData.append(`lock_account_debit_note[sale_order_items_attributes][${index}][id]`, item.id);
                }
                const resolvedId = item.item_id || itemOptions.find(opt => opt.name === item.name)?.id;
                if (resolvedId) {
                    formData.append(`lock_account_debit_note[sale_order_items_attributes][${index}][lock_account_item_id]`, String(resolvedId));
                } else {
                    formData.append(`lock_account_debit_note[sale_order_items_attributes][${index}][item_name]`, item.name);
                }
                formData.append(`lock_account_debit_note[sale_order_items_attributes][${index}][rate]`, String(item.rate));
                formData.append(`lock_account_debit_note[sale_order_items_attributes][${index}][quantity]`, String(item.quantity));
                formData.append(`lock_account_debit_note[sale_order_items_attributes][${index}][total_amount]`, String(item.amount));
                formData.append(`lock_account_debit_note[sale_order_items_attributes][${index}][name]`, item.description || '');
                formData.append(`lock_account_debit_note[sale_order_items_attributes][${index}][description]`, item.description || '');
                formData.append(`lock_account_debit_note[sale_order_items_attributes][${index}][lock_account_ledger_id]`, item.account || '');
                formData.append(`lock_account_debit_note[sale_order_items_attributes][${index}][tax_type]`, item.item_tax_type || '');
                formData.append(`lock_account_debit_note[sale_order_items_attributes][${index}][tax_group_id]`, String(item.tax_group_id || ''));
                formData.append(`lock_account_debit_note[sale_order_items_attributes][${index}][tax_exemption_id]`, String(item.tax_exemption_id || ''));
            });

            const response = await fetch(`https://${baseUrl}/lock_account_debit_notes/${id}.json?lock_account_id=${lock_account_id}`, {
                method: 'PUT',
                headers: {
        ...
        return (
            <div className="flex items-center justify-center h-96">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading debit note...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="p-6 space-y-6 relative">
            ...
            <header className="mb-4">
                <button type="button" onClick={() => navigate('/club-management/debit-note')} className="flex items-center gap-2 text-black font-medium mb-2">
                    <ArrowLeft className="h-4 w-4 text-black" />
                    Back to Debit Note List
                </button>
                <h1 className="text-2xl font-bold text-black">Edit Debit Note</h1>
            </header>
            ...
            <div className="space-y-6">
                ...
                <Section title="Debit Note Details" icon={<Calendar className="w-5 h-5" />}>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-2">Debit Note Date<span className="text-red-500">*</span></label>
                            <TextField fullWidth type="date" value={debitNoteDate} onChange={e => setDebitNoteDate(e.target.value)} sx={fieldStyles} InputLabelProps={{ shrink: true }} />
                        </div>
                        <div>
                    ...
                    <div className="mt-6">
                        ...
                        <TextField fullWidth multiline rows={2} value={subject} onChange={e => setSubject(e.target.value)} placeholder="Let your customer know what this Debit Note is for" />
                    </div>
                </Section>
            ...
            <div className="flex items-center gap-3 justify-center pt-2">
                ...
                <Button onClick={() => navigate('/club-management/debit-note')} disabled={isSubmitting} variant="outline" className="fm-button-fix px-8 py-2">
                    Cancel
                </Button>
            </div>
...
export default DebitNoteClubEditPage;
