...
export const QuotesDashboard: React.FC = () => {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            const lock_account_id = localStorage.getItem('lock_account_id');
            await axios.delete(
                `https://${baseUrl}/lock_account_quotes/${selectedDeleteId}.json${lock_account_id ? `?lock_account_id=${lock_account_id}` : ''}`,
                {
                    headers: {
                        'Content-Type': 'application/json',
            ...
            toast.success('Quote deleted successfully!', { duration: 3000 });
    ...
    const handleToggleStatus = async (order: SalesOrder) => {
        try {
            const baseUrl = localStorage.getItem('baseUrl');
            const token = localStorage.getItem('token');
            const url = `https://${baseUrl}/lock_account_quotes/${order.id}/toggle_active.json`;
