...
export const EditPaymentReceivedPage: React.FC = () => {
  const { id } = useParams();
  ...
  // skip the invoice fetch triggered by the initial data load setting selectedCustomerId
  const skipInvoiceFetchRef = useRef(true);
  ...
  useEffect(() => {
    ...
    setSubmitting(true);
    axios.get(`https://${baseUrl}/lock_payments/${id}.json?lock_account_id=${lock_account_id}`, { headers: authHeaders })
      .then((res) => {
        const data = res.data?.data || res.data;
        if (!data) return;

        setSelectedCustomerId(data.customer_id || data.payment_of_id || "");
        setPaymentNumber(data.payment_number || data.order_number || "");
        setDate(data.payment_date ? format(new Date(data.payment_date), "yyyy-MM-dd") : format(new Date(), "yyyy-MM-dd"));
        
        // Map payment mode case-insensitively
        const modes = ["Bank Remittance", "Bank Transfer", "Cash", "Cheque", "Credit Card", "UPI"];
        const rawMode = data.payment_mode || data.payment_method || "";
        const matchedMode = modes.find(m => m.toLowerCase() === rawMode.toLowerCase().replace(/_/g, ' ')) || rawMode;
        setPaymentMode(matchedMode);
        
        setReference(data.neft_reference || data.reference || "");
        setNotes(data.notes || "");
        
        if (data.tax_deducted) {
          setTaxDeducted("yes");
          setTdsAccount(data.tds_lock_account_ledger_id?.toString() || "Advance Tax");
        } else {
          setTaxDeducted("no");
        }
        ...
        if (data.advance) {
          setActiveTab(1);
          setAdvanceAmount(data.paid_amount?.toString() || data.payment_amount?.toString() || data.total_amount?.toString() || data.amount?.toString() || "");
          setAdvanceBankCharges(data.bank_charges?.toString() || "");
          setAdvanceDepositTo(data.deposit_to_ledger_id?.toString() || data.deposit_to_account_id?.toString() || "");
          setPlaceOfSupply(data.place_of_supply || "");
          setDescriptionOfSupply(data.description_of_supply || "");
          
          if (data.tax_rate_id) {
            setAdvanceTax(`GST ${data.tax_rate_id}%`);
          } else if (data.tax_rate && data.tax_rate.rate) {
            setAdvanceTax(`GST ${data.tax_rate.rate}%`);
          } else if (data.tax_rate) {
            setAdvanceTax(`GST ${data.tax_rate}%`);
          }
        } else {
          setActiveTab(0);
          setAmountReceived(data.paid_amount?.toString() || data.payment_amount?.toString() || data.total_amount?.toString() || data.amount?.toString() || "");
          setBankCharges(data.bank_charges?.toString() || "");
          setDepositTo(data.deposit_to_ledger_id?.toString() || data.deposit_to_account_id?.toString() || "");
          
          // Pre-populate invoiceRows if existing
          if (data.invoice_payments && Array.isArray(data.invoice_payments)) {
            const mappedRows = data.invoice_payments.map((inv) => ({
              ...
              paymentReceivedOn: inv.paymentReceivedOn || format(new Date(), "yyyy-MM-dd"),
              withholdingTax: inv.withholdingTax?.toString() || '',
              payment: inv.payment?.toString() || ''
            }));
  ...
  useEffect(() => {
    ...
    // Skip the fetch on initial load — existing payment data already set the rows
    if (skipInvoiceFetchRef.current) {
      skipInvoiceFetchRef.current = false;
      return;
    }
  ...
  // if "received full amount" is checked, keep amountReceived synced with invoice payments
  useEffect(() => {
