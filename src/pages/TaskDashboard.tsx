...
export const TaskDashboard = () => {
  ...
  const handleSearch = (value: string) => {
    setSearchTerm(value);
    if (value) {
      const filtered = taskData.filter(task => task.id.toLowerCase().includes(value.toLowerCase()) || task.taskNumber.toLowerCase().includes(value.toLowerCase()) || task.description.toLowerCase().includes(value.toLowerCase()) || task.category.toLowerCase().includes(value.toLowerCase()));
  ...
  const handleStatusFilter = (status: string) => {
    ...
    if (status === 'all') {
      ...
      const filtered = taskData.filter(task => task.status.toLowerCase() === status.toLowerCase());
      setFilteredTasks(filtered);
    }
  };
  ...
  const handleExport = () => {
    const tasksToExport = selectedTasks.length > 0 ? filteredTasks.filter(task => selectedTasks.includes(task.id)) : filteredTasks;
    const csvContent = [['Task ID', 'Task Number', 'Description', 'Category', 'Sub Category', 'Created By', 'Assigned To', 'Status', 'Priority', 'Created On'], ...tasksToExport.map(task => [task.id, task.taskNumber, task.description, task.category, task.subCategory, task.createdBy, task.assignedTo, task.status, task.priority, task.createdOn])].map(row => row.join(',')).join('\n');
  ...
  return <div className="p-4 sm:p-6">
      ...
      <div className="flex flex-wrap gap-3 items-center mb-6">
        ...
        <div className="relative ml-auto w-full sm:w-64">
          ...
          <Input placeholder="Search tasks..." value={searchTerm} onChange={e => handleSearch(e.target.value)} className="pl-10 bg-white" />
        </div>
