...
const EditMoMPage = () => {
  ...
  const { data: momDetail, loading } = useSelector(
    (state: RootState) => state.fetchMoMDetail
  );
    ...
    if (momDetail && typeof momDetail === "object" && "title" in momDetail) {
      ...
      if (mom.mom_tasks && Array.isArray(mom.mom_tasks)) {
        const mappedPoints = mom.mom_tasks.map((task: any) => ({
          ...
          tags: task.company_tag_id
            ? [{ value: task.company_tag_id, label: task.tag_name || "" }]
            : [],
        }));
  ...
  const handleSubmit = async () => {
    ...
    combinedUsers.forEach((user: any, index) => {
      formDataPayload.append(
        `mom_detail[mom_attendees_attributes][${index}][id]`,
        user.id
      );
      formDataPayload.append(
    ...
    if (deletedAttendeeIds.length > 0) {
      ...
      deletedAttendeeIds.forEach((id) => {
        formDataPayload.append(
          `mom_detail[mom_attendees_attributes][${attendeeIndex}][id]`,
          String(id)
        );
        formDataPayload.append(
          `mom_detail[mom_attendees_attributes][${attendeeIndex}][_destroy]`,
          "true"
        );
        attendeeIndex++;
      });
    ...
    if (deletedPointIds.length > 0) {
      ...
      deletedPointIds.forEach((id) => {
        formDataPayload.append(
          `mom_detail[mom_tasks_attributes][${pointIndex}][id]`,
          String(id)
        );
        formDataPayload.append(
          `mom_detail[mom_tasks_attributes][${pointIndex}][_destroy]`,
          "true"
        );
        pointIndex++;
      });
    ...
    try {
      ...
      toast.error(
        typeof error === "string" ? error : "Failed to update meeting minutes"
      );
    }
  };
      ...
      <Paper elevation={0} className="my-4">
        ...
        <div className="mb-8">
          <Typography variant="subtitle1" fontWeight="600" color="text.primary">
            {existingAttachments.length + attachments.length} Document(s)
            Attached
          </Typography>
