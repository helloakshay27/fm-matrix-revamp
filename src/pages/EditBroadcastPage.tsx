    ...
    useEffect(() => {
        const fetchBroadcastData = async () => {
            try {
                ...
                // Parse the expire_time to get date
                let expiryDate = "";
                if (response.expire_time) {
                    const expireDateTime = new Date(response.expire_time);
                    expiryDate = expireDateTime.toISOString().split('T')[0];
                }
                ...
                if (response.shared === 1) {
                    ...
                    // Get site IDs from the response
                    if (response.site_ids && response.site_ids.length > 0) {
                        selectedSites = response.site_ids;
                    }
                }
                    ...
                    <div className="p-6 bg-white">
                        <div className={`grid ${formData.showOnHomeScreen === "yes" ? "grid-cols-1 md:grid-cols-5" : "grid-cols-1"} gap-6`}>
                            ...
                            <div>
                                ...
                                {formData.attachment || existingAttachmentUrl ? (
                                    <div className="relative border-2 border-dashed border-gray-400 rounded-lg w-full max-w-[200px] h-40 flex items-center justify-center bg-white">
                                        <span className="absolute top-2 left-3 text-sm font-medium text-gray-700 truncate max-w-[80%]">
                                            {formData.attachment?.name || "Existing"}
                                        </span>
                                        <button
                            ...
                            {formData.showOnHomeScreen === "yes" && (
                                <div>
                                    ...
                                    {formData.coverImage || existingCoverImageUrl ? (
                                        <div className="relative border-2 border-dashed border-gray-400 rounded-lg w-full max-w-[200px] h-40 flex items-center justify-center bg-white">
                                            <span className="absolute top-2 left-3 text-sm font-medium text-gray-700 truncate max-w-[80%]">
                                                {formData.coverImage?.name || "Existing"}
                                            </span>
                                            <button
