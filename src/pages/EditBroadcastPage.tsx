    ...
    useEffect(() => {
        ...
        const fetchBroadcastData = async () => {
            try {
                ...
                // Parse the expire_time to get date and time
                let endDate = "";
                let endTime = "";
                if (response.expire_time) {
                    const expireDateTime = new Date(response.expire_time);
                    endDate = expireDateTime.toISOString().split('T')[0];
                    const hours = String(expireDateTime.getHours()).padStart(2, '0');
                    const minutes = String(expireDateTime.getMinutes()).padStart(2, '0');
                    endTime = `${hours}:${minutes}`;
                }

                // Determine share with value
                let shareWith = "all";
                let selectedIndividuals = [];
                let selectedGroups = [];

                if (response.shared === 1) {
                    // Check if it's individuals or groups
                    if (response.swusers && response.swusers.length > 0) {
                        shareWith = "individuals";
                        selectedIndividuals = response.swusers.map(user => user.id);
                    } else if (response.group_id) {
                        shareWith = "groups";
                        selectedGroups = [response.group_id];
                    }
                }
                ...
                setFormData({
                    ...
                    markAsImportant: response.isImportant || false,
                    endDate,
                    endTime,
                    shareWith,
                    selectedIndividuals,
                ...
                >
                    ...
                    >
                        ...
                        <Typography variant="body2" color="text.secondary">
                            Choose files | {(attachments.length + existingAttachments.length) > 0 ? `${attachments.length + existingAttachments.length} file(s) chosen` : "No file chosen"}
                        </Typography>
                    </Box>
                            ...
                            {existingAttachments.map((file, index) => (
                                ...
                                >
                                    <CardMedia
                                        component="img"
                                        height="100"
                                        image={file.document_url}
                                        alt={file.document_file_name || "Attachment"}
                                        sx={{ objectFit: "cover" }}
                                    />
                                    <IconButton
                                        sx={{
                                    ...
                                    >
                                        {file.document_file_name || "Existing"}
                                    </Typography>
                                </Card>
                            ))}
