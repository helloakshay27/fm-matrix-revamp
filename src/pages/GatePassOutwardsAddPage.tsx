...
<td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4"><TextField variant="outlined" placeholder='Unit' size="small" sx={{ '& .MuiOutlinedInput-root': { borderRadius: '0px' } }} value={row.unit} onChange={(e) => {
                      const value = e.target.value;
                      if (/^[a-zA-Z\s]*$/.test(value)) handleRowChange(row.id, 'unit', value);
                    }}
                      inputProps={{ pattern: "[a-zA-Z\s]*" }}
                    /></td>
                    <td className="px-4 py-4"><TextField placeholder='Enter Description' variant="outlined" size="small" value={row.description} sx={{ '& .MuiOutlinedInput-root': { borderRadius: '0px' } }} onChange={(e) => handleRowChange(row.id, 'description', e.target.value)} /></td>
                    <td className="px-4 py-4">
