...
export const TaskSubmissionPage: React.FC = () => {
  ...
  const getChecklistFromTaskDetails = (): ChecklistItem[] => {
    ...
    return activityResp.map((item: any, index: number) => ({
      ...
      type: item.type === "radio-group" ? "radio" : item.type === "checkbox-group" ? "checkbox" : "text",
      required: item.required === "true" || item.required === true,
      options: item.values?.map(val => val.label || val.value) || ["Yes", "No"],
    }));
  };
  ...
  const getTaskName = () => {
    return taskDetails?.task_details?.task_name || "Task Submission";
  };
  ...
  const getTaskStatus = () => {
    return taskDetails?.task_details?.status?.display_name || 
  ...
  useEffect(() => {
    const fetchTaskDetails = async () => {
      ...
      try {
        ...
        const mappedDetails = (response as any).task_occurrence
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-sm text-gray-600 mt-1">
              Task #{taskDetails?.task_details?.id || taskDetails?.id} • 
              Assigned to: {taskDetails?.task_details?.assigned_to || "Not assigned"} • 
              {taskDetails?.task_details?.scheduled_on || "No schedule"}
            </div>
            <div className="text-sm text-gray-500 mt-1">
              {getLocationDetails()}
            </div>
            <div className="text-sm text-gray-500 mt-1">
              Status: <span className="font-medium text-gray-700">{getTaskStatus()}</span> • 
              {taskDetails?.task_details?.task_duration && (
                <>Duration: {taskDetails.task_details.task_duration} • </>
              )}
              Service Code: {taskDetails?.task_details?.asset_service_code || "N/A"}
            </div>
          </div>
        </div>
      </div>
