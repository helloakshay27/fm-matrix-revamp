...
export const TrainingConductedDashboard = () => {
  ...
  const fetchAuditsConducted = async () => {
    try {
      ...
      toast.error("Failed to fetch training conducted data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePrintReport = async (
    ...
    if (!printPdfUrl) {
      toast.error("PDF report not available for this training");
      return;
    }
  ...
  const handleDeleteReport = async (
    ...
    if (!deleteUrl) {
      toast.error("Cannot delete report for this training");
      return;
    }
      ...
      case "actions":
        return shouldShow("Training", "show") ? (
          <Button
            variant="ghost"
            size="sm"
            onClick={() => console.log("View conducted training:", item.id)}
          >
      ...
      <div className="mb-6">
        <div>
          <p className="text-[#1a1a1a] opacity-70 mb-2">
            Training &gt; Training Conducted List
          </p>
          <h1 className="text-2xl font-bold text-[#1a1a1a]">
            TRAINING CONDUCTED LIST
          </h1>
        </div>
      </div>
