...
// // ─── Section wrapper (identical to Create Page) ───────────────────────────────
...
// // ─── Interfaces (identical to Create Page) ────────────────────────────────────
// interface Vendor {
//     id: string;
//     name: string;
//     email: string;
...
// // ─── BulkItemModal (identical to Create Page) ────────────────────────────────
...
// // ─── INDIAN STATES (same as Create Page) ─────────────────────────────────────
// const INDIAN_STATES = [
...
//     const { id } = useParams<{ id: string }>();   // PO id from route
...
//     // ── Loading state for the initial PO fetch ────────────────────────────────
...
//         name: '', item_id: null, description: '',
//         quantity: 1, rate: 0, discount: 0,
//         discountType: 'percentage', tax: '', taxRate: 0, amount: 0,
//         account_id: 0, item_tax_type: '', tax_group_id: null, tax_exemption_id: null,
//     }]);
...
//     // ─────────────────────────────────────────────────────────────────────────
//     // Helper utilities (identical to Create Page)
//     // ─────────────────────────────────────────────────────────────────────────
...
//     // ─────────────────────────────────────────────────────────────────────────
//     // FETCH EXISTING PO AND PREFILL (edit-specific)
//     // ─────────────────────────────────────────────────────────────────────────
//     const prefillForm = useCallback(async (po: any, vendorList: Vendor[]) => {
//         // Basic fields
//         setPurchaseOrderNumber(po.po_number || po.purchase_order_number || '');
//         setReferenceNumber(po.reference_number || '');
//         setPurchaseOrderDate(po.po_date || '');
...
//         setAdjustment(Number(po.adjustment) || 0);
//         setAdjustmentLabel(po.adjustment_label || 'Adjustment');
//         setDiscountOnTotal(Number(po.discount) || 0);

//         // Tax type (TDS/TCS)
//         if (po.tax_type) setTaxType(po.tax_type as 'TDS' | 'TCS');

//         // Existing attachments
//         if (Array.isArray(po.attachments)) setExistingAttachments(po.attachments);

//         // Vendor
//         const vendorId = po.pms_supplier_id?.toString() || po.supplier?.id?.toString() || '';
//         const matched = vendorList.find(v => v.id === vendorId);
//         if (matched) setSelectedVendor(matched);

//         // Fetch full vendor detail so address books / gst details load
//         if (vendorId) {
//             try {
//                 const res = await axios.get(
//                     `https://${baseUrl}/pms/suppliers/${vendorId}.json?access_token=${token}`
...
//                 // Set selected address IDs from PO response
//                 const billId = po.billing_address_id?.toString() || nextBilling[0]?.id?.toString() || null;
//                 const shipId = po.shipping_address_id?.toString() || nextShipping[0]?.id?.toString() || null;
//                 setSelectedBillingAddressId(billId);
//                 setSelectedShippingAddressId(shipId);

//                 const nextGst: any[] = Array.isArray(data.gst_details) ? data.gst_details : [];
//                 setGstDetails(nextGst);
//                 const gstId = po.gst_detail_id?.toString() || null;
//                 setSelectedGstDetailId(gstId || nextGst[0]?.id?.toString() || null);
//             } catch (err) {
//                 console.error('Error fetching vendor detail during prefill:', err);
//             }
//         }

//         // Payment terms
//         setPaymentTerms(po.payment_term_id?.toString() || '');

//         // Delivery address type
//         const dType = po.delivery_adress_type || po.delivery_address_type || 'organization';
//         setDeliveryAddressType(dType as 'organization' | 'customer');

//         // Line items
//         const rawItems: any[] = po.pms_po_inventories || po.inventories || po.line_items || [];
...
//                     item_id: ri.pms_inventory_id?.toString() || ri.item_id?.toString() || null,
//                     name: ri.inventory?.inventory_name || ri.name || '',
//                     description: ri.prod_desc || ri.description || '',
//                     quantity: qty,
//                     rate,
...
//                     taxRate: 0,
//                     amount,
//                     account_id: ri.ledger_id || ri.account_id || 0,
//                     item_tax_type: ri.item_tax_type || '',
//                     tax_group_id: ri.tax_group_id || null,
...
//             // vendors must already be loaded before prefill so we can match by id
//             // we pass the current vendor list; if empty we load inline
//             setLoadingPO(false);
//             return po;
//         } catch (err) {
...
//     // ─────────────────────────────────────────────────────────────────────────
//     // On mount: load all dropdown data then fetch + prefill PO
//     // ─────────────────────────────────────────────────────────────────────────
//     useEffect(() => {
//         const init = async () => {
//             // 1. Fetch vendors
...
//             if (po) await prefillForm(po, vendorList);
...
//                 setItemOptions(raw.map((item: any) => ({
//                     id: item.id,
...
//                         res.data.map((pt: any) => ({ id: pt.id?.toString(), name: pt.name, days: pt.no_of_days }))
//                     );
//                 }
//             } catch (err) {
...
//             // 5. Org state
//             try {
//                 const orgId = localStorage.getItem('org_id') || localStorage.getItem('organisation_id');
//                 if (orgId) {
//                     const res = await axios.get(
...
//             } catch (err) { setAccountGroups([]); }

//             // 10. Delivery customers
//             try {
//                 const res = await axios.get(
//                     `https://${baseUrl}/lock_account_customers.json?lock_account_id=${lock_account_id}`,
//                     { headers: { Authorization: `Bearer ${token}` } }
//                 );
//                 setDeliveryCustomers(res.data || []);
...
//     // ── Fetch taxes when taxType changes ──────────────────────────────────────
//     useEffect(() => {
...
//     const taxRate2 = selectedTaxObj?.rate || 0;
...
//                     ...(item.id && !item.id.includes('-') ? { id: item.id } : {}),
...
//             const selectedTaxObj2 = taxOptions.find(t => t.name === selectedTax);
...
//                     tax_id: selectedTaxObj2?.id,
//                     tax_type: taxType,
//                     discount: totalDiscount,
//                     adjustment,
//                     sub_total: subTotal,
//                     tax_value: (afterDiscount * taxRate2) / 100,
//                     tax_percentage: selectedTaxObj2?.rate || 0,
...
//                                                                         )),
//                                                                     ]
//                                                                     : <MenuItem key={`group-${group.id}`} value={group.id}>{group.group_name}</MenuItem>
//                                                             )
...
//                                         {taxOptions.map(tax => (
//                                             <MenuItem key={tax.id || tax.name} value={tax.name}>{tax.name}</MenuItem>
//                                         ))}
//                                     </Select>
//                                 </FormControl>
...
//                     <div className="space-y-4">
//                         {/* Existing attachments from the saved PO */}
...
//             {/* ── Vendor Drawer (identical to Create Page) ───────────────────── */}
...
export const PurchaseOrderEditPage: React.FC = () => {
    ...
    const applyPOData = async (po: any, vendorList: Vendor[], loadedItemOptions: any[]) => {
        ...
        const poTaxType: 'TDS' | 'TCS' = (po.tax_type?.toUpperCase() === 'TCS') ? 'TCS' : 'TDS';
        setTaxType(poTaxType);

        // Fetch tax options for this type, and apply the saved tax name
        // We store the name to look up because the PO typically stores tax_id, not name
        // Try name first, then look up by id
        const poTaxName = po.tax_name || po.tax?.name || '';
        const poTaxId = po.tax_id?.toString() || '';
        ...
        try {
            ...
            if (poTaxName) {
                const matched = opts.find((t: any) => t.name === poTaxName);
                if (matched) { setSelectedTax(matched.name); pendingTaxNameRef.current = null; }
            } else if (poTaxId) {
                const matched = opts.find((t: any) => String(t.id) === poTaxId);
                if (matched) { setSelectedTax(matched.name); }
            }
        } catch { /* silent */ }
        ...
        if (dType === 'customer' && po.delivery_address_id) {
            // deliveryCustomers may not be loaded yet; store for later application
            pendingDeliveryCustomerIdRef.current = String(po.delivery_address_id);
        }
        ...
        if (rawItems.length > 0) {
            const mappedItems: Item[] = rawItems.map((ri: any, idx: number) => {
                ...
                const savedInventoryId = ri.pms_inventory_id?.toString() || ri.item_id?.toString() || null;
