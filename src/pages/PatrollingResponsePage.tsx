...
import { Eye, QrCode, MapPin, ExternalLink, Shield, Activity } from 'lucide-react';
...
export const PatrollingResponsePage = () => {
  ...
  const fetchCheckpoints = useCallback(async (page: number = 1, search?: string) => {
    setIsLoading(true);
    try {
      ...
      if (!result.success) throw new Error(result.message || 'Failed to fetch checkpoints');
  ...
  const columns = React.useMemo(() => [
    { key: 'location_path', label: 'Location', sortable: true, draggable: true, defaultVisible: true, hideable: true },
    { key: 'scheduled_times', label: 'Scheduled Time', sortable: false, draggable: true, defaultVisible: true, hideable: true },
    { key: 'created_on', label: 'Created On', sortable: true, draggable: true, defaultVisible: true, hideable: true },
    { key: 'start_date', label: 'Start Date', sortable: true, draggable: true, defaultVisible: true, hideable: true },
    { key: 'end_date', label: 'End Date', sortable: true, draggable: true, defaultVisible: true, hideable: true },
    { key: 'grace_time_hours', label: 'Grace Time(Hours)', sortable: true, draggable: true, defaultVisible: true, hideable: true },
    { key: 'active', label: 'Active/Inactive', sortable: true, draggable: true, defaultVisible: true, hideable: true },
  ...
  const renderCell = (item: CheckpointListItem, columnKey: string) => {
    switch (columnKey) {
      ...
      case 'created_on':
        return <span className="text-sm">{formatDateTime(item.created_on)}</span>;
      case 'start_date':
  ...
  const renderActions = (item: CheckpointListItem) => (
    <div className="flex items-center gap-1">
      <Button
        ...
        onClick={() => navigate(`/security/patrolling/checkpoints/${item.id}/history`)}
      >
  ...
  return (
    <div className="flex-1 p-4 sm:p-6 bg-white min-h-screen">
      ...
      <Dialog open={showQrPreview} onOpenChange={setShowQrPreview}>
        <DialogContent className="max-w-sm" aria-describedby="qr-preview-desc">
          <DialogHeader>
            ...
            <div id="qr-preview-desc" className="sr-only">QR code preview for checkpoint</div>
          </DialogHeader>
          <div className="flex items-center justify-center p-4 bg-white rounded-lg border border-gray-200">
            {qrPreviewUrl ? (
              <img src={qrPreviewUrl} alt="QR Code" className="max-w-full h-auto max-h-[400px]" />
            ) : (
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Patrolling Checkpoints</h1>
          <p className="text-sm text-gray-600">View and manage patrolling checkpoints</p>
        </div>
      </div>
