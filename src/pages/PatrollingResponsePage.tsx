...
export const PatrollingResponsePage = () => {
  ...
  const { onPatrolResponseViewed, onPatrolResponseFiltered } = usePatrolEvents();
  ...
  const fetchPatrollingResponses = useCallback(async (
    ...
    try {
      ...
      if (!hasTrackedView.current) {
        onPatrolResponseViewed({ row_count: result.data.pagination?.total_count || result.data.summary?.total_visits || 0 });
  ...
  const handleApplyFilters = () => {
    ...
    const activeFilterNames = Object.entries(newFilters)
      .filter(([_, v]) => v !== '' && v !== null && v !== undefined)
      .map(([k]) => k)
      .join(',');
      
    if (activeFilterNames) {
      onPatrolResponseFiltered({ filter_by: activeFilterNames as any });
    }
