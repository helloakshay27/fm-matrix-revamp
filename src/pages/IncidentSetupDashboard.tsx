      ...
      <div className="flex gap-8">
        ...
        <div className="flex-1">
          {selectedCategory === 'Incident Disclaimer' ? (
            // Empty section for Incident Disclaimer
            <div></div>
          ) : (
            <>
              {isEditing ? (/* Edit Form Modal */
                ...
                <>
                  ...
                  <div className="bg-white p-6 rounded-lg border shadow-sm mb-6">
                    <div className="space-y-4">
                      {selectedCategory === 'Escalations' ? <>
                          <h2 className="text-lg font-semibold text-gray-900 mb-4">Add New Escalation</h2>
                          ...
                          <div>
                            ...
                            <div className="space-y-2">
                              <div className="flex flex-wrap gap-2">
                                {escalateToUsers.split(',').filter(user => user.trim()).map((user, index) => <div key={index} className="bg-gray-200 px-3 py-1 rounded-md flex items-center gap-2">
                                    <span className="text-sm">{user.trim()}</span>
                                    <button onClick={() => {
                              const userList = escalateToUsers.split(',').filter(u => u.trim() !== user.trim());
                              ...
                              <Select onValueChange={value => {
                          const currentUsers = escalateToUsers ? escalateToUsers.split(',').map(u => u.trim()) : [];
                          if (!currentUsers.includes(value)) {
                            const newUsers = [...currentUsers, value].filter(u => u);
                          ...
                          <h2 className="text-lg font-semibold text-gray-900 mb-4">Add New Approval Setup</h2>
                          ...
                          <div>
                            ...
                            <div className="space-y-2">
                              <div className="flex flex-wrap gap-2">
                                {selectedApprovalUsers.split(',').filter(user => user.trim()).map((user, index) => <div key={index} className="bg-gray-200 px-3 py-1 rounded-md flex items-center gap-2">
                                    <span className="text-sm">{user.trim()}</span>
                                    <button onClick={() => {
                              const userList = selectedApprovalUsers.split(',').filter(u => u.trim() !== user.trim());
                              setSelectedApprovalUsers(userList.join(', '));
                            }} className="text-gray-500 hover:text-gray-700">
                                      ×
                                    </button>
                                  </div>)}
                              ...
                              <Select onValueChange={value => {
                          const currentUsers = selectedApprovalUsers ? selectedApprovalUsers.split(',').map(u => u.trim()) : [];
                          if (!currentUsers.includes(value)) {
                            const newUsers = [...currentUsers, value].filter(u => u);
                            setSelectedApprovalUsers(newUsers.join(', '));
                          }
                        }}>
                                <SelectTrigger className="w-full">
                                  <SelectValue placeholder="Select users to add" />
                          ...
                          <h2 className="text-lg font-semibold text-gray-900 mb-4">Add New Sub Category</h2>
                          ...
                          <h2 className="text-lg font-semibold text-gray-900 mb-4">Add New Sub Sub Category</h2>
                            ...
                            <Select value={selectedSubCategory} onValueChange={setSelectedSubCategory}>
                              ...
                              <SelectContent className="bg-white z-50">
                                {subCategories
                                  ...
                                  .map(subCategory => <SelectItem key={subCategory.id} value={subCategory.subCategory}>
                          ...
                          <h2 className="text-lg font-semibold text-gray-900 mb-4">Add New Secondary Sub Category</h2>
                          ...
                          <div>
                            ...
                            <Input
                              placeholder="Enter Secondary Sub Category Name"
                              value={categoryName}
                              onChange={(e) => setCategoryName(e.target.value)}
                              className="focus:ring-[#C72030] focus:border-[#C72030]"
                            />
                          ...
                          <h2 className="text-lg font-semibold text-gray-900 mb-4">Add New Secondary Sub Sub Category</h2>
                            ...
                            <Select value={selectedSecondarySubCategory} onValueChange={setSelectedSecondarySubCategory}>
                              ...
                              <SelectContent className="bg-white z-50">
                                {secondarySubCategories
                                  ...
                                  .map(subCategory => <SelectItem key={subCategory.id} value={subCategory.secondarySubCategory}>
                                        {subCategory.secondarySubCategory}
                                      </SelectItem>)}
                              </SelectContent>
                            </Select>
                          ...
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                              Secondary Sub Sub Category Name
                            </label>
                            <Input
                              placeholder="Enter Secondary Sub Sub Category Name"
                              value={categoryName}
                              onChange={(e) => setCategoryName(e.target.value)}
                              className="focus:ring-[#C72030] focus:border-[#C72030]"
                            />
                          ...
                          <h2 className="text-lg font-semibold text-gray-900 mb-4">Add New Secondary Sub Sub Sub Category</h2>
                            ...
                            <Select value={selectedSecondarySubSubCategory} onValueChange={setSelectedSecondarySubSubCategory}>
                              ...
                              <SelectContent className="bg-white z-50">
                                {secondarySubSubCategories
                                  ...
                                  .map(subSubCategory => <SelectItem key={subSubCategory.id} value={subSubCategory.secondarySubSubCategory}>
                                        {subSubCategory.secondarySubSubCategory}
                                      </SelectItem>)}
                              </SelectContent>
                            </Select>
                          ...
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                              Secondary Sub Sub Sub Category Name
                            </label>
                            <Input
                              placeholder="Enter Secondary Sub Sub Sub Category Name"
                              value={categoryName}
                              onChange={(e) => setCategoryName(e.target.value)}
                              className="focus:ring-[#C72030] focus:border-[#C72030]"
                            />
                          ...
                          <h2 className="text-lg font-semibold text-gray-900 mb-4">Add New {selectedCategory}</h2>
                  ...
                  <div className="bg-white rounded-lg border shadow-sm">
                    <div className="p-6">
                      <h2 className="text-lg font-semibold text-gray-900 mb-4">{selectedCategory} List</h2>
                      
                      <Table>
                        <TableHeader>
                          <TableRow className="bg-gray-50 border-b border-[#D5DbDB]">
                            {selectedCategory === 'Secondary Sub Category' ? <>
                        ...
                        <TableBody>
                          {selectedCategory === 'Secondary Sub Category' ? secondarySubCategories.map(secondarySub => <TableRow key={secondarySub.id} className="hover:bg-gray-50 border-b border-[#D5DbDB]">
                                ...
                                <TableCell className="px-4 py-3">
                                 ...
                                 </TableRow>) : selectedCategory === 'Secondary Sub Sub Category' ? secondarySubSubCategories.map(item => <TableRow key={item.id} className="hover:bg-gray-50 border-b border-[#D5DbDB]">
                            ...
                            </TableRow>) : selectedCategory === 'Sub Category' ? subCategories.map(subCategory => <TableRow key={subCategory.id} className="hover:bg-gray-50 border-b border-[#D5DbDB]">
                              <TableCell className="px-4 py-3 text-sm font-medium text-gray-900">{subCategory.category}</TableCell>
                              <TableCell className="px-4 py-3 text-sm text-gray-600">{subCategory.subCategory}</TableCell>
                              <TableCell className="px-4 py-3">
                                <div className="flex gap-2">
                                  <Button variant="ghost" size="sm" className="text-blue-600 hover:text-blue-800" onClick={() => handleEdit(subCategory, 'Sub Category')}>
                                    <Edit className="w-4 h-4" />
                                  </Button>
                                  <Button variant="ghost" size="sm" className="text-red-600 hover:text-red-800" onClick={() => handleDelete(subCategory, 'Sub Category')}>
                                    <Trash2 className="w-4 h-4" />
                                  </Button>
                                </div>
                              </TableCell>
