...
export const WorkOrderAddPage: React.FC = () => {
      ...
      const cloneData = async () => {
        try {
          ...
          setFormData({
            ...
            woDate: data.work_order?.wo_date ? data.work_order.wo_date.split("T")[0] : "",
  ...
  const handleDetailsChange = (id: number, field: string, value: string) => {
  ...
  const addNewDetailsForm = () => {
    const newId = Math.max(...detailsForms.map((form) => form.id), 0) + 1;
    const newForm = {
      id: newId,
      service: "",
      productDescription: "",
  ...
  const validateForm = () => {
    ...
    if (!formData.relatedTo) {
      toast.error("Please enter related to");
      return false;
    }
    for (const item of detailsForms) {
      ...
      if (!item.productDescription) {
        toast.error("Please enter a product description for all items");
        return false;
      }
      ...
      if (!item.expectedDate) {
        toast.error("Please enter an expected date for all items");
        return false;
      }
  ...
  return (
    <div className="p-6 mx-auto">
      ...
      <h1 className="text-2xl font-bold mb-6">ADD WORK ORDER</h1>
