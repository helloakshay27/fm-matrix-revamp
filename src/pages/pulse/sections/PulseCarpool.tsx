...
import {
  ...
  fetchCarpoolTrend,
  fetchCarpoolTrust,
  fetchCarpoolTopDrivers,
  fetchCarpoolRidesList,
  fetchCarpoolReportedRidesList,
...
function kpiEntries(
  raw: unknown
): { label: string; value: number; currency: boolean }[] {
  if (!raw || typeof raw !== "object") return [];
  return Object.entries(raw as Record<string, unknown>)
    ...
    .map(([k, v]) => ({
      label: titleCase(k),
      value: v as number,
      currency: /revenue|fare|amount|price|earning/i.test(k),
    }));
}
...
function normalizeTrust(raw: unknown): {
  stars: StarPoint[];
  summary: TrustSummary | undefined;
} {
  const obj = raw as Partial<TrustResponse> | undefined;
  if (!obj?.stars || !Array.isArray(obj.series))
    return { stars: [], summary: undefined };
...
export function PulseCarpool({ filters }: Props) {
  ...
  useEffect(() => {
    if (!loading)
      fetchCarpoolRidesList(filters, ridesPage)
        .then(setRides)
        .catch(console.error);
  }, [ridesPage]);
  ...
  useEffect(() => {
    if (!loading)
      fetchCarpoolReportedRidesList(filters, reportsPage)
      ...
      <div className="pd-section-header">
        ...
        <div>
          ...
          <div className="pd-section-subtitle">
            Ride sharing activity, trust and safety across sites
          </div>
        </div>
      </div>
          ...
          <div className="pd-chart-card">
            <div className="pd-chart-title">
              ...
              {trustSummary && (
                ...
                >
                  {trustSummary.open_reports.toLocaleString()} open /{" "}
                  {trustSummary.total_reports.toLocaleString()} reports
                </span>
              )}
            </div>
            {trustSummary && (
              ...
              >
                <span>
                  ...
                  <strong style={{ color: C.purple }}>
                    {trustSummary.avg_driver_rating.toFixed(1)}★
                  </strong>{" "}
                  ({trustSummary.driver_rating_count})
                </span>
                <span>
                  Avg Passenger Rating:{" "}
                  <strong style={{ color: C.teal }}>
                    {trustSummary.avg_passenger_rating.toFixed(1)}★
                  </strong>{" "}
                  ({trustSummary.passenger_rating_count})
                </span>
                <span>
                  Total Ratings:{" "}
                  <strong>{trustSummary.total_ratings.toLocaleString()}</strong>
                </span>
              </div>
            )}
            <div className="pd-chart-inner">
