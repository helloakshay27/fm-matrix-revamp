...
import {
  fetchEventsKpi,
  fetchEventsRegistrationsKpi,
  fetchEventsByCategory,
  fetchTopEvents,
  type EventsKpi,
  type EventsRegistrationsKpi,
...
export function PulseEvents({ filters }: Props) {
  ...
  const hasData = !!(
    kpi ||
    regKpi ||
    (byCategory && byCategory.categories.length) ||
    (topEvents && topEvents.top_events.length)
  );
      ...
      <div className="pd-section-header">
        ...
        <div>
          ...
          <div className="pd-section-subtitle">
            Event schedule, registrations and attendance
          </div>
        </div>
      </div>
            ...
            <table className="pd-table">
              <thead>
                <tr>
                  ...
                  <th className="pd-num">Attended</th>
                </tr>
              </thead>
              <tbody>
                {topEvents.top_events.map((e, i) => (
                  <tr key={e.event_id}>
                    ...
                    <td style={{ whiteSpace: "nowrap", fontSize: 12 }}>
                      {fmtDateTime(e.from_time)}
                    </td>
                    <td style={{ whiteSpace: "nowrap", fontSize: 12 }}>
                      {fmtDateTime(e.to_time)}
                    </td>
                    <td>
                      <span
                        className={`pd-badge ${e.is_paid ? "pd-badge-warn" : "pd-badge-yes"}`}
                      >
                        {e.is_paid ? "Paid" : "Free"}
                      </span>
                    </td>
