...
import { fetchProjectsTags, updateProjectsTags } from "@/store/slices/projectTagSlice";
...
const ProjectTags = () => {
    ...
    const handleCheckboxChange = async (item: any) => {
        ...
        try {
            ...
            setTags((prevData: any[]) =>
                prevData.map((row) =>
                    row.id === itemId ? { ...row, active: newStatus } : row
                )
            );

            toast.success(`Tag ${newStatus ? "activated" : "deactivated"} successfully`);
        } catch (error) {
