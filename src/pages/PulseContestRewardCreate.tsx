...
const PulseContestRewardCreate = () => {
    ...
    const fetchContests = async () => {
        ...
        try {
            const response = await axios.get(
                `https://${baseUrl}/contests.json?q[content_type_eq]=special_discount`,
                {
                    headers: { Authorization: `Bearer ${token}` },
                }
            );
            ...
            setContests(
                arr.map((c: any) => ({
                    ...
                    prizes: (c.prizes || []).map((p: any) => ({
                        ...
                        prize_allocated: p.prize_allocated,
                    })),
                }))
            );
        } catch {
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    <MuiSelect
                                        ...
                                        label="Contest *"
                                        onChange={(e) =>
                                            handleContestChange(e.target.value as string)
                                        }
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    <MuiSelect
                                        ...
                                        label="Prize *"
                                        onChange={(e) =>
                                            handlePrizeChange(e.target.value as string)
                                        }
                                    ...
                                    >
                                        {!form.contest_id ? (
                                            ...
                                            prizes
                                                .map((p) => (
                                                    <MenuItem key={p.id} value={String(p.id)} disabled={p.prize_allocated}>
                                                        {p.coupon_code}
                                                    </MenuItem>
                                                ))
                                        )}
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 sticky top-6">
                        ...
                        <div className="space-y-3 text-sm">
                            <SummaryRow
                                label="User"
                                value={
                                    users.find((u) => String(u.id) === form.user_id)?.full_name ||
                                    "—"
                                }
                            />
                            <SummaryRow
                                label="Contest"
                                value={
                                    contests.find((c) => String(c.id) === form.contest_id)
                                        ?.name || "—"
                                }
                            />
                            <SummaryRow
                                label="Prize"
                                value={
                                    prizes.find((p) => String(p.id) === form.prize_id)?.title ||
                                    "—"
                                }
                            />
                            <SummaryRow
                                ...
                                value={
                                    form.reward_type
                                        ? form.reward_type.charAt(0).toUpperCase() +
                                        form.reward_type.slice(1)
                                        : "—"
                                }
                            />
                            <SummaryRow
