    ...
    const fetchFacilityBookingDetails = async () => {
        try {
            ...
            setFormData({
                ...
                amenities: {
                    tv: responseData.generic_tags?.some(tag => tag.category_name === "TV") || false,
                    whiteboard: responseData.generic_tags?.some(tag => tag.category_name === "Whiteboard") || false,
                    casting: responseData.generic_tags?.some(tag => tag.category_name === "Casting") || false,
                    smartPenForTV: responseData.generic_tags?.some(tag => tag.category_name === "Smart Pen for TV") || false,
                    wirelessCharging: responseData.generic_tags?.some(tag => tag.category_name === "Wireless Charging") || false,
                    meetingRoomInventory: responseData.generic_tags?.some(tag => tag.category_name === "Meeting Room Inventory") || false,
                },
                seaterInfo: responseData.seater_info || "Select a seater",
                floorInfo: responseData.location_info || "Select a floor",
            ...
            setSelectedFile(responseData?.cover_image?.document ? [responseData.cover_image.document] : []);
            ...
            setCancellationRules([
                {
                    ...
                    time: { type: responseData.cutoff_hr || "Hr", value: responseData.cutoff_min || "00", day: responseData.cutoff_day || "0" },
                    deduction: responseData.return_percentage || "",
                },
                {
                    description: "If user cancels the booking selected hours/days prior to schedule, a percentage of the amount will be deducted",
                    time: { type: responseData.cutoff_second_hr || "Hr", value: responseData.cutoff_second_min || "00", day: responseData.cutoff_second_day || "0" },
                    deduction: responseData.return_second_percentage || "",
                },
                {
                    description: "If user cancels the booking selected hours/days prior to schedule, a percentage of the amount will be deducted",
                    time: { type: responseData.cutoff_third_hr || "Hr", value: responseData.cutoff_third_min || "00", day: responseData.cutoff_third_day || "0" },
                    deduction: responseData.return_third_percentage || "",
                },
            ]);
        } catch (error) {
