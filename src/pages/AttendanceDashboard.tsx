...
export const AttendanceDashboard = () => {
  ...
  const handleSearch = (value: string) => {
    setSearchTerm(value);
    if (value) {
      const filtered = attendanceData.filter(person => person.name.toLowerCase().includes(value.toLowerCase()) || person.department.toLowerCase().includes(value.toLowerCase()));
  ...
  return <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <Input placeholder="Search attendance..." value={searchTerm} onChange={e => handleSearch(e.target.value)} className="pl-10 w-64 bg-white" />
          </div>
          <Button variant="outline" onClick={handleReset} className="border-[#C72030] text-[#C72030] hover:bg-[#C72030]/10">
            Reset
          </Button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border">
        <Table>
          ...
          <TableBody>
            {currentItems.map(person => <TableRow key={person.id}>
                <TableCell>
                  <Button variant="ghost" size="sm" onClick={() => handleViewDetails(person.id)} className="hover:bg-gray-100">
          ...
          <Pagination>
            <PaginationContent>
              ...
              return <PaginationItem key={pageNumber}>
                    <PaginationLink href="#" isActive={currentPage === pageNumber} onClick={e => {
                  e.preventDefault();
