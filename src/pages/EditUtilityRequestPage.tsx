...
export const EditUtilityRequestPage = () => {
    ...
    // Fetch utilization details by ID
    useEffect(() => {
        if (!id) {
            setDetailsError('No utilization ID provided');
            return;
        }
        ...
        const fetchUtilizationDetails = async () => {
            ...
            try {
                ...
                const response = await fetch(`${url}/compile_utilizations/${id}.json`, {
                    method: 'GET',
                    headers: {
                ...
                if (!response.ok) {
                    throw new Error(`Failed to fetch utilization details: ${response.status} ${response.statusText}`);
                }
                ...
                setDetailsError(err instanceof Error ? err.message : 'Failed to fetch utilization details');
            } finally {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Failed to update utilization: ${response.status} ${response.statusText}${errorData.error ? ': ' + errorData.error : ''}`);
            }
    ...
    // Show loading screen while fetching utilization details
    if (loadingDetails) {
        return (
            <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
                <div className="flex items-center justify-center min-h-[400px]">
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-gray-600">Loading utilization details...</p>
                    </div>
                </div>
            </div>
        );
    ...
    // Show error if failed to load utilization details
    if (detailsError) {
        return (
            <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
                <div className="flex items-center justify-center min-h-[400px]">
                    <div className="flex flex-col items-center gap-4">
                        <p className="text-red-500 text-center">{detailsError}</p>
                        <Button
                            onClick={() => navigate('/utility/utility-request')}
                            className="bg-[#C72030] hover:bg-[#A01B29] text-white"
                        >
