...
export const TicketDashboard = () => {
  ...
  const handleViewDetails = (ticketId: string) => {
    ...
    if (currentPath.includes("/club-management/helpdesk")) {
      navigate(`/club-management/helpdesk/details/${ticketId}`);
  ...
  const handleExport = async () => {
    console.log(
      "TicketDashboard - Export action for tickets:",
      selectedTickets
    );
  ...
  const handleStatusCardClick = (cardType: string) => {
    ...
    if (cardType === "total") {
      ...
      setFilters((prevFilters) => {
        ...
        console.log(
          "Total card - Clearing status filters, keeping:",
          otherFilters
        );
        return otherFilters;
      });
  ...
  const TruncatedDescription = ({
    ...
    return (
      <div className="w-32 max-w-[150px] group relative">
  ...
  const renderCell = (item, columnKey) => {
    if (columnKey === "actions") {
      return (
        <div className="flex items-center justify-center gap-1 w-full h-full min-h-[40px]">
          ...
          <div
            title={`${
              item.is_golden_ticket ? "Remove" : "Mark as"
            } Golden Ticket`}
            className="p-1 hover:bg-gray-100 rounded transition-colors"
          >
            <Star
    ...
    if (columnKey === "issue_status") {
      return (
        <span
          className={`px-2 py-1 rounded text-xs animate-scale-in cursor-pointer hover:opacity-80 transition-opacity ${
            item.issue_status === "Pending"
            ...
            {[
              ...
              return (
                <div
                  ...
                  onClick={() =>
                    item.clickable && handleStatusCardClick(item.type)
                  }
                >
