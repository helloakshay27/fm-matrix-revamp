...
export const EditSurveyPage = () => {
  ...
  const [existingSurveyImage, setExistingSurveyImage] =
    useState<SurveyImage | null>(null);
  ...
  const fetchSurveyData = async () => {
    try {
      ...
      const hasTicketConfig =
        surveyData.ticket_configs && surveyData.ticket_configs.active;
      ...
      if (hasTicketConfig && surveyData.ticket_configs) {
        ...
        const subcategoryId =
          ticketConfig.subcategory_id || ticketConfig.sub_category_id;
        ...
        if (ticketConfig.category_id && subcategoryId) {
          ...
          console.log(
            ...
            subcategoryId,
            "from field:",
            ticketConfig.subcategory_id ? "subcategory_id" : "sub_category_id"
          );
          ...
          try {
            const loadedSubcategories = await loadTicketSubCategories(
            ...
            // Set the subcategory immediately after loading subcategories
          ...
          console.log(
            "Case 2: Only subcategory ID available, finding parent category"
          );
          console.log(
            "Using subcategory_id for reverse lookup:",
            subcategoryId
          );
          try {
            ...
            console.error(
              "Error loading subcategories to find parent category:",
              error
            );
          }
        ...
        const assignedToId =
          ticketConfig.assigned_to_id || ticketConfig.assignedtoid;
        if (assignedToId) {
  ...
  const handleRemoveQuestion = (id: string) => {
    ...
    const activeQuestions = questions.filter((q) => {
      ...
      return (
        ...
        hasValidId &&
        isNotNullData &&
        (hasValidText || hasValidAnswerType || q.id.startsWith("new_"))
      );
    });
    ...
    if (activeQuestions.length > 1) {
      setQuestions((prevQuestions) => {
        ...
        console.log(
          ...
          remainingActiveQuestions.length
        );
  ...
  const handleUpdateQuestion = async () => {
    ...
    try {
      ...
      questions.forEach((question, questionIndex) => {
        ...
        const isNewQuestion =
          !question.id || question.id === "1" || question.id.startsWith("new_");
      ...
      if (createTicket) {
        ...
        console.log(
          "   category_type:",
          ticketSubCategory ? "subcategory" : "category"
        );
      }
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-2">
                <input
                  ...
                  className="w-4 h-4 text-[#C72030] bg-white border-gray-300 rounded focus:ring-[#C72030] focus:ring-2 accent-[#C72030] cursor-pointer"
                />
                <label
                  htmlFor="form-view"
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-2">
                ...
                <label
                  ...
                  className="text-sm font-medium text-gray-700 cursor-pointer"
                >
                  Create Ticket on Question Response
                </label>
              </div>
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <MuiSelect
                        ...
                        onChange={(e) =>
                          handleTicketCategoryChange(e.target.value)
                        }
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <MuiSelect
                        ...
                        disabled={
                          loadingTicketSubCategories || !ticketCategoryId
                        }
                      >
                        <MenuItem value="">
                      ...
                      {(() => {
                        const activeCount = questions.filter((q) => {
                          const isMarkedForDeletion =
                            q.markedForDeletion === true;
                          const hasValidId = q.id && q.id.trim() !== "";
                          const hasValidText = q.text && q.text.trim() !== "";
                          const hasValidAnswerType =
                            q.answerType && q.answerType.trim() !== "";
                          const isNotNullData =
                            q.text !== null && q.answerType !== null;
                          return (
                            !isMarkedForDeletion &&
                            hasValidId &&
                            isNotNullData &&
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {(() => {
                  ...
                  const activeQuestions = questions.filter((question) => {
                    ...
                    const hasValidAnswerType =
                      question.answerType && question.answerType.trim() !== "";
                    const isNotNullData =
                      question.text !== null && question.answerType !== null;

                    console.log(
                      `Question ${question.id}: markedForDeletion=${isMarkedForDeletion}, hasValidId=${hasValidId}, hasValidText=${hasValidText}, hasValidAnswerType=${hasValidAnswerType}, isNotNullData=${isNotNullData}`
                    );
                  ...
                  console.log(
                    "Active questions after filtering:",
                    activeQuestions
                  );
                  return activeQuestions;
                })().map((question, index) => (
                  <Card
                    key={`active-${question.id}-${index}`}
                    className="relative"
                  >
