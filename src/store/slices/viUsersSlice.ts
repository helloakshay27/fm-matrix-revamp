    ...
    async ({
        ...
        try {
            ...
            const transformedUsers: ViUser[] = response.data.users.map((user) => ({
                ...
                roleName: user.lock_user_permission?.role_name || user.lock_role?.name || "",
                ...
                registrationSource: user.lock_user_permission?.registration_source || "",
            ...
            if (axios.isAxiosError(error)) {
                return rejectWithValue(error.response?.data?.message || "Failed to fetch Vi Users");
            }
...
export const fetchViRoles = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
        ...
        try {
            ...
            if (axios.isAxiosError(error)) {
                return rejectWithValue(error.response?.data?.message || "Failed to fetch Vi Roles");
            }
    ...
    async ({
        ...
        try {
            ...
            if (lockUserPermissionId) {
                formData.append("user[lock_user_permissions_attributes][0][id]", String(lockUserPermissionId));
            }
            ...
            if (webEnabled !== undefined && lockUserPermissionId) {
                formData.append("user[lock_user_permissions_attributes][0][web_enabled]", String(webEnabled));
            }
