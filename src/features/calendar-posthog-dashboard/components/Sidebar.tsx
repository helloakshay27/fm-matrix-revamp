  ...
  return (
    <aside className="sidebar">
      <h1 className="brandmark">
        <span className="bm-full">Calendar App</span>
        <span className="bm-mini">CA</span>
      </h1>
      <p className="brandmark-sub">Calendar App · tenant value TBD</p>
      <nav aria-label="Sections">
        <div className="nav-group">
          ...
          {ORDER.map((key) => (
            ...
            >
              <span className="ni-ic">{NAV_ICONS[key]}</span>
              <span className="ni-t">{PAGE_TITLES[key]}</span>
            </button>
          ))}
        </div>
      </nav>
