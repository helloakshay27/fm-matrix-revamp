/**
 * Chart palette. The HTML original read these off `getComputedStyle(documentElement)` at
 * render time; here the two themes are declared directly so a chart can be rendered
 * without a live DOM measurement. Values track `vi-posthog-dashboard.css` — keep in sync.
 */
export type ViTheme = 'light' | 'dark';
...
export function paletteFor(theme: ViTheme): ChartPalette {
