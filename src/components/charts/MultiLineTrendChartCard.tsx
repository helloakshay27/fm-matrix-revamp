...
function LineTooltip({ active, payload, label, unit }: TooltipProps<number, string> & { unit?: string }) {
...
function LineLegend({ payload }: LegendProps) {
...
export interface MultiLineTrendChartCardProps {
  title: string;
  subtitle?: string;
...
/**
 * Plain (unfilled) multi-series line chart with an optional dashed threshold
 * reference line — mirrors "Vendor response time trend" in fm_matrix_phase10
 * (29).html. New chart form: AreaTrendChartCard/MultiAreaTrendChartCard both
 * fill under the line; this compares several trends against a fixed SLA
 * target line, where a fill would visually compete with the threshold.
 */
export function MultiLineTrendChartCard({
  title,
  subtitle,
  data,
  categoryKey,
            ...
            <LineChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              ...
              {referenceValue !== undefined && (
                <ReferenceLine
                  ...
                  strokeDasharray="4 3"
                  label={{ value: referenceLabel, position: "insideTopRight", fontSize: 10, fill: "var(--color-text)" }}
                />
              )}
              {series.map((s) => (
                <Line
