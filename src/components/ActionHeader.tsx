...
export const ActionHeader = () => {
    const { currentModule, availableModules, setCurrentModule, isActionSidebarVisible } =
        useActionLayout();
    const { isSidebarCollapsed } = useLayout();

    // Don't render if action sidebar is not visible
    if (!isActionSidebarVisible || availableModules.length === 0) {
        return null;
    }
                    ...
                    <div className="flex w-max lg:w-full space-x-4 md:space-x-6 lg:space-x-0 md:justify-start lg:justify-between whitespace-nowrap">
                        {availableModules.map((module) => (
                            <button
                                ...
                                className={`pb-3 text-sm transition-colors whitespace-nowrap flex-shrink-0 ${currentModule === module.module_name
