        ...
        <tbody>
          {!rows.length ? (
            ...
            <tr key={i}>
              ...
              <td className="border px-2 py-2 text-left">{r.contract_start_date || r.start_date || ''}</td>
              <td className="border px-2 py-2 text-left">{r.contract_end_date || r.end_date || ''}</td>
              <td className="border px-2 py-2 text-left">{r.renewal_reminder || r.renewal_alert || r.renewal_status || ''}</td>
              <td className="border px-2 py-2 text-left">{Number(r.projected_renewal_cost ?? r.contract_value ?? r.projected_value ?? 0).toLocaleString()}</td>
              <td className="border px-2 py-2 text-left">{r.vendor_contact || r.vendor_name || r.vendor_email || r.vendor_details || ''}</td>
              <td className="border px-2 py-2 text-left">{r.status || r.contract_status || 'Expired'}</td>
            </tr>
          ))}
        </tbody>
      </table>
