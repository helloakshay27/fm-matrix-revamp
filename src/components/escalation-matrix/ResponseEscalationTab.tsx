...
import {
  createResponseEscalation,
  clearState,
  fetchResponseEscalations,
  updateResponseEscalation,
  deleteResponseEscalation,
} from "@/store/slices/responseEscalationSlice";
import {
  ResponseEscalationApiFormData,
  FMUserDropdown,
  EscalationMatrixPayload,
  ResponseEscalationGetResponse,
  UpdateResponseEscalationPayload,
...
const responseEscalationSchema = z.object({
  categoryIds: z
    ...
    .max(15, "Maximum 15 categories allowed"),
...
export const ResponseEscalationTab: React.FC = () => {
  ...
  const { data: categoriesData, loading: categoriesLoading } = useSelector(
    (state: RootState) => state.helpdeskCategories
  );
  const {
    ...
    updateLoading,
    deleteLoading,
  } = useSelector((state: RootState) => state.responseEscalation);
  ...
  const categoryOptions =
    categoriesData?.helpdesk_categories?.map((cat) => ({
      value: cat.id,
      label: cat.name,
    })) || [];
  const userOptions =
  ...
  const getUserName = (id: number) => {
    return (
      escalationUsers.find((user) => user.id === id)?.full_name ||
      "Unknown User"
    );
  };
  ...
  const availableCategories =
    categoriesData?.helpdesk_categories?.filter(
      (cat) => !selectedCategories.includes(cat.id)
    ) || [];

  const getAvailableUsers = (level: keyof typeof selectedUsers) => {
    return escalationUsers.filter(
      (user) => !selectedUsers[level].includes(user.id)
    );
  };
  ...
  const handleUserSelect = (
    level: keyof typeof selectedUsers,
    userId: number
  ) => {
    ...
    const newUsers = {
      ...selectedUsers,
      [level]: [...selectedUsers[level], userId],
    };
  ...
  const handleUserRemove = (
    ...
    const newUsers = {
      ...selectedUsers,
      [level]: selectedUsers[level].filter((id) => id !== userId),
    };
    setSelectedUsers(newUsers);
    form.setValue("escalationLevels", newUsers);
  };
        ...
        if (typeof error === "string") {
          ...
          if (
            ...
            if (
              ...
              categoryError.toLowerCase().includes("already been taken")
            ) {
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <Card>
          <CardHeader>
            <CardTitle className="text-brand">Response Escalation Configuration</CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            {/* Category Selection Dropdown */}
            <div className="space-y-3">
              <Label className="text-base font-semibold text-brand">Category Type</Label>
              <ReactSelect
                isMulti
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-[#EDEAE3]">
                      <TableHead className="font-semibold text-brand">Levels</TableHead>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          className="h-8 w-8 p-0 text-brand hover:text-brand-hover hover:bg-[#EDEAE3]"
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                ...
                                escalation rule for{" "}
                                {getCategoryName(rule.category_id)}? This action
                                cannot be undone.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-[#EDEAE3] border-b border-gray-200 hover:bg-[#EDEAE3]">
                          <TableHead className="font-semibold text-brand text-left py-3 px-4 w-1/3">
                            Category Type
                          </TableHead>
                          <TableHead className="font-semibold text-brand text-left py-3 px-4 w-1/6">
                            Levels
                          </TableHead>
                          <TableHead className="font-semibold text-brand text-left py-3 px-4">
                            Escalation To
                          </TableHead>
                        </TableRow>
                      </TableHeader>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          ...
          <form
            onSubmit={form.handleSubmit(handleUpdateRule)}
            className="space-y-6"
          >
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-brand">Category Selection</CardTitle>
              </CardHeader>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-brand">Escalation Matrix</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                {(["e1", "e2", "e3", "e4", "e5"] as const).map((level) => (
                  <div key={level} className="space-y-3">
                    <Label className="text-base font-semibold text-brand">
