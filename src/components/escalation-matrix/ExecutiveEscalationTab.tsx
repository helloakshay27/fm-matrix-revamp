    ...
    <div className="space-y-8">
      ...
      <div className={`bg-white rounded-lg border border-gray-200 p-8 ${hasExecutive ? 'opacity-60' : ''}`}>
        <div className="space-y-6">
          {/* Header Row 1 */}
          <div className="grid grid-cols-6 gap-6 font-medium text-brand-accent text-sm border-b border-gray-200 pb-3">
            <div>Levels</div>
          ...
          {/* Header Row 2 */}
          <div className="grid grid-cols-6 gap-6 font-medium text-brand-accent text-sm">
            <div></div>
          ...
          {escalationData.map((item, index) => (
            <div key={item.level} className="grid grid-cols-6 gap-6 items-center py-2">
              <div className="bg-[#EDEAE3] px-3 py-2 rounded border text-sm font-semibold text-brand-accent">
                {item.level}
              </div>
      ...
      <Card className="border border-gray-200">
        <CardHeader className="border-b border-gray-200 bg-white">
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg font-semibold text-brand-accent">Executive Escalation Rules</CardTitle>
            <div className="flex items-center gap-3">
              <Label htmlFor="escalation-filter" className="text-sm font-medium text-brand-accent">
                Filter by Type
              </Label>
              ...
              {filteredRules.map((ruleGroup, index) => (
                <div key={`rule-group-${index}`} className="border border-gray-200 rounded-lg bg-white shadow-sm">
                  <div className="flex items-center justify-between p-4 border-b border-gray-100 bg-gray-50">
                    <div className="flex items-center gap-3">
                      <h3 className="text-base font-semibold text-brand-accent">Rule {index + 1}</h3>
                      <div className="flex gap-2">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-8 w-8 p-0 text-brand-accent hover:text-brand-hover"
                    ...
                    <Table>
                      ...
                      <TableHeader>
                        <TableRow className="bg-[#EDEAE3] border-b border-gray-200 hover:bg-[#EDEAE3]">
                          <TableHead className="font-semibold text-brand-accent text-left py-3 px-4 w-1/6">Levels</TableHead>
                          <TableHead className="font-semibold text-brand-accent text-left py-3 px-4">Escalation To</TableHead>
                          <TableHead className="font-semibold text-brand-accent text-left py-3 px-4 w-1/4">P1 Timing</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {[ 'E0', 'E1', 'E2', 'E3'].map((levelName) => {
                          ...
                          return (
                            <TableRow key={levelName} className="border-b border-gray-100 hover:bg-gray-50/50">
                              <TableCell className="py-4 px-4 align-top font-semibold text-brand-accent">
                                {levelName}
                              </TableCell>
                      ...
                      <div className="space-y-3">
                        {editData.map((item, idx) => (
                          <div key={item.level} className="grid grid-cols-6 gap-6 items-center py-1">
                            <div className="bg-[#EDEAE3] px-3 py-2 rounded border text-sm font-semibold text-brand-accent">{item.level}</div>
                            <div className="col-span-2">
                              <ReactSelect
                                isMulti
                                options={userOptions}
