...
export const MeterDetailsSection: React.FC<MeterDetailsProps> = ({
  ...
  const handleToggleClick = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    console.log('Meter toggle clicked, current state:', meterDetailsApplicable);
    onMeterDetailsToggle(!meterDetailsApplicable);
  };

  const handleHeaderClick = () => {
    console.log('Meter header clicked, current expanded state:', isExpanded);
    onToggle();
  };
  ...
  return (
    <div className="bg-white shadow-sm rounded-lg overflow-hidden">
          ...
          <div className="flex items-center gap-2 ml-4">
            <label className="relative inline-flex items-center cursor-pointer" onClick={handleToggleClick}>
              <input
                ...
                onChange={() => {}}
              />
              <div className={`w-11 h-6 ${meterDetailsApplicable ? 'bg-green-400' : 'bg-gray-300'} peer-focus:outline-none rounded-full peer transition-colors duration-200 ease-in-out after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all after:duration-200 after:ease-in-out ${meterDetailsApplicable ? 'after:translate-x-full after:border-white' : 'after:translate-x-0'}`}></div>
            </label>
      ...
      {isExpanded && (
        <div className={`transition-opacity duration-200 ${meterDetailsApplicable ? 'opacity-100' : 'opacity-40'}`}>
                ...
                <div className="flex items-center gap-6">
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      disabled={!meterDetailsApplicable}
                    />
