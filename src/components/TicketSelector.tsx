...
interface TicketSelectorProps {
  ...
  customOptions?: SelectorOption[];
  title?: string;
}

export function TicketSelector({ onSelectionChange, customOptions, title }: TicketSelectorProps) {
  const defaultOptions = customOptions ?? ticketOptions;
  ...
  const displayTitle = title ?? 'Ticket Selector';
      ...
      <PopoverContent className="w-64 p-0" align="end">
        <div className="bg-white border border-[hsl(var(--analytics-border))]">
          <div className="p-3 border-b border-[hsl(var(--analytics-border))]">
            <h3 className="font-medium text-[hsl(var(--analytics-text))]">{displayTitle}</h3>
          </div>
