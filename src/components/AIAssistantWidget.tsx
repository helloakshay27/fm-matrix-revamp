...
export const AIAssistantWidget: React.FC<AIAssistantWidgetProps> = ({ allowedModuleId }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await fetch(
                "https://dashboard.lockated.com/api-fastapi/ai-process/",
                {
                    ...
                    body: JSON.stringify({
                        ...
                        sub_module_id: selectedSubModuleId && selectedSubModuleId !== 'all' ? parseInt(selectedSubModuleId) : null,
                    }),
                }
            );
            ...
            const historyEntry: ChatHistory = {
                ...
                submodule: selectedModule?.submodules.find(s => s.id.toString() === selectedSubModuleId)?.name || ''
            };
                            ...
                            <div className="space-y-4 animate-in fade-in duration-500">
                                ...
                                {response.results && Object.keys(response.results).length > 0 && (
                                    <div className="space-y-4">
                                        {Object.entries(response.results).map(([key, data]: [string, any]) => {
                                            if (data.type === 'excel' && data.sheets) {
                                                return (
                                                    <div key={key} className="bg-white dark:bg-slate-800 rounded-lg p-4 border border-[#C4B89D]/40">
                                                        <div className="flex items-center justify-between mb-4">
                                                            ...
                                                            <Button
                                                                size="sm"
                                                                onClick={() => downloadAllSheets(response.results)}
                                                        ...
                                                        <div className="space-y-3">
                                                            {Object.entries(data.sheets).map(([sheetName, sheetData]: [string, any]) => (
                                                                <div key={sheetName} className="border border-[#C4B89D]/30 rounded-lg overflow-hidden">
                                                                    <div
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <div className="flex items-center gap-2">
                                                                            <Button
                                                                                ...
                                                                                onClick={(e) => {
                                                                                    e.stopPropagation();
                                                                                    downloadAsExcel(sheetData, sheetName);
                                                                                }}
                                                                    ...
                                                                    {expandedSheet === sheetName && sheetData.sample && sheetData.sample.length > 0 && (
                                                                        <div className="overflow-x-auto bg-white">
                                                                            <table className="w-full text-xs">
                                                                                ...
                                                                                <tbody>
                                                                                    {sheetData.sample.slice(0, 10).map((row: any, rowIdx: number) => (
                                                                                        <tr
                                                                                            key={rowIdx}
                                                                                            className="border-b border-slate-200 hover:bg-slate-50"
                                                                                        >
                                                                                            {sheetData.columns.map((col: string, colIdx: number) => (
                                                                                                <td
                                                                                                    key={colIdx}
                                                                            ...
                                                                            {sheetData.sample.length > 10 && (
                                                                                <div className="bg-[#DBC2A9]/10 px-4 py-2 text-center text-xs text-slate-600">
                                                                                    Showing 10 of {sheetData.sample.length} rows. Download Excel for full data.
                                                                                </div>
                                                                            )}
                                                                        </div>
                                                                    )}
                                ...
                                {selectedModule && selectedModule.submodules.length > 0 && (
                                    <div className="space-y-2 animate-in fade-in duration-300">
                                        <label className="text-sm font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                                            ...
                                            Select Sub-Module <span className="text-xs text-slate-500">(Optional)</span>
                                        </label>
                                        <Select
