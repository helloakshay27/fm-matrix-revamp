...
interface BCTaskEditModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
  baseUrl: string;
...
const SimpleTaskForm = ({
  ...
  useEffect(() => {
    if (isListening && transcript && activeId === "bc-simple-edit-description") {
  ...
  useEffect(() => {
    if (editData && !isPrefilled) {
      setFormData((prev) => ({
        ...
        cronExpression: editData.cron_expression || "",
      }));

      const parsedStart = parseDateObj(editData.start_date);
      const parsedEnd = parseDateObj(editData.due_date);
      if (parsedStart) setStartDate(parsedStart);
      if (parsedEnd) setEndDate(parsedEnd);

      if (editData.effort_duration) {
        setTotalWorkingHours(parseEffortDuration(editData.effort_duration));
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (Array.isArray(dateWiseHours) && dateWiseHours.length > 0) {
        taskAllocationTimesAttributes = dateWiseHours.map((item: any) => ({
          date: item.date,
          hours: item.hours,
      ...
      taskAllocationTimesAttributes.forEach((item: any, index: number) => {
        if (item.id) {
      ...
      await axios({ method, url, data: formDataPayload, headers: { Authorization: `Bearer ${token}` } });
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              {supported && (
                <IconButton
                  size="small"
                  onClick={() => {
                    if (isListening && activeId === "bc-simple-edit-description") {
                      stopListening();
                    } else {
                      const current = quillEditorRef.current?.root.innerHTML;
                      setBaseValue(current === "<p><br></p>" ? "" : current || "");
                      startListening("bc-simple-edit-description");
                    }
                  }}
                  sx={{
                    color: isListening && activeId === "bc-simple-edit-description" ? "#C72030" : "inherit",
                  }}
                >
                  {isListening && activeId === "bc-simple-edit-description" ? (
                    <Mic size={18} />
                  ) : (
                    <MicOff size={18} />
                  )}
...
export default BCTaskEditModal;
