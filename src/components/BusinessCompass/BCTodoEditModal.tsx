...
interface BCTodoEditModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSuccess?: () => void;
    editData: any;
...
const BCTodoEditModal = ({ isOpen, onClose, onSuccess, editData }: BCTodoEditModalProps) => {
    ...
    useEffect(() => {
        if (isListening && transcript) {
            if (activeId === "bc-todo-edit-title") {
                const newValue = baseValue ? `${baseValue} ${transcript}` : transcript;
                setTitle(newValue);
            } else if (activeId === "bc-todo-edit-description") {
                const newValue = baseValue ? `${baseValue} ${transcript}` : transcript;
    ...
    useEffect(() => {
        if (isOpen && editData) {
            ...
            if (editData.target_date) {
                setDate(editData.target_date.substring(0, 10));
            } else {
                setDate('');
            }
    ...
    const handleSubmit = async (e: any) => {
        try {
            ...
            const todoId = editData?.id || editData?.todo_id;
    ...
    return (
        ...
        >
            <DialogTitle className="relative py-6 !px-0">
                <h3 className="text-lg font-medium text-center">Edit ToDo</h3>
                <X
                                ...
                                endAdornment: supported && (
                                    <InputAdornment position="end">
                                        <IconButton
                                            size="small"
                                            onClick={() => {
                                                if (isListening && activeId === "bc-todo-edit-title") {
                                                    stopListening();
                                                } else {
                                                    setBaseValue(title);
                                                    startListening("bc-todo-edit-title");
                                                }
                                            }}
                                            color={isListening && activeId === "bc-todo-edit-title" ? "secondary" : "default"}
                                            sx={{ color: isListening && activeId === "bc-todo-edit-title" ? "#C72030" : "inherit" }}
                                        >
                                            {isListening && activeId === "bc-todo-edit-title" ? <Mic size={18} /> : <MicOff size={18} />}
                                        </IconButton>
                                    </InputAdornment>
                                ),
                            }}
                            ...
                            <div className="flex items-center justify-between mb-2">
                                ...
                                {supported && (
                                    <IconButton
                                        size="small"
                                        onClick={() => {
                                            if (isListening && activeId === "bc-todo-edit-description") {
                                                stopListening();
                                            } else {
                                                setBaseValue(description);
                                                startListening("bc-todo-edit-description");
                                            }
                                        }}
                                        color={isListening && activeId === "bc-todo-edit-description" ? "secondary" : "default"}
                                        sx={{ color: isListening && activeId === "bc-todo-edit-description" ? "#C72030" : "inherit" }}
                                    >
                                        {isListening && activeId === "bc-todo-edit-description" ? <Mic size={18} /> : <MicOff size={18} />}
                                    </IconButton>
                                )}
                            </div>
...
export default BCTodoEditModal;
