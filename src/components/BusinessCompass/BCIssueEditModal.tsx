...
const parseEffortDuration = (duration: string): number => {
...
interface BCIssueEditModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
  baseUrl: string;
...
const SimpleIssueForm = ({
  ...
  useEffect(() => {
    if (isListening && transcript && activeId === "bc-simple-issue-edit-description") {
  ...
  useEffect(() => {
    if (editData && !isPrefilled) {
      setFormData((prev) => ({
        ...prev,
        issueTitle: editData.title || "",
        description: editData.description || "",
        responsiblePerson: editData.responsible_person_id || "",
        issueType: editData.issue_type || "",
        priority: String(editData.priority_number || editData.priority || ""),
      }));

      const parsedStart = parseDateObj(editData.start_date);
      const parsedEnd = parseDateObj(editData.end_date);
      if (parsedStart) setStartDate(parsedStart);
      if (parsedEnd) setEndDate(parsedEnd);

      if (editData.estimated_hour) {
        setTotalWorkingHours(Number(editData.estimated_hour));
      }

      setIsPrefilled(true);
    }
  }, [editData, isPrefilled]);

  useEffect(() => {
    if (isPrefilled && quillEditorRef.current && editData?.description) {
      quillEditorRef.current.root.innerHTML = editData.description;
    }
  }, [isPrefilled, editData]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = issueId
        ...
        : `https://${baseUrl}/business_compass/issues.json`;
      ...
      fd.append("issue[status]", editData?.status || "open");
      ...
      toast.success(issueId ? "Issue updated successfully." : "Issue created successfully.");
      onSuccess?.();
      onClose();
    } catch (err: any) {
      ...
      if (errors && typeof errors === "object") {
        ...
        toast.error(issueId ? "Failed to update issue." : "Failed to create issue.");
      }
    } finally {
  ...
  return (
    <form className="pb-20 overflow-y-auto text-[12px]" onSubmit={handleSubmit}>
      <div id="addIssue" className="pr-3 max-w-[95%] mx-auto">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              {supported && (
                <IconButton
                  size="small"
                  onClick={() => {
                    if (isListening && activeId === "bc-simple-issue-edit-description") {
                      stopListening();
                    } else {
                      const current = quillEditorRef.current?.root.innerHTML;
                      setBaseValue(current === "<p><br></p>" ? "" : current || "");
                      startListening("bc-simple-issue-edit-description");
                    }
                  }}
                  sx={{
                    color: isListening && activeId === "bc-simple-issue-edit-description" ? "#C72030" : "inherit",
                  }}
                >
                  {isListening && activeId === "bc-simple-issue-edit-description" ? (
                    <Mic size={18} />
                  ) : (
                    <MicOff size={18} />
                  )}
        ...
        <Box sx={{ mb: 2 }}><Box sx={{ fontSize: "12px", mb: 1 }}>Attachments</Box><Attachments attachments={files} setAttachments={setFiles} /></Box>
...
export default BCIssueEditModal;
