...
import {
  ...
  ListboxOption,
  Transition,
} from "@headlessui/react"; // Keep for UserCustomDropdown
...
const EditableTextField = ({
  ...
  const handleKeyDown = (event) => {
    if (event.key === "Enter") {
      ...
      onUpdate(localValue, rowId, columnId);
      saveRow(rowId);
    }
  };
...
const initialTasksData = [
  {
    id: "T-01",
    taskTitle: "Task 1 (Expandable)",
    status: "Open",
    responsiblePerson: "Abdul Ghaffar",
    ...
    subRows: [
      {
        id: "T-01-S1",
        taskTitle: "Subtask 1 for Task 1",
        status: "In Progress",
        responsiblePerson: "Duhita Raut",
        startDate: "2025-01-01",
        endDate: "2025-01-02",
        duration: "0d:0h:0m:0s",
        priority: "Urgent",
      ...
      {
        id: "T-01-S2",
        taskTitle: "Subtask 2 for Task 1",
        status: "Open",
        responsiblePerson: "Charlie C",
  ...
  {
    id: "T-02",
    taskTitle: "Task 2 (Not Expandable)",
    status: "Completed",
    responsiblePerson: "Alice A",
...
const TaskTable = () => {
  const [data, setData] = useState(() =>
    initialTasksData.map((task) => ({
      ...
      subRows: task.subRows
        ? task.subRows.map((sub) => ({
          ...sub,
          duration: calculateDuration(sub.startDate, sub.endDate),
        }))
        : [],
    }))
  );
          ...
          if (row.id === rowId) {
            ...
            if (columnId === "startDate" || columnId === "endDate") {
              updatedRow.duration = calculateDuration(
                columnId === "startDate" ? newValue : updatedRow.startDate,
                columnId === "endDate" ? newValue : updatedRow.endDate
              );
            }
            return updatedRow;
          }
          if (row.subRows && row.subRows.length > 0) {
            return { ...row, subRows: updateRecursively(row.subRows) };
          }
          return row;
        });
      };
      ...
      const finalizeRecursively = (rows) => {
        return rows.map((r) => {
          ...
          if (r.subRows && r.subRows.length > 0) {
            return { ...r, subRows: finalizeRecursively(r.subRows) };
          }
          return r;
        });
      };
    ...
    () => [
      {
        ...
        cell: ({ row }) =>
          row.getCanExpand() ? (
            ...
            <span style={{ paddingLeft: `${row.depth * 1 + 0.5}rem` }}>
              &nbsp;
            </span>
          ),
      },
      {
        ...
        cell: ({ row, getValue }) => (
          <Link
            to={`/tasks/${row.original.id.replace("T-", "")}`}
            className="text-xs text-blue-600 hover:text-blue-800 hover:underline p-1"
          >
            <span style={{ paddingLeft: `${row.depth * 0.5}rem` }}>
              {getValue().toString().slice(0, 5)}
            </span>
          </Link>
        ),
      },
      {
        ...
        cell: ({ row, getValue, column, table }) => {
          const titleContent = (
            <span
              onClick={
                row.getCanExpand() && !row.original.isEditing
              ...
              style={{
                paddingLeft: `${!row.getCanExpand() && row.depth > 0 ? 1 : 0.3
                  }rem`,
              }}
            >
              {getValue()}
          ...
          return row.original.isEditing ? (
            <EditableTextField
              inputRef={row.original.isNew ? newRowFirstInputRef : null}
              value={getValue()}
      ...
      },
      {
        ...
        cell: ({ row, getValue, column, table }) =>
          row.original.isEditing ? (
            ...
            getValue() || <span className="text-gray-400 p-3">Select date</span>
          ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row, getValue, column, table }) =>
          row.original.isEditing ? (
            ...
            <span className="text-xs p-3 ">{getValue()}</span>
          ),
      },
      {
  ...
  const canShowAddPlaceholder = !data.some(
    (row) =>
      row.isNew ||
      row.isEditing ||
      (row.subRows && row.subRows.some((sr) => sr.isNew || sr.isEditing))
  );
  ...
  const numEmptyRowsToFill = Math.max(
    0,
    MIN_DISPLAY_ROWS - effectiveRowCountForHeight
  );
  const totalRowsForHeightCalc = Math.max(
    MIN_DISPLAY_ROWS,
    effectiveRowCountForHeight
  );
      ...
      >
        <table className="w-full  table-auto  text-sm table-fixed">
          <thead className="sticky top-0 bg-gray-50">
            {" "}
            {/* Ensure header z-index is lower than dropdowns */}
              ...
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <th
                    ...
                    className="border-r-2 p-2 text-center text-gray-600 font-semibold"
                  >
                    {flexRender(
            ...
            {actualDataRows.map((row) => (
              <Fragment key={row.id}>
                <tr
                  ...
                  className={`hover:bg-gray-50 even:bg-gray-100 ${isListboxOpen && editingRowId === row.original.id
                      ? "relative z-30"
                      : ""
                    }`} // Example: lift row with open dropdown
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <td
                      ...
                      className={`border-r-2 text-left p-0 align-middle`}
                    >
