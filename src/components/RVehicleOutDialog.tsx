...
export const RVehicleOutDialog = ({ isOpen, onClose, vehicleNumber }: RVehicleOutDialogProps) => {
  const handleSubmit = () => {
    console.log('Marking vehicle as Out:', vehicleNumber);
    // Handle the "Out" action here
    onClose();
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-sm bg-white [&>button]:hidden">
        <DialogHeader className="flex flex-row items-center justify-between border-b pb-4">
          <DialogTitle className="text-lg font-semibold">Registered Vehicle Out</DialogTitle>
          <Button
            variant="ghost"
            size="icon"
            onClick={onClose}
