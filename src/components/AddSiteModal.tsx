...
import { siteService, SiteFormData, SiteData, CompanyOption, HeadquarterOption, RegionOption } from '@/services/siteService';
...
export const AddSiteModal: React.FC<AddSiteModalProps> = ({
  ...
  const fetchDropdownData = async () => {
    ...
    try {
      ...
      // Fetch companies
      const companiesResponse = await fetch(getFullUrl('/pms/company_setups/company_index.json'), {
        method: 'GET',
        headers: {
      ...
      if (companiesResponse.ok) {
        ...
        if (companiesResult && companiesResult.code === 200 && Array.isArray(companiesResult.data)) {
          companiesData = companiesResult.data;
        } else if (companiesResult && Array.isArray(companiesResult.companies)) {
      ...
      // Fetch headquarters
      const headquartersResponse = await fetch(getFullUrl('/headquarters.json'), {
        headers: {
          'Authorization': getAuthHeader(),
      ...
      if (headquartersResponse.ok) {
        ...
        } else if (headquartersResult && headquartersResult.headquarters && Array.isArray(headquartersResult.headquarters)) {
          headquartersData = headquartersResult.headquarters;
        } else if (headquartersResult && headquartersResult.data && Array.isArray(headquartersResult.data)) {
          headquartersData = headquartersResult.data;
        }
      } else {
        console.error('Failed to fetch headquarters:', headquartersResponse.statusText);
      ...
      if (regionsResponse.ok) {
        ...
        } else if (regionsResult && regionsResult.regions && Array.isArray(regionsResult.regions)) {
          regionsData = regionsResult.regions;
        } else if (regionsResult && regionsResult.data && Array.isArray(regionsResult.data)) {
        ...
        {isLoadingDropdowns ? (
          <div className="flex items-center justify-center py-8">
            ...
            <div className="text-xs text-gray-400 mt-2">
              Debug: Companies: {companies.length}, HQ: {headquarters.length}, Regions: {regions.length}
            </div>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  {headquarters.length === 0 ? (
                    <SelectItem value="no-data" disabled>No headquarters found</SelectItem>
                  ) : (
                    headquarters.map((hq) => (
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  {filteredRegions.length === 0 ? (
                    <SelectItem value="no-data" disabled>No regions found</SelectItem>
                  ) : (
          ...
          <div className="flex justify-end space-x-2 pt-4">
            ...
            <Button
              type="submit"
              disabled={isLoading || !formData.name || !formData.company_id || !formData.headquarter_id }
            >
