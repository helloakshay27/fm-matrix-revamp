...
export const EditStatusDialog = ({
  ...
  useEffect(() => {
    ...
    setSelectedStatus(currentStatusId ? String(currentStatusId) : '');
    setRcaTemplateIds([]);
    setCorrectiveActionTemplateIds([]);
    setPreventiveActionTemplateIds([]);
    ...
    const fetchAll = async () => {
      try {
        const [statusRes, templateRes] = await Promise.all([
          apiClient.get('/pms/admin/complaint_statuses.json'),
          apiClient.get('/communication_templates.json')
        ]);

        setStatuses(statusRes.data || []);
        setCommunicationTemplates(templateRes.data || []);
      } catch {
      ...
      if (complaintId) {
        try {
          const res = await apiClient.get(`/pms/admin/complaints/${complaintId}.json`);
          const data = res.data;
          setTicketData(data);
    ...
    fetchAll();
  }, [open, complaintId, currentStatusId]);
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      if (ticketData?.asset_service === 'Asset') {
        ...
        formData.append('asset_id', '');
        formData.append('service_id', String(ticketData.asset_or_service_id || ''));
      }

      const toastId = toast.loading('Updating status...');
      await apiClient.post('/complaint_logs.json', formData);
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            <Button
              onClick={handleApply}
              disabled={loading}
              className="flex-1 bg-[#8B4B8C] text-white"
            >
              Apply
            </Button>
