...
interface PowerConsumptionTopManagementCardProps {
  startDate?: string;
  endDate?: string;
}
...
const TILES: TileConfig[] = [
  {
    ...
    unit: "EBKWH",
    endpoint: "/utility_dashboard/card_ht_main_meter_total_consumption",
    bg: "#EFEFFB",
    color: "#6B5EA8",
    defaultInfo: "Total power consumption drawn from the HT main meter (mains).",
  },
  {
    key: "renewable",
    label: "Total Renewable",
    unit: "KWH",
    endpoint: "/utility_dashboard/card_transformer_total",
    bg: "rgba(183,220,212,0.30)",
    color: "#2E7D6B",
    defaultInfo: "Total renewable energy consumption.",
  },
  {
    key: "dg",
    label: "DG Total",
    unit: "DGKWH",
    endpoint: "/utility_dashboard/card_dg_total",
    bg: "rgba(227,144,144,0.15)",
    color: "#D97655",
    defaultInfo: "Total power consumption from DG sets.",
  },
  {
    key: "ev",
    label: "EV Consumption",
    unit: "KWH",
    endpoint: "/utility_dashboard/card_ev_consumption",
    bg: "rgba(237,196,136,0.25)",
    color: "#B8860B",
    defaultInfo: "Total EV charging power consumption.",
  },
];

// The 4 card_* endpoints below haven't been hit against a live backend from
// this environment, so each value is read tolerantly — same flat
// { response, info: { info | formula } } shape already assumed for the
// sibling card_* utility endpoints elsewhere in this app (EnergyIntensityCard,
// CarbonEmissionCard, FuelConsumptionCard) — instead of trusting one exact
// field name.
...
const PowerConsumptionTopManagementCard: React.FC<PowerConsumptionTopManagementCardProps> = ({
  startDate,
  endDate,
}) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-md p-4">
      <div className="mb-6 pb-3 border-b border-gray-200 -mx-4 px-4 pt-3">
        ...
        >
          Power Consumption Top Management
        </h3>
      </div>
...
export default PowerConsumptionTopManagementCard;
