...
const UtilityConsumptionCard: React.FC<Props> = ({ data }) => {
  const root = data?.data ?? data ?? {};
  const kpi = root?.response ?? root ?? {};

  const formatKwh = (val: any) =>
    val ? `${Number(val).toLocaleString()} kWh` : '0 kWh';

  const stats = [
    { label: 'Power Consumption', value: formatKwh(kpi.power_consumption) },
    { label: 'DG Total', value: formatKwh(kpi.dg_total) },
    { label: 'Solar', value: formatKwh(kpi.solar) },
    { label: 'Renewable Total', value: formatKwh(kpi.renewable_total) },
  ];
...
export default UtilityConsumptionCard;
