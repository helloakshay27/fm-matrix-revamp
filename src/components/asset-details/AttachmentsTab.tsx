...
interface Asset {
  ...
  asset_other_uploads?: { id: number; document: string; document_name: string }[];
...
export const AttachmentsTab: React.FC<AttachmentsTab> = ({ asset }) => {
  const [activeType, setActiveType] = useState(attachmentTypes[0].name);

  const filesMap: Record<string, { id: number; document: string; document_name: string }[]> = {
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 overflow-x-auto pb-2">
        {attachmentTypes.map((type, index) => (
          <Button
            ...
            className={`flex items-center gap-2 whitespace-nowrap min-w-fit px-4 py-2 rounded-lg ${type.name === activeType ? "bg-blue-500 hover:bg-blue-600 text-white border-blue-500" : "border-gray-300 text-gray-700 hover:bg-gray-50 bg-white"}`}
            onClick={() => setActiveType(type.name)}
          >
            <div className="w-4 h-4 rounded flex-shrink-0 bg-red-700"></div>
            <span className="text-sm font-medium">{type.name}</span>
            <span className="text-xs">{filesMap[type.name].length} Files</span>
          </Button>
        ))}
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {filesMap[activeType].length === 0 ? (
            <div className="col-span-full text-center text-gray-400 py-8">No files available.</div>
          ) : (
            filesMap[activeType].map((file, index) => (
              <div key={file.id || index} className="flex flex-col items-center p-4 border rounded-lg hover:shadow-md transition-shadow bg-white">
                {/* File Preview */}
                <a href={file.document} target="_blank" rel="noopener noreferrer" className="w-full h-32 bg-gray-100 rounded mb-3 flex items-center justify-center border overflow-hidden">
