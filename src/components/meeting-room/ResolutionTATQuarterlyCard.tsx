...
export const ResolutionTATQuarterlyCard: React.FC<ResolutionTATQuarterlyCardProps> = ({
  ...
  const chartData = useMemo(() => {
    ...
    console.log('🔍 ResolutionTATQuarterlyCard - performanceArray:', performanceArray);
    ...
    if (Array.isArray(performanceArray) && performanceArray.length > 0) {
      return performanceArray
        .filter((item: any) => {
          ...
          const hasCurrentData = item.current_period?.resolution_tat?.total_tickets > 0;
          const hasPreviousData = item.previous_period?.resolution_tat?.total_tickets > 0;
          ...
          <>
            ...
            <div className="p-3 rounded-md">
              <p className="text-xs text-gray-700">
                ...
                resolution TAT achieved in the current and previous quarter.
              </p>
            </div>
          </>
        )}
