...
interface CommonAreaCategoryWiseProactiveCardProps {
  data: UnitCategorywiseData | null;
  onDownload?: () => void;
  className?: string;
}

// Same response shape as chart_unit_categorywise (UnitCategoryWiseCard.tsx,
// confirmed live) — this is the common-area + proactive-only sibling of that
// same endpoint family, per the "New FM Dashboard Collection" at the repo
// root.
export const CommonAreaCategoryWiseProactiveCard: React.FC<CommonAreaCategoryWiseProactiveCardProps> = ({
  data,
  onDownload,
  className = ""
}) => {
    ...
    <div className={`bg-white rounded-xl shadow-sm ${className}`}>
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        <h3 className="text-base font-semibold text-gray-900" style={{ fontFamily: 'Work Sans, sans-serif' }}>
          Common Area Category-wise Tickets (Proactive)
        </h3>
        {onDownload && (
          <Download
            data-no-drag="true"
            ...
            {chartData.length > 0 ? (
              ...
              <div className="flex items-center justify-center h-full">
                <div className="text-center py-8 text-gray-500">
                  No proactive common area category-wise data available for the selected date range
                </div>
              </div>
            )}
          </ResponsiveContainer>
