...
export const AMCServiceStatsCard: React.FC<AMCServiceStatsCardProps> = ({
  ...
  const getStatusIcon = (type: string) => {
    switch (type) {
      case "Completed":
        return (
          <CheckCircle className="w-5 h-5" style={{ color: palette.primary }} />
        );
      case "Pending":
        return (
          <Clock className="w-5 h-5" style={{ color: palette.primaryLight }} />
        );
      case "Overdue":
        return (
          <AlertTriangle
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {data.map((item, index) => {
                ...
                return (
                  <div
                    key={index}
                    className={`text-center p-4 ${colors.bg} rounded-xl border ${colors.border} shadow-sm`}
                  >
                    <div className="flex items-center justify-center mb-2">
                      {getStatusIcon(item.type)}
                      <span className="text-sm font-medium ml-2 text-[#1A1A1A]">
                        {item.type}
                      </span>
                    </div>
