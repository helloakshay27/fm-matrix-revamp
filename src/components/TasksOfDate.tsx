...
const CountdownTimer = ({ targetDate }) => {
  ...
  return <div className="text-left text-xs">{countdown}</div>;
};
  ...
  const todayAllocation = (task.allocation_times || [])
    ...
    .reduce(
      (acc, a) => {
        ...
        const totalHours =
          acc.hours + (a.hours || 0) + Math.floor(totalMins / 60);
...
const DroppableDay = ({
  ...
  const calculationHrs =
    typeof dateInfo.allocated_hours_num !== "undefined"
    ...
    >
      ...
      {dateInfo.isPending && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="bg-yellow-50 text-yellow-800 text-xs px-2 py-1 rounded">
            Processing...
          </div>
        </div>
      )}
...
const TasksOfDate = ({
  ...
  const selectedHoursNum =
    (Number(selectedDayAvailability?.allocated_hours) || 0) +
    (availabilityUpdates[selectedDateString] || 0);
